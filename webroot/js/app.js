...
caseapp.controller('taskController', function ($scope,$cacheFactory,$compile, $http, osCommonMethods, $interval, $timeout) {
    ...
    $scope.isRestrictedPlan = function() { return false; };
    $scope.isTrialPlan = function() { return false; };
    $scope.isExpiredPlan = function() { return false; };
...
//    };
        ...
        $.post(HTTP_ROOT + 'easycases/taskactions', {
            ...
            if (res) {
                ...
                if (res.succ) {
                    ...
                    $scope.$apply();
                    if (actiontype.toLowerCase() == 'new') {
                        showTopErrSucc('success', _('Status of task #') + taskNum + ' changed to ' + actiontype + '.');
                    } else {
    ...
    $scope.createlog = function (casesid, title, index) {
        ...
        $.post(HTTP_ROOT + "easycases/checkDependantActionAllowed", {
            cid: casesid
            ...
            if (dependant_task_action_allowed && dependant_task_action_allowed == 'No') {
                ...
                $(".slct_task >span").html(_("Select Task"));
                if (clkdDateExst) {
                ...
                $.post(HTTP_ROOT + "easycases/existingTask", {
                    'projuniqid': prjunid,
                    'list': 'list',
                    'tid': casesid
                }, function (data) {
...
//                        if (logid != '') {
//                            $.ajax({
//                                url: HTTP_ROOT + "easycases/timelogDetails",
//                                data: {
//                                    'projuniqid': prjunid,
//                                    logid: logid
//                                },
        ...
        if (conf == false) {
            ...
            $.post(strurl, {
                ...
                if (data) {
                    ...
                    $('#caseResolve').val('');
                    if (typ == 'all') {
                        showTopErrSucc('success', _("Task ") + al_cno_d + _(" archived successfully."));
                    } else {
    ...
    $scope.deleteCase = function (id, cno, pid, dtlsid, recurring,index) {
        if (confirm("Are you sure you want to delete the task #" + cno + " ?")) {
            ...
            $.post(strurl, {
                ...
                if (data == 0) {
                    $("#" + curRow).fadeIn(100);
                    showTopErrSucc('error', _("Failed to delete task #") + cno + ". Please try again.");
                } else {
                    $scope.res.caseAll = removeScopeObj($scope.res.caseAll,index);
...
//                    $("#" + dtlsid).remove();
    ...
    $scope.displayAssignToMem = function (csId, project, caseAssgnUid, caseUniqId, page, cno, client_status) {
        ...
        if (countJS(PUSERS) && PUSERS[project]) {
            ...
            $.post(HTTP_ROOT + "easycases/ajaxAssigntoMem", {
                "project": project
            }, function (data) {
                if (data) {
                    PUSERS = data;
 ...
 */
angular.module('commonMethods').filter('t', function () {
