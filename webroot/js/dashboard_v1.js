...
function ajaxCaseView(page) {
    ...
    if (((page == 'taskgroups' || hashtag[0] == 'taskgroups') && page != 'tasks')) {
        var project_uniq_id = $('#projFil').val();
        if (project_uniq_id == 'all') {
            showTopErrSucc('error', _('Please select a project to view task groups.'));
    ...
    if ($('#lviewtype').val() && $('#lviewtype').val() == 'compact') {
    ...
    var caseBunit = $('#caseBunit').val();
    var caseTeamUser = $('#caseTeamUser').val();
    var caseApprovalStatus = $('#caseApprovalStatus').val();
    var caseEpics = $('#caseEpics').val();
    var caseFeatures = $('#caseFeatures').val();
    var caseSkill = $('#caseSkill').val();
    ...
    $.post(strURL + strAction, {
        ...
        'casegroupby': (task_grp_by == 'None' || empty(task_grp_by)) ? "None" : task_grp_by,
        caseNew: caseNew,
        limit1: limit1,
        limit2: limit2,
        casePageType: page,
        ...
        if (res) {
            if (typeof client !== 'undefined' && client && typeof res.iotoserver != 'undefined') {
                try {
            ...
            $.material.init();
            setTimeout(function () {
                loadCaseMenu(HTTP_ROOT + "requests/ajaxCaseMenu", {
                    "projUniq": projFil,
                    "pageload": 0,
                    "page": "dashboard",
            ...
            if (hashtag[0] != "tasks" && hashtag[0] != 'taskgroups') {
            ...
            $('.due_dt_tlist').off().on('click', function (e) {
                ...
                var is_splitted = $(this).attr('data-split');
                if (is_splitted == 0) {
            ...
            if (localStorage.getItem("tour_open_chk") == '1') {
                ...
                hopscotch.startTour(GBl_tour, 0);
            }

            $('.show_task_filter').show();
        }
        setTimeout(function () {
...
function filterBacklog(id) {
    ...
    setGetFilterSprint('backlog', checkedArr, 0);
    if (typeof arguments[1] != 'undefined') {
        return checkedArr;
    } else {
        ajaxBacklogView(checkedArr);
    }
}

function ajaxTaskGroupBy(obj) {
    createCookie("TASKSORTBY", JSON.stringify([]), 365, DOMAIN_COOKIE);
    var type = $(obj).attr("data-type");
    var task_group_by = localStorage.getItem('AJAX_TASK_GROUPBY');
    if (type == task_group_by) {
        remember_filters('AJAX_TASK_GROUPBY', 'None');
    } else {
        var type = $(obj).attr("data-type");
...
function TaskGroupByItem() {
    ...
    if (task_group_by != '' && task_group_by != null) {
        if (task_group_by == 'Assign to') {
            ...
            $('.dynamic_task_title').attr('title', 'Grouped By : ' + actLabel);
            $('.A_ssign_To').attr("checked", "checked");
        } else if (task_group_by == 'Status') {
            var actLabel = 'Status';
            $('.dynamic_task_title').attr('title', 'Grouped By : ' + actLabel);
            $('.S_tatus').attr("checked", "checked");
        } else if (task_group_by == 'Date') {
            var actLabel = 'Updated Date';
            $('.dynamic_task_title').attr('title', 'Grouped By : ' + actLabel);
            $('.D_ate').attr("checked", "checked");
        } else if (task_group_by == 'Priority') {
            var actLabel = 'Priority';
            $('.dynamic_task_title').attr('title', 'Grouped By : ' + actLabel);
            $('.P_riority').attr("checked", "checked");
        } else if (task_group_by == 'None') {
            var actLabel = 'None';
            $('.dynamic_task_title').attr('title', 'Grouped By : ' + actLabel);
            $('.N_one').attr("checked", "checked");
        } else {
            $('.N_one').attr("checked", "checked");
        }
    } else {
...
function clearBacklogFilter() {
    if ($.trim($('#inner-search-backlog').val()) != '') {
        $('#inner-search-backlog').val('');
        $('#srch_inner_load1-bklog').show();
        ajaxBacklogView(filterBacklog('quk_fil_bklog', 1));
    }
    $('.filterBacklogicon').hide();
    $('.filterBacklogicon_srch').show();
}

function ajaxBacklogView(inpt) {
    ...
    if ((!$('.menu-backlog').length || !$('.menu-sprint').length) && METHODOLOGY != 'scrum') {
        window.location.href = HTTP_ROOT + "dashboard";
        return false;
    }
    $('#task_paginate').html('');
    var strURL = HTTP_ROOT;
    strURL = strURL + "requests/";
    var projFil = $('#projFil').val();
    var cko = getCookie('TASKGROUPBY');
    var caseChangeAssignto = $('#caseChangeAssignto').val();
    $(".side-nav li").removeClass('active');
    $(".menu-backlog").addClass('active');
    if (!(localStorage.getItem("theme_setting") === null)) {
        var th_set_obj = JSON.parse(localStorage.getItem("theme_setting"));
        $(".side-nav li").each(function () {
            $(this).removeClass(th_class_str);
        });
        $('.menu-backlog').addClass(th_set_obj.sidebar_color + " gradient-shadow");
    }
    if ($('#lviewtype').val() && $('#lviewtype').val() == 'compact') {
        $('#select_view div').removeClass('disable');
        $('#kbview_btn').removeClass('disable');
    ...
    var taskgroup_fil = '';
    var hashtag = parseUrlHash(urlHash);
    var caseSearch = $.trim($("#inner-search-backlog").val());
    $("#case_search").val("");
    var caseMenuFilters = $('#caseMenuFilters').val();
    var case_srch = $('#case_srch').val();
    var projIsChange = $('#projIsChange').val();
    ...
    var hashtag = parseUrlHash(urlHash);
    var casebckLogSort = [];
    if ($('#cas_sort_dir,#def_cas_sort_dir,#cas_sort_dir_asc,#cas_sort_dir_dsc').hasClass('dn')) {
        localStorage.removeItem("backlog_case_sort");
    }
    if (localStorage.backlog_case_sort) {
        casebckLogSort = JSON.parse(localStorage.getItem("backlog_case_sort"));
    }
    $('#startTourBtn').hide();
    var strAction = 'case_backlog';
    $.post(strURL + strAction, {
        projFil: projFil,
        mstype: mstype,
        priFil: priFil,
        ...
        if (res) {
            ...
            var projIsChange = $('#projIsChange').val();
            var caseMenuFilters = $('#caseMenuFilters').val();
            $("#caseViewSpan").html(tmpl("case_backlogs_tmpl", res));
            notShowEmptyDropdown();

            function restoreSearchFieldFocus(selector) {
                if (caseSearch && caseSearch.trim() !== '') {
                    setTimeout(function () {
                        var searchField = $(selector);
                        searchField.focus();
                        // Move cursor to end of text
                        var length = searchField.val().length;
                        searchField[0].setSelectionRange(length, length);
                    }, 50);
                }
            }

            // Restore focus to search field after results load
            restoreSearchFieldFocus('#inner-search-backlog');
            if (res.parallel_sprint == 1) {
                $('.openSprint').text(0);
                $('.wipSprint').text(0);
                $('.closeSprint').text(0);
                $.each(res.startedStatus, function (key, data) {
                    $("#st_crt_btn_" + key).find('.openSprint').text(data.opn);
                    $("#st_crt_btn_" + key).find('.wipSprint').text(data.wip);
                    $("#st_crt_btn_" + key).find('.closeSprint').text(data.done);
                });
            } else {
                $.each(res.startedStatus, function (key, data) {
                    $("#st_crt_btn_" + key).find('.openSprint').text(data.opn);
            ...
            if ($.trim(caseSearch) != '') {
                ...
                $('.filterBacklogicon_srch').hide();
            }
            if (getCookie("TASKGROUPBY") != "" || page == 'taskgroup' || page == 'backlog') {
                var ignoreTr = '.white_bg_tr,.quicktskgrp_tr,.quicktskgrp_tr,.quicktskgrp_tr_lnk,thead > tr';
                $('.sortby_btn').prop('disabled', true);
                $('.sortby_btn').addClass('disable-btn');
                $(".compactview_tbl").sortable({
                    ...
                    update: function (ev, ui) {
                        ...
                        $.post(url, {
                            ...
                            if (res1) {
                                showTopErrSucc('success', _('Sprint reordered successfully') + '.');
                            }
                        });
                    }
                });
                ...
                $(".compactview_tbl > tbody").sortable({
                    ...
                    stop: function (event, ui) {
                        ...
                        var id = $('#' + tr_id).prev('tr').not(ignoreTr).attr("id");
                        if (typeof id == 'undefined') { }
                        var prev_miid = $(event.target).find('tr:first-child').attr('id').replace("empty_milestone_tr", "");
                        var drped_idx = $(".compactview_tbl tr").not(ignoreTr).index(ui.item[0]);
                        var p_id = 0;
                        var n_id = 0;
                        var p_idx_id = drped_idx + 1;
                        ...
                        if (typeof n_id == 'undefined' && typeof p_id == 'undefined') {
                            ...
                            if (parseInt($('#st_crt_btn_' + n_mlstn_id).attr('data-status')) && n_mlstn_id != prev_miid) {
                                if (!confirm(_('The scope of the sprint will be changed, do you still want to proceed?'))) {
                                    return false;
                                }
                            }
                            update_sequence(caseIds, n_mlstn_id, project_id, prev_miid);
                        }
                    },
                    update: function (ev, ui) {
                        delete_cookie('TASKSORTBY');
            ...
            $.material.init();
            var $eventSelect = $("#bcklog_sort_by").select2();
            $eventSelect.on('select2:select', function (e) {
                let data = e.params.data;
                let sort_data = {};
                if (data.id != "") {
                    $('#cas_sort_dir').removeClass('dn');
                    if (localStorage.backlog_case_sort) {
                        var exist_sort = JSON.parse(localStorage.getItem("backlog_case_sort"));
                        if (exist_sort.dir != "") {
                            exist_sort.id = data.id;
                            localStorage.removeItem("backlog_case_sort");
                            localStorage.setItem("backlog_case_sort", JSON.stringify(exist_sort));
                            var ids_array = typeof getCookie('PREOPENED_TASK_GROUP_IDS') != 'undefined' ? JSON.parse(getCookie('PREOPENED_TASK_GROUP_IDS')) : [];
                            $.each(ids_array, function (key, val) {
                                if (val != '0') {
                                    if ($('.os_sprite' + val).find('.material-icons').text() == 'keyboard_arrow_down') {
                                        showBacklogByMilestone(val, res.pid, $('.os_sprite' + val));
                                    }
                                }
                            });
                            showBacklogByMilestone(0, res.pid, $('.os_sprite0'));
                        } else {
                            $('#def_cas_sort_dir').removeClass('dn');
                        }
                    } else {
                        sort_data.id = data.id;
                        sort_data.dir = 'desc';
                        localStorage.setItem("backlog_case_sort", JSON.stringify(sort_data));
                        $('#cas_sort_dir').removeClass('dn');
                        $('#def_cas_sort_dir').removeClass('dn');
                    }
                } else {
                    $('#cas_sort_dir,#def_cas_sort_dir,#cas_sort_dir_asc,#cas_sort_dir_dsc').addClass('dn');
                    if (localStorage.backlog_case_sort) {
                        localStorage.removeItem("backlog_case_sort");
                    }
                    var ids_array = typeof getCookie('PREOPENED_TASK_GROUP_IDS') != 'undefined' ? JSON.parse(getCookie('PREOPENED_TASK_GROUP_IDS')) : [];
            ...
            $(document).on('click', '.case_srt_icons', function () {
                let sort_data = JSON.parse(localStorage.getItem("backlog_case_sort"));
                let sort_dir = $(this).attr('data-dir');
                $('#cas_sort_dir').removeClass('dn');
                switch (sort_dir) {
                    case "desc":
                        $('#def_cas_sort_dir,#cas_sort_dir_asc').addClass('dn');
                        $('#cas_sort_dir_dsc').removeClass('dn');
                        sort_data.dir = sort_dir;
                        break;
                    default:
                        $('#def_cas_sort_dir,#cas_sort_dir_dsc').addClass('dn');
                        $('#cas_sort_dir_asc').removeClass('dn');
                        sort_data.dir = sort_dir;
                }
                localStorage.removeItem("backlog_case_sort");
                localStorage.setItem("backlog_case_sort", JSON.stringify(sort_data));
            ...
            var act_filt_txt_quk = '';
            var act_filt_txt_assn = '';
            var act_filt_txt_type = '';
            var act_filt_txt_status = '';
            if (res.BkFilters) {
                $(".quk_fil_bklog").each(function (i) {
                    if ($.inArray($(this).val(), res.BkFilters) != -1) {
                        $(this).prop('checked', true);
                        if ($(this).next('label').attr('for') != 'quk_last_upd' && $(this).next('label').attr('for') != 'quk_assn') {
                            if ($(this).next('label').attr('for').indexOf('quk_type_') !== -1) {
                                act_filt_txt_type = (act_filt_txt_type == '') ? $(this).next('label').text() : act_filt_txt_type + ' , ' + $(this).next('label').text();
                            } else if ($(this).next('label').attr('for').indexOf('quk_status_') !== -1) {
                                act_filt_txt_status = (act_filt_txt_status == '') ? $(this).next('label').text() : act_filt_txt_status + ' , ' + $(this).next('label').text();
                            } else {
                                act_filt_txt_assn = (act_filt_txt_assn == '') ? $(this).next('label').text() : act_filt_txt_assn + ' , ' + $(this).next('label').text();
                            }
                            $('.assn_fil_bklog_drop').addClass('open');
                        } else {
                            act_filt_txt_quk = (act_filt_txt_quk == '') ? $(this).next('label').text() : act_filt_txt_quk + ' , ' + $(this).next('label').text();
                        }
                    }
                });
                if (act_filt_txt_quk != '') {
                    $('#quk_filter_backlog').text(act_filt_txt_quk).attr('title', act_filt_txt_quk);;
                    $('#quk_filter_backlog').css('color', "#2d6dc4");
                } else {
                    $('#quk_filter_backlog').text(_('Quick Filter')).attr('title', _('Quick Filter'));;
                }
                if (act_filt_txt_assn != '') {
                    $('#assn_filter_backlog').text(act_filt_txt_assn).attr('title', act_filt_txt_assn);;
                    $('#assn_filter_backlog').css('color', "#2d6dc4");
                } else {
                    $('#assn_filter_backlog').text(_('Assignee')).attr('title', _('Assignee'));;
                }
                if (act_filt_txt_type != '') {
                    $('#type_filter_backlog').text(act_filt_txt_type).attr('title', act_filt_txt_type);;
                    $('#type_filter_backlog').css('color', "#2d6dc4");
                } else {
                    $('#type_filter_backlog').text(_('Task Types')).attr('title', _('Task Types'));;
                }
                if (act_filt_txt_status != '') {
                    $('#status_filter_backlog').text(act_filt_txt_status).attr('title', act_filt_txt_status);;
                    $('#status_filter_backlog').css('color', "#2d6dc4");
                } else {
                    $('#status_filter_backlog').text(_('Status')).attr('title', _('Status'));;
                }
            }
            $("#empty_milestone_tr0").find('.os_sprite0').trigger('click');
            var ids_array = typeof getCookie('PREOPENED_TASK_GROUP_IDS') != 'undefined' ? JSON.parse(getCookie('PREOPENED_TASK_GROUP_IDS')) : [];
            $.each(ids_array, function (key, val) {
                if (val != '0') {
                    $("#empty_milestone_tr" + val).find('.os_sprite' + val).trigger('click');
                }
            });
        }
    });
...
function setGetFilterSprint(name, value, type) {
    if (type == 1) {
        var storFil = localStorage.getItem(name);
        if (storFil != '') {
...
function clearSprintFilter(type) {
    const isBacklog = type === "backlogByStatus";
    const searchField = isBacklog ? '#inner-search-backlogstatus' : '#inner-search-sprint';
    const loader = isBacklog ? '#srch_inner_load1-backlogstatus' : '#srch_inner_load1-sprint';
    const ajaxFunction = isBacklog ? ajaxBacklogBystatusView : ajaxSprintView;
    if ($.trim($(searchField).val()) !== '') {
        $(searchField).val('');
        $(loader).show();
        ajaxFunction(filterSprint('quk_fil_sprint', 1));
    }
    $('.filterSprinticon').hide();
    $('.filterSprinticon_srch').show();
}

function sprintCheckboxdueDate(x) {
    localStorage.removeItem("SPRINT_DUE_DATE");
    $("#chk_dated").val(0);
    if (x && !$("#sprint_duedate_" + x).is(":checked")) {
        sprintCheckboxdueDate("");
        return false;
    }
    $(".cbox_dated").removeAttr("checked");
    if (x) {
        $("#sprint_duedate_" + x).prop("checked", "checked");
        filterSprint('quk_fil_sprint');
    } else {
        filterSprint('quk_fil_sprint');
    }
}
$(function () {
    setTimeout(function () {
        if ((localStorage.getItem("SPRINT_DUE_DATE") != undefined)) {
            $("#due_filter_sprint").html(localStorage.getItem("SPRINT_DUE_DATE"));
        }
    }, 10000);
});
sprint_t = "";
...
function sprintSearchDuedate(x) {
    var start_date = document.getElementById("sprint_duefrm");
    var end_date = document.getElementById("sprint_dueto");
    var errMsg;
    var done = 1;
    ...
    if (done == 0) {
        ...
        var from = $("#sprint_duefrm").val();
        var to = $("#sprint_dueto").val();
        document.getElementById("sprint_duedate_overdue").checked = false;
        document.getElementById("sprint_duedate_24").checked = false;
        var x = "sprint_due_" + from + "_" + to;
        var sdd = from + " - " + to;
        localStorage.setItem("SPRINT_DUE_DATE", sdd);
        $("#due_filter_sprint").html(localStorage.getItem("SPRINT_DUE_DATE"));
        sprint_t = to;
        sprint_f = from;
        $("#sprint_duedate_customrange").val(x);
        $("#chk_dated").val(x);
        $('#sprint_duedate_customrange').prop("checked", "checked");
        $("#chk_dated").val(1);
        filterSprint('quk_fil_sprint');
    }
}

function openDueDate(x) {
    $('#sprint_duedate_customrange').change(function () {
        if (!this.checked) {
            localStorage.removeItem("SPRINT_DUE_DATE");
            $('#custom_dat_op').hide();
            filterSprint('quk_fil_sprint');
...
function sprintDuedate() {
    $("#sprint_duefrm").datepicker({
        format: "M d, yyyy",
        todayHighlight: true,
        changeMonth: false,
        changeYear: false,
        ...
        var dateText = $("#sprint_duefrm").datepicker("getFormattedDate");
        $("#sprint_dueto").datepicker("setStartDate", dateText);
    });
    $("#sprint_dueto").datepicker({
        format: "M d, yyyy",
        todayHighlight: true,
        changeMonth: false,
        changeYear: false,
        ...
        var dateText = $("#sprint_dueto").datepicker("getFormattedDate");
        $("#sprint_duefrm").datepicker("setEndDate", dateText);
    });
...
function sprintchkserch() {
    $('#sprint_duedate_customrange').prop("checked", "checked");
}

function sprintStartDate(x) {
    localStorage.removeItem("SPRINT_START_DATE");
    $("#chk_datedd").val(0);
    if (x && !$("#sprint_start_" + x).is(":checked")) {
        sprintStartDate("");
        return false;
    }
    $(".cbox_datedd").removeAttr("checked");
    if (x) {
        $("#sprint_start_" + x).prop("checked", "checked");
        filterSprint('quk_fil_sprint');
    } else {
        filterSprint('quk_fil_sprint');
    }
...
$(function () {
    setTimeout(function () {
        if ((localStorage.getItem("SPRINT_START_DATE") != undefined)) {
            $("#start_filter_sprint").html(localStorage.getItem("SPRINT_START_DATE"));
            $('#start_filter_sprint').css('color', "#2d6dc4");
        }
    }, 10000);
});

function openStartDate(x) {
    $('#sprint_start_customrange').change(function () {
        if (!this.checked) {
            localStorage.removeItem("SPRINT_START_DATE");
            $('#custom_dat_st').hide();
            filterSprint('quk_fil_sprint');
            ...
            $(".cbox_datedd").removeAttr("checked");
            $('#sprint_start_customrange').prop("checked", "checked");
            $('#custom_dat_st').show();
        }
    });
}

function sprintStartdatePicer() {
    $("#sprint_startfrm").datepicker({
        format: "M d, yyyy",
        todayHighlight: true,
        changeMonth: false,
        changeYear: false,
        ...
        var dateText = $("#sprint_startfrm").datepicker("getFormattedDate");
        $("#sprint_startto").datepicker("setStartDate", dateText);
    });
    $("#sprint_startto").datepicker({
        format: "M d, yyyy",
        todayHighlight: true,
        changeMonth: false,
        changeYear: false,
        ...
        var dateText = $("#sprint_startto").datepicker("getFormattedDate");
        $("#sprint_startfrm").datepicker("setEndDate", dateText);
    });
    $("#ui-datepicker-div").click(function (e) {
        e.stopPropagation();
    });
...
function sprintSearchStartdate(x) {
    var start_date = document.getElementById("sprint_startfrm");
    var end_date = document.getElementById("sprint_startto");
    var errMsg;
    var done = 1;
    if (start_date.value.trim() == "") {
        errMsg = _("From Date cannot be left blank!");
    ...
    if (done == 0) {
        ...
        var from = $("#sprint_startfrm").val();
        var to = $("#sprint_startto").val();
        document.getElementById("sprint_start_one").checked = false;
        document.getElementById("sprint_start_24").checked = false;
        document.getElementById("sprint_start_week").checked = false;
        document.getElementById("sprint_start_month").checked = false;
        document.getElementById("sprint_start_year").checked = false;
        var x = "sprint_start_" + from + "_" + to;
        var ssd = from + " - " + to;
        localStorage.setItem("SPRINT_START_DATE", ssd);
        $("#start_filter_sprint").html(localStorage.getItem("SPRINT_START_DATE"));
        $('#start_filter_sprint').css('color', "#2d6dc4");
        sprint_st = to;
        sprint_sf = from;
        $("#sprint_start_customrange").val(x);
        $("#chk_datedd").val(x);
        $('#sprint_start_customrange').prop("checked", "checked");
        $("#chk_datedd").val(1);
        filterSprint('quk_fil_sprint');
...
function strtchkserch() {
    $('#sprint_start_customrange').prop("checked", "checked");
}

function sprint_date() {
    $("#frm_dd").datepicker({
        format: "M d, yyyy",
        todayHighlight: true,
        changeMonth: false,
        ...
        var dateText = $("#frm_dd").datepicker("getFormattedDate");
        $("#to_dd").datepicker("setStartDate", dateText);
    });
    $("#to_dd").datepicker({
        format: "M d, yyyy",
        todayHighlight: true,
        ...
        var dateText = $("#to_dd").datepicker("getFormattedDate");
        $("#frm_dd").datepicker("setEndDate", dateText);
    });
    $("#ui-datepicker-div").click(function (e) {
        e.stopPropagation();
    });
...
function open_renge(id) {
    $('#quk_time_customrange').change(function () {
        if (!this.checked) {
            localStorage.removeItem("SPRINT_UPDATE_DATE");
            $('#c_ran').hide();
            filterSprint('quk_fil_sprint');
        } else {
            $(".cbox_datess").removeAttr("checked");
            $('#quk_time_customrange').prop("checked", "checked");
            $('#c_ran').show();
        }
    });
}
...
function chkdatee(x) {
    localStorage.removeItem("SPRINT_UPDATE_DATE");
    $("#chk_dates").val(0);
    if (x && !$("#quk_time_" + x).is(":checked")) {
        chkdatee("");
        return false;
    }
    $(".cbox_datess").removeAttr("checked");
    if (x) {
        $("#quk_time_" + x).prop("checked", "checked");
        filterSprint('quk_fil_sprint');
    } else {
        filterSprint('quk_fil_sprint');
    }
...
$(function () {
    setTimeout(function () {
        if ((localStorage.getItem("SPRINT_UPDATE_DATE") != undefined)) {
            $("#time_filter_sprint").html(localStorage.getItem("SPRINT_UPDATE_DATE"));
        }
    }, 10000);
});
t = "";
...
function checksprintDate(id) {
    var start_date = document.getElementById("frm_dd");
    var end_date = document.getElementById("to_dd");
    var errMsg;
    var done = 1;
    if (start_date.value.trim() == "") {
        errMsg = _("From Date cannot be left blank!");
    ...
    if (done == 0) {
        ...
        var from = $("#frm_dd").val();
        var to = $("#to_dd").val();
        document.getElementById("quk_time_one").checked = false;
        document.getElementById("quk_time_24").checked = false;
        document.getElementById("quk_time_week").checked = false;
        document.getElementById("quk_time_month").checked = false;
        document.getElementById("quk_time_year").checked = false;
        var x = from + "_" + to;
        var sud = from + " - " + to;
        localStorage.setItem("SPRINT_UPDATE_DATE", sud);
        $("#time_filter_sprint").html(localStorage.getItem("SPRINT_UPDATE_DATE"));
        t = to;
        f = from;
        $("#quk_time_customrange").val(x);
        $("#chk_dates").val(x);
        $('#quk_time_customrange').prop("checked", "checked");
        $("#chk_dates").val(1);
        filterSprint('quk_fil_sprint');
...
function filterSprint(id) {
    if (localStorage.getItem("SPRINT_DUE_DATE") != undefined) {
        $("#due_filter_sprint").html(localStorage.getItem("SPRINT_DUE_DATE"));
        $('#due_filter_sprint').css('color', "#2d6dc4");
    }
    if (localStorage.getItem("SPRINT_START_DATE") != undefined) {
        $("#start_filter_sprint").html(localStorage.getItem("SPRINT_START_DATE"));
        $('#start_filter_sprint').css('color', "#2d6dc4");
    ...
    if (localStorage.getItem("SPRINT_UPDATE_DATE") != undefined) {
        $("#time_filter_sprint").html(localStorage.getItem("SPRINT_UPDATE_DATE"));
        $('#time_filter_sprint').css('color', "#2d6dc4");
    }
    if ($("#chk_dates").val() != 0) {
        setTimeout(function () {
            chkserch();
    ...
    if ($("#chk_dated").val() != 0) {
        setTimeout(function () {
            sprintchkserch();
            $('#custom_dat_op').show();
            $("#sprint_duefrm").val(sprint_f);
            $("#sprint_dueto").val(sprint_t);
        }, 3000);
    }
    if ($("#chk_datedd").val() != 0) {
        setTimeout(function () {
            strtchkserch();
            $('#custom_dat_st').show();
            $("#sprint_startfrm").val(sprint_sf);
            $("#sprint_startto").val(sprint_st);
        }, 3000);
    }
    var checkedArr = [];
    $("." + id).each(function () {
    ...
    setGetFilterSprint("sprint", checkedArr, 0);
    if (typeof arguments[1] != "undefined") {
        return checkedArr;
    } else {
        ajaxSprintView(checkedArr);
    }
}

function filterBacklogStatus(id) {
    const checkedValues = $(`.${id}:checked`).map((i, checkbox) => checkbox.value).get();
    setGetFilterSprint("backlogstatus", checkedValues, 0);
    if (arguments.length > 1) {
        return checkedValues;
    } else {
        ajaxBacklogBystatusView(checkedValues);
    }
}
function filterSprintByID(id) {
    $('#projIsChange').val($('#projFil').val());
    var checkedVar = '';
    checkedVar = id;
    setGetFilterSprint('sprint_by_id', checkedVar, 0);
    ajaxSprintView(filterSprint('quk_fil_sprint', 1));
}
var lastCall = 0;
var lastCallBacklogStatus = 0;

let ajaxBacklogBystatusView = (inpt, type) => {
    lastCallBacklogStatus = 1;
    $('#startTourBtn').hide();
    $('#caseViewSpan').html('');
    $('#task_paginate').html('');
    var filters = '';
    ...
    var sprint_by_id = setGetFilterSprint('backlogBystatus', '', 1);
    if (!$('.menu-backlog').length || !$('.menu-sprint').length) {
        window.location.href = HTTP_ROOT + "dashboard";
        return false;
    }
    ...
    $('#caseViewSpan').hide();
    $('#caseLoader').show();
    $(".kban_all_task").css('overflow-y', 'hidden');
    var caseDate = $('#caseDate').val();
    $("#case_search").val("");
    var caseMenuFilters = $('#caseMenuFilters').val();
    var case_srch = $('#case_srch').val();
    ...
    if (projFil != projIsChange) {
        sprint_by_id = setGetFilterSprint('backlogBystatus', '', 0);
    }
    var caseAssignTo = $('#caseAssignTo').val();
    var caseSearch = $.trim($("#inner-search-backlogstatus").val());
    var fileUrl = strURL + "backlogByStatus";
    if (!filters.length) {
        try {
            const storedFilters = localStorage.getItem('backlogstatus');
            filters = storedFilters ? JSON.parse(storedFilters) : [];
        } catch (e) {
            console.error("Error parsing localStorage backlogstatus:", e);
            filters = [];
        }
    }
    $.post(fileUrl, {
        ...
        if (res.status == 'success') {
            res.appliedTeamFilters = localStorage.getItem('backlogstatus');
            if (!(localStorage.getItem("theme_setting") === null)) {
                var th_set_obj = JSON.parse(localStorage.getItem("theme_setting"));
                $(".side-nav li").each(function () {
                    $(this).removeClass(th_class_str);
            ...
            $(".kban_all_task").css('overflow-y', 'auto');
            $("#kanban_list").show();
            var result = document.getElementById('kanban_list');
            if (res.sprint_title == '') {
                result.innerHTML = tmpl("active_sprint_empty_tmpl", res);
            } else {
                if (morecontent) {
                    ...
                    $('#totaln').val(res.totaln);
                    $('#totali').val(res.totali);
                    $('#totalc').val(res.totalc);
                    $('#' + morecontent).find('.kanban_content .kbtask_div:last').after(tmpl("backlog_status_tmpl", res));
                } else {
                    result.innerHTML = tmpl("backlog_status_tmpl", res);
                }
            }
            window.scrollTo(0, 0);
            lastCall = 0;
            if ($.trim(caseSearch) != '') {
                ...
                $('.filterSprinticon_srch').hide();
                // Restore focus to search field after results load
                setTimeout(function() {
                    var searchField = $('#inner-search-backlogstatus');
                    searchField.focus();
                    // Move cursor to end of text
                    var length = searchField.val().length;
                    searchField[0].setSelectionRange(length, length);
            ...
            if (res.sprint_title == '') {
                $('#sprint_title_hd').html(_('Active Sprint'));
            } else {
                $('#sprint_title_hd').html(res.sprint_title);
                $('#sprint_hr_cnt').html(res.daysRemain);
                $('#sprint_st_date').html(res.sprint_st_date);
                $('#sprint_ed_date').html(res.sprint_ed_date);
                $('#sprint_title_tool').attr('title', res.sprint_title);
            }
            $('[rel="tooltip_down_sth"]').tipsy({
                gravity: 'n',
                fade: true
            });
            $.material.init();
            ...
            if (!morecontent) {
                $('.kanban_content').scroll(function (event) {
                    var loadmorecontent = $(this).parent().parent().attr('id');
                    if (Math.round($(this).scrollTop()) + Math.round($(this).innerHeight()) >= (parseInt($(this)[0].scrollHeight) - 1)) {
                        ...
                        if (lastCall == 0) {
                            globalkanbantimeout = setTimeout("ajaxBacklogBystatusView(filterSprint('quk_fil_sprint',1),'" + loadmorecontent + "')", 300);
                        }
                    }
                });
            }
                    ...
                    stop: function (event, ui) {
                        ...
                        if (new_sts == 0) {
                            ...
                            if (projFil.toLowerCase() != 'all') {
                                ...
                                $.post(HTTP_ROOT + 'boards/ajaxChangeSequence', {
                                    'caseId': '0',
                                    'taskuids': task_ids,
                                    ...
                                    if (res.success == 'fail') {
                                        ...
                                        ajaxBacklogBystatusView(filterSprint('quk_fil_sprint', 1));
                                    } else {
                                        ajaxBacklogBystatusView(filterSprint('quk_fil_sprint', 1));
                                    }
                                }, 'json');
                            ...
                            $.post(HTTP_ROOT + 'boards/ajaxChangeLegend', {
                                'caseId': '0',
                                ...
                                if (res.success == 'fail') {
                                    ...
                                    $('#cnter_' + ext_sts_id).text(parseInt($('#cnter_' + ext_sts_id).text()) - 1);
                                    $('#cnter_' + curr_sts_id).text(parseInt($('#cnter_' + curr_sts_id).text()) + 1);
                                    if (new_sts == 3) {
                                        $(ui.item).find('h6[id^="titlehtml"]').addClass("line_through");
                                    } else {
                                        $(ui.item).find('h6[id^="titlehtml"]').removeClass("line_through");
                                    }
                                    showTopErrSucc('success', _('Task moved successfully.'));
                                    $.post(HTTP_ROOT + "easycases/ajaxemail", {
                                        'json_data': res.ajax_email.data,
                                        'type': 1,
                                        'draggable_status': 'draggable_status'
                                    });
                                    if ((typeof res.haschield != 'undefined' && res.haschield.length > 0) || res.isCustom == 1 || res.isCustom == 0) {
                                        ajaxBacklogBystatusView(filterSprint('quk_fil_sprint', 1));
                                    }
                                }
                            }, 'json');
            ...
            $.material.init();
            if ($.trim($('#inner-search-backlogstatus').val()) != '') {
                $('#inner-search-backlogstatus').addClass('open').css('width', '200px');
            }
            $('.slide_rht_con').css('margin-top', '-65px');
            if (res.BkFilters) {
                var act_filt_txt_quk = '';
                var act_filt_txt_assn = '';
                $(".quk_fil_teams").each(function (i) {
                    if ($.inArray($(this).val(), res.BkFilters) != -1) {
                        $(this).prop("checked", true);
                        if ($(this).next("label").attr("for") != "teams_") {
                            act_filt_txt_assn = act_filt_txt_assn == "" ? $(this).next("label").text() : act_filt_txt_assn + " , " + $(this).next("label").text();
                        } else {
                            act_filt_txt_quk = act_filt_txt_quk == "" ? $(this).next("label").text() : act_filt_txt_quk + " , " + $(this).next("label").text();
                        }
                ...
                if (act_filt_txt_assn != "") {
                    $("#assn_filter_teams").text(act_filt_txt_assn).attr("title", act_filt_txt_assn);
                    $('#assn_filter_teams').css('color', "#2d6dc4");
                } else {
                    $("#assn_filter_teams").text(_("Teams")).attr("title", _("Teams"));
                }
                //$('.asnfilter_sprint_heap').addClass('open');
            }
            var clearCaseSearch = $("#clearCaseSearch").val();
            $("#clearCaseSearch").val("");
            $("[rel=tooltip]").tipsy({
            ...
            if (res.sprint_title != '') {
                if (!morecontent) {
                    $.post(HTTP_ROOT + "Backlogs/defaultCaseStatus", {
                        "projUniq": projFil,
                        "pageload": 0,
                        "caseMenuFilters": 'kanban',
                        "caseMenuType": 'backlogByStatus',
                        'case_date': caseDate,
                        'case_due_date': '',
                        'caseStatus': '',
                        'caseTypes': '',
                        ...
                        if (data) {
                            if (localStorage.getItem("SPRINT_DUE_DATE") != undefined) {
                                $("#due_filter_sprint").html(localStorage.getItem("SPRINT_DUE_DATE")).text(localStorage.getItem("SPRINT_DUE_DATE")).attr("title", "Due Date :" + localStorage.getItem("SPRINT_DUE_DATE"));
                                $('#due_filter_sprint').css('color', "#2d6dc4");
                            }
                            if (localStorage.getItem("SPRINT_START_DATE") != undefined) {
                                $("#start_filter_sprint").html(localStorage.getItem("SPRINT_START_DATE")).text(localStorage.getItem("SPRINT_START_DATE")).attr("title", "Start Date :" + localStorage.getItem("SPRINT_START_DATE"));
                                $('#start_filter_sprint').css('color', "#2d6dc4");
                            }
                            if (localStorage.getItem("SPRINT_UPDATE_DATE") != undefined) {
                                $("#time_filter_sprint").html(localStorage.getItem("SPRINT_UPDATE_DATE")).text(localStorage.getItem("SPRINT_UPDATE_DATE")).attr("title", "Update Date :" + localStorage.getItem("SPRINT_UPDATE_DATE"));
                                $('#time_filter_sprint').css('color', "#2d6dc4");
                            }
                            $(".slide_rht_con").css("margin-top", "-65px");
                            $("#upperDiv_not").hide();
                            ...
                            var n = "";
                            $("#cnter_newTask").text(data.nw);
                            $("#cnter_inprogressTask").text(data.opn);
                            $("#cnter_resolvedTask").text(data.rslv);
                            $("#cnter_closedTask").text(data.cls);
                            $("[rel=tooltip]").tipsy({
                                gravity: "s",
                                fade: true
                            });
                ...
                $("#cnter_newTask").html(0);
                $("#cnter_inprogressTask").html(0);
                $("#cnter_resolvedTask").html(0);
                $("#cnter_closedTask").html(0);
            }
        } else {
            showTopErrSucc("error", res.msg);
            $('#caseLoader').hide();
            if (res.hasOwnProperty('teamEmpty') && res.teamEmpty == 1) {
                $('#inner-search-backlogstatus').val('');
                localStorage.removeItem('backlogstatus');
                clearSprintFilter('backlogByStatus');
                ajaxBacklogBystatusView(filterSprint('quk_fil_sprint', 1));
            }
            return false;
        }
...
function ajaxSprintView(inpt, type) {
    lastCall = 1;
    $('#startTourBtn').hide();
    $('#caseViewSpan').html('');
    $('#task_paginate').html('');
    ...
    var sprint_by_id = setGetFilterSprint('sprint_by_id', '', 1);
    if (!$('.menu-backlog').length || !$('.menu-sprint').length) {
        window.location.href = HTTP_ROOT + "dashboard";
        return false;
    }
    ...
    if (!(localStorage.getItem("theme_setting") === null)) {
        ...
        $('.menu-sprint').addClass(th_set_obj.sidebar_color + " gradient-shadow");
    }
    if (typeof (type) != 'undefined' && type != 'kanban') {
        ...
        if (morecontent == 'newTask' && (parseInt($('#totaln').val()) <= parseInt(newTask_limit))) {
            lastCall = 0;
            return;
        } else if (morecontent == 'inprogressTask' && (parseInt($('#totali').val()) <= parseInt(inProgressTask_limit))) {
            lastCall = 0;
            return;
        } else if (morecontent == 'closedTask' && (parseInt($('#totalc').val()) <= parseInt(closedTask_limit))) {
            lastCall = 0;
            return;
        }
    } else { }
    ...
    var projIsChange = $('#projIsChange').val();
    if (projFil != projIsChange) {
        sprint_by_id = setGetFilterSprint('sprint_by_id', '', 0);
    }
    var caseAssignTo = $('#caseAssignTo').val();
    var caseSearch = $.trim($("#inner-search-sprint").val());
    var fileUrl = strURL + "active_sprint";
    $.post(fileUrl, {
        "projFil": projFil,
        "projIsChange": projIsChange,
        "casePage": casePage,
        ...
        if (res.status == 'success') {
            ...
            if (res.sprint_title == '') {
                ...
                if (morecontent) {
                    ...
                    $('#totali').val(res.totali);
                    $('#totalc').val(res.totalc);
                    $('#' + morecontent).find('.kanban_content .kbtask_div:last').after(tmpl("active_sprint_tmpl", res));
                } else {
                    result.innerHTML = tmpl("active_sprint_tmpl", res);
                }
            }
            window.scrollTo(0, 0);
            lastCall = 0;
                ...
                $('.kanban_content').scroll(function (event) {
                    ...
                    if (Math.round($(this).scrollTop()) + Math.round($(this).innerHeight()) >= (parseInt($(this)[0].scrollHeight) - 1)) {
                        if (globalkanbantimeout != null)
                            clearTimeout(globalkanbantimeout);
                        if (lastCall == 0) {
                            globalkanbantimeout = setTimeout("ajaxSprintView(filterSprint('quk_fil_sprint',1),'" + loadmorecontent + "')", 300);
                        }
                    }
                });
            }
                    ...
                    stop: function (event, ui) {
                        ...
                        if (new_sts == 0) {
                            ...
                            if (projFil.toLowerCase() != 'all') {
                                ...
                                $.post(HTTP_ROOT + 'boards/ajax_change_sequence', {
                                    'caseId': '0',
                                    'taskuids': task_ids,
                                    'taskuids_All': task_ids_all,
                                    'proj_id': cur_proj_id,
            ...
            if ($.trim($('#inner-search-sprint').val()) != '') {
                $('#inner-search-sprint').addClass('open').css('width', '200px');
            }
            $('.slide_rht_con').css('margin-top', '-65px');
            if (res.BkFilters) {
                var act_filt_txt_quk = '';
                ...
                var act_filt_txt_type = '';
                var act_filt_txt_status = '';
                var act_filt_txt_time = "";
                var act_filt_txt_due = "";
                var act_filt_txt_strt = "";
                $(".quk_fil_sprint").each(function (i) {
                    if ($.inArray($(this).val(), res.BkFilters) != -1) {
                        $(this).prop("checked", true);
                        if ($(this).next("label").attr("for") != "quk_last_upd" && $(this).next("label").attr("for") != "quk_assn") {
                            if ($(this).next("label").attr("for").indexOf("quk_type_") !== -1) {
                                ...
                                act_filt_txt_status = act_filt_txt_status == "" ? $(this).next("label").text() : act_filt_txt_status + " , " + $(this).next("label").text();
                            } else if ($(this).next("label").attr("for").indexOf("quk_time_") !== -1) {
                                act_filt_txt_time = act_filt_txt_time == "" ? $(this).next("label").text() : act_filt_txt_time;
                            } else if ($(this).next("label").attr("for").indexOf("sprint_duedate_") !== -1) {
                                act_filt_txt_due = act_filt_txt_due == "" ? $(this).next("label").text() : act_filt_txt_due;
                            } else if ($(this).next("label").attr("for").indexOf("sprint_start_") !== -1) {
                                act_filt_txt_strt = act_filt_txt_strt == "" ? $(this).next("label").text() : act_filt_txt_strt;
                            } else {
                                act_filt_txt_assn = act_filt_txt_assn == "" ? $(this).next("label").text() : act_filt_txt_assn + " , " + $(this).next("label").text();
                            }
                        } else {
                ...
                if (act_filt_txt_quk != "") {
                    $("#quk_filter_sprint").text(act_filt_txt_quk).attr("title", act_filt_txt_quk);
                    $('#quk_filter_sprint').css('color', "#2d6dc4");
                } else {
                    $("#quk_filter_sprint").text(_("Quick Filter")).attr("title", _("Quick Filter"));
                }
                if (act_filt_txt_assn != "") {
                    $("#assn_filter_sprint").text(act_filt_txt_assn).attr("title", act_filt_txt_assn);
                    $('#assn_filter_sprint').css('color', "#2d6dc4");
                } else {
                    $("#assn_filter_sprint").text(_("Assignee")).attr("title", _("Assignee"));
                }
                if (act_filt_txt_type != "") {
                    $("#type_filter_sprint").text(act_filt_txt_type).attr("title", act_filt_txt_type);
                    $('#type_filter_sprint').css('color', "#2d6dc4");
                } else {
                    $("#type_filter_sprint").text(_("Task Types")).attr("title", _("Task Types"));
                }
                if (act_filt_txt_status != "") {
                    $("#status_filter_sprint").text(act_filt_txt_status).attr("title", act_filt_txt_status);
                    $('#status_filter_sprint').css('color', "#2d6dc4");
                } else {
                    $("#status_filter_sprint").text(_("Status")).attr("title", _("Status"));
                }
                if (act_filt_txt_time != "") {
                    $("#time_filter_sprint").text(act_filt_txt_time).attr("title", "Last Update:" + act_filt_txt_time);
                    $('#time_filter_sprint').css('color', "#2d6dc4");
                } else {
                    $("#time_filter_sprint").text(_("Last Update")).attr("title", _("Last Update"));
                }
                if (act_filt_txt_due != "") {
                    $("#due_filter_sprint").text(act_filt_txt_due).attr("title", "Due Date :" + act_filt_txt_due);
                    $('#due_filter_sprint').css('color', "#2d6dc4");
                } else {
                    $("#due_filter_sprint").text(_("Due Date")).attr("title", _("Due Date"));
                }
                if (act_filt_txt_strt != "") {
                    $("#start_filter_sprint").text(act_filt_txt_strt).attr("title", "Start Date :" + act_filt_txt_strt);
                    $('#start_filter_sprint').css('color', "#2d6dc4");
                } else {
                    $("#start_filter_sprint").text(_("Start Date")).attr("title", _("Start Date"));
                }
            }
            if (typeof res.BkFiltersName != "undefined") {
                $("#change_filter_sprint").text(res.BkFiltersName).attr("title", res.BkFiltersName);
                $('#change_filter_sprint').css('color', "#2d6dc4");
            } else {
                $("#change_filter_sprint").text(_("Sprint")).attr("title", _("Sprint"));
            }
            var clearCaseSearch = $("#clearCaseSearch").val();
            $("#clearCaseSearch").val("");
            $("[rel=tooltip]").tipsy({
...
function casePaging(page) {
    ...
    } else if (parseUrlHash(urlHash) == 'backlog') {
        localStorage.setItem('BACKLOG_PAGE', page);
...
// Show/hide the "Notify via Email" recipients list in the comment box. Hidden
// by default; the heading acts as the toggle. Kept as a named function (not an
// inline handler) because the comment box is rendered through the tmpl() micro
// template, which mangles inline JS containing quotes.
function toggleNotifyEmail(el) {
    var $h = $(el);
    var isOpen = $h.toggleClass('open').hasClass('open');
    $h.next('.notify-email-body').stop(true, true)[isOpen ? 'slideDown' : 'slideUp'](150);
    $h.find('.notify-email-caret').css('transform', isOpen ? 'rotate(180deg)' : '');
}
...
easycase.imageTypeIcon = function (format) {
    var iconsArr = ["gd", "db", "zip", "xls", "doc", "jpg", "png", "bmp", "pdf", "tif", "txt", "psd", "video", "ppt", "sql", "csv", "od"];
    if (format == undefined) {
        return;
    }
...
easycase.getColorPriority = function (casePriority) {
    if (casePriority == "NULL" || casePriority == "") {
        ...
        return '<b style="color:#A40E26;">' + _('Urgent Priority') + '</b>';
    }
}
easycase.refreshTaskList = function (dtlsid, details) {
    /*
     * Rebuilds the legacy list markup. On the new task pages that would paint
     * the old list over the Vue app, so reload the Vue list instead. Guarded
     * here rather than at the ~100 call sites.
     */
    ...
    if (hashPage == 'timelog' || ['tasks', 'kanban', 'taskgroup', 'subtask', 'calendar', 'searchFilters', 'milestonelist', 'taskgroups', 'epics', 'features'].includes(params[0]) || filterV == 'calendar') {
        if (filterV == 'kanbantask' && (params[0] == 'kanban' || params[0] == 'calendar')) {
            ...
            if (params[0] == 'tasks') {
                ajaxCaseView();
            } else if (params[0] == 'epics') {
                ajaxCaseView('epics');
            } else if (params[0] == 'features') {
                ajaxCaseView('features');
...
easycase.ajaxCaseDetails = function (caseUniqId, type, dtls) {
    ...
    if (PAGE_NAME == "resourceAvailability") {
        $('#myModal').modal('hide');
        $(".cmn_popup").hide();
    }
    var isPopup = 1;
    ...
    var strURL = HTTP_ROOT + "easycases/caseDetails";
    $('#caseLoaderPopupKB').show();
    $('#lastTotReplies-lst').val(totalReplies);
    var projFil = $('#projFil').val();
    $.post(strURL, {
        "projdrp_uniq_id": projFil,
        "caseUniqId": caseUniqId,
        "details": dtls
    }, function (data) {
        if (data) {
            localStorage.setItem('is_splitted', data.is_splitted);
            ...
            if (!is_next && !is_prev && !is_reload && !is_action) {
                ...
                var _currentHash = (_currentHref.split('#/')[1] || '').split('/')[0];
                if (_currentHash !== 'details') {
                    localStorage.setItem("last_url", _currentHref);
                }
            }
            // Keep the detail URL on whatever page opened it. Hard-coding
            // 'dashboard' meant opening a task from the Vue task pages
            // rewrote the address bar to the legacy list, so a refresh — or
            // closing the panel — landed you there instead of where you were.
            var _detailBase = (typeof CONTROLLER !== 'undefined' && CONTROLLER === 'taskviews')
                ? window.location.pathname
                : HTTP_ROOT + 'dashboard';
            var newUrl = _detailBase + '#/details/' + caseUniqId;
            ...
            $(`#checklist_body${caseUniqId}`).sortable({
                ...
                stop: function (event, ui) {
                    $.ajax({
                        url: `${HTTP_ROOT}check-lists/ajaxReorderchecklist`,
                        type: 'POST',
                        dataType: 'json',
            ...
            $('.caleder-due-date').off().on('click', function (e) {
                var is_splitted = localStorage.getItem('is_splitted');
                if (is_splitted == 0) {
            ...
            $('.add-task-wiki', '.remove-task-wiki').hide();
            if (typeof showCaseWikiTaskTree !== 'undefined') {
                showCaseWikiTaskTree(caseUniqId, projFil);
            }
        } else {
...
easycase.loadTinyMce = function (csAtId) {
    ...
    tinymce.init({
        ...
        plugins: 'image paste importcss autolink directionality fullscreen link   table charmap hr pagebreak nonbreaking anchor advlist lists wordcount autoresize help mention emoticons',
        menubar: false,
        branding: false,
        statusbar: false,
        toolbar: 'undo redo | bold italic underline strikethrough | outdent indent |  numlist bullist | forecolor backcolor | fullscreen | emoticons',
        toolbar_sticky: true,
...
easycase.showbacklog = function (task_data) {
    easycase.routerHideShow('backlog');
    $('#caseLoader').show();
    var spFil = setGetFilterSprint('backlog', '', 1);
    if (spFil != '') {
        ajaxBacklogView(spFil);
    } else {
        ajaxBacklogView(0);
    }
}
easycase.showbacklogBystatus = function () {
    $('#caseLoader').show();
    var spFil = setGetFilterSprint('backlogBystatus', '', 1);
    if (spFil != '') {
        ajaxBacklogBystatusView(spFil);
    } else {
        ajaxBacklogBystatusView(0);
    }
}
easycase.showsprint = function (task_data) {
    $('#caseLoader').show();
    var spFil = setGetFilterSprint('sprint', '', 1);
    if (spFil != '') {
        ajaxSprintView(spFil);
    } else {
        ajaxSprintView(0);
    }
}
...
easycase.showTaskLists = function (page) {
    if (page === 'epics' && !isAllowed('View All Epic', $('#projFil').val())) {
        location.hash = '/tasks';
        return;
    }
    if (page === 'features' && !isAllowed('View All Feature', $('#projFil').val())) {
        location.hash = '/tasks';
        return;
    }
    if (refreshTasks == 1) {
    ...
    $(".side-nav li").removeClass('active');
    $(".menu-cases" + (page === 'epics' || page === 'features' ? '-' + page : '')).addClass('active');
    if (!(localStorage.getItem("theme_setting") === null)) {
        var th_set_obj = JSON.parse(localStorage.getItem("theme_setting"));
        $(".side-nav li").each(function () {
            $(this).removeClass(th_class_str);
    ...
    displayMenuProjects('dashboard', '8', '');
}
        ...
        $http.post(URL, data, config).success(function (data, status, headers, config) {
            if (type == "more") {
                ...
                if (data.activity.length > 0) {
                    $("#PieChart").show();
                    $http.post(HTTP_ROOT + 'users/activityPichart', $.param({
                        'pjid': projid
...
caseapp.controller("overdue_Controller", function ($scope, $http) {
    $scope.loadOverdue = function (type) {
        ...
        $http.post(HTTP_ROOT + "users/ajaxOverdue/", data, config).success(function (data, status, headers, config) {
...
caseapp.controller('EditableRowCtrl', function ($scope, $filter, $http) {
    ...
    $scope.showeditTimeRange = function (i, log) {
        ...
        tinymce.init({
            ...
            plugins: 'image paste importcss autolink directionality fullscreen link  table charmap hr pagebreak nonbreaking anchor advlist lists wordcount autoresize help',
            menubar: false,
            branding: false,
            statusbar: false,
...
caseapp.controller('timelogController', function ($scope, $http) {
    ...
    $scope.noDataImage = `${HTTP_ROOT}img/no-data/notimelog.png`;
    ...
    $scope.fetchTimelog = function (column, page) {
        ...
        $http.post(HTTP_ROOT + "Requests/timeLog", params_data, config).success(function (data, status, headers, config) {
            $('#caseLoader').hide();
            $scope.records = data;
            ...
            if (localStorage.getItem("tour_type") == '4') {
                ...
                var savedStepNum = localStorage.getItem("tour_step_num");
                var startStep;
                if (savedStepNum && parseInt(savedStepNum) >= 0) {
                    startStep = parseInt(savedStepNum);
                   
                } else {
                  
                    startStep = (localStorage.getItem("tour_type_nxt") == '0') ? 0 : 2;
                }
                ...
                var startTourWhenReady = function() {
                    attemptCount++;
                    
                   
                    if (startStep === 0) {
                        if ($('#timer_open').length > 0) {
                           
                            hopscotch.startTour(GBl_tour, startStep);
                            localStorage.removeItem("tour_step_num");
                        ...
                        if ($('.tour_tlog_listing').length > 0 || $('#tour_tlog_listing').length > 0) {
                ...
                setTimeout(startTourWhenReady, 500);
            }
            $('.r-u-link').hide();
            if (SES_TYPE < 3) {
                $('.r-u-link').show();
    ...
    return function (input, options) {
        // Parse with the explicit server format (date('M d Y H:i:s')) so moment
        // does not fall back to the JS Date parser and log a deprecation warning.
        return moment(input, 'MMM DD YYYY HH:mm:ss').locale('eng').format(options.split('\'')[1])
    }
});
...
caseapp.filter("splitTask", function () {
    return function (input, options) {
        ...
        const task_dtl = input.split('||');
        return task_dtl[options] !== undefined ? task_dtl[options] : null;
    };
});
...
caseapp.controller("upcoming_Controller", function ($scope, $http) {
    $scope.loadUpcoming = function (type) {
        ...
        $http.post(HTTP_ROOT + "users/ajaxUpcoming/", data, config).success(function (data, status, headers, config) {
...
function loadActivity(type) {
    ...
    var strURL = HTTP_ROOT + "users/ajaxActivity/";
...
function loadMentionList(type) {
    ...
    var strURL = HTTP_ROOT + "users/ajaxMentionedList";
            ...
            if (caseid.length == 0) {
                ...
                $.post(HTTP_ROOT + "requests/ajaXTaskMassAction", {
                    ...
                    if (res.status == 'success') {
                        ...
                        showTopErrSucc('error', _('Oops! Something went wrong'));
                    }
                }, 'json');
            }
        }
...
function startCase(id, no, dtlsid) {
    ...
    if ((hashtag[0] == 'kanban') || (hashtag[0] == 'activesprint') || (hashtag[0] == 'backlogByStatus') || (hashtag[0] == 'milestonelist') || (($('#caseMenuFilters').val() != 'cases') || (hashtag[0] == 'details')) || (getCookie('TASKGROUPBY') == 'milestone') || isPopup == 1 || (hashtag[0] == 'taskgroups')) {
        if (isSub) {
...
function setNewCase(id, no, dtlsid) {
    ...
    if ((hashtag[0] == 'kanban') || (hashtag[0] == 'activesprint') || (hashtag[0] == 'milestonelist') || (($('#caseMenuFilters').val() != 'cases') || (hashtag[0] == 'details')) || hashtag[0] == 'details' || (getCookie('TASKGROUPBY') == 'milestone') || isPopup == 1 || (hashtag[0] == 'taskgroups')) {
        if (isSub) {
...
function setCloseCase(id, no, dtlsid) {
    ...
    if ((hashtag[0] == 'kanban') || (hashtag[0] == 'activesprint') || (hashtag[0] == 'backlogByStatus') || (hashtag[0] == 'milestonelist') || (($('#caseMenuFilters').val() != 'cases') || (hashtag[0] == 'details')) || hashtag[0] == 'details' || (getCookie('TASKGROUPBY') == 'milestone') || isPopup == 1 || (hashtag[0] == 'taskgroups')) {
        if (isSub) {
...
function setCustomStatus(id, no, uid, statusid, masterid, actiontype) {
    var strurl = HTTP_ROOT + "easycases/checkParentBeforeStatus";
        ...
        if (reconf == 1 || confirm(reconf)) {
            $.post(HTTP_ROOT + "easycases/changeCustomStatus", {
                ...
                if (res.succ) {
                    ...
                    if ((hashtag[0] == 'tasks' || hashtag[0] == 'taskgroup') && (parseInt($("#subtask-container").height()) < 70) && !is_popup) {
                    ...
                    } else if (hashtag[0] == 'details' || hashtag[0] == 'timesheet_weekly' || hashtag[0] == 'timesheet' || hashtag[0] == 'kanban' || hashtag[0] == 'milestonelist' || hashtag[0] == 'backlog' || hashtag[0] == 'activesprint' || (parseInt($("#subtask-container").height()) > 70) || (is_popup && (hashtag[0] == 'tasks' || hashtag[0] == 'taskgroup' || hashtag[0] == 'taskgroups'))) {
                        if (hashtag[0] == 'details') {
                            refreshTasks == 1;
                        }
                        if (is_sub == 'sub' || is_sub == 'link') {
                            showTopErrSucc('success', _("Status changed successfully") + ".");
                            if (is_sub == 'sub' || res.parent_id != '') {
                        ...
                        } else if (hashtag[0] == 'details' || hashtag[0] == 'timesheet_weekly' || hashtag[0] == 'timesheet' || hashtag[0] == 'kanban' || hashtag[0] == 'backlog' || hashtag[0] == 'activesprint' || is_popup) {
                    ...
                    if (hashtag[0] == 'tasks') {
                        ...
                        refreshTasks == 1;
                    } else if (hashtag[0] == 'details') { } else if (hashtag[0] == 'timesheet_weekly') { } else if (hashtag[0] == 'activesprint') {
                        easycase.showsprint();
                    } else if (hashtag[0] == 'backlog') { } else if ($('#caseMenuFilters').val() == 'milestonelist') {
                        showMilestoneList();
...
function copyChecklist(count, obj) {
    var project_id = $(obj).attr('data-prjid');
    var case_id = $(obj).attr('data-caseid');
    var case_no = $(obj).attr('data-caseno');
    $.post(HTTP_ROOT + "check-lists/ajaxCopyChecklists", {
        'project_id': project_id,
        'case_id': case_id
    }, function (data) {
        if (data) {
            if (data == 'error') {
                showTopErrSucc('error', _("There are no checklists present inside the choosen task."));
                closePopup();
                return false;
            } else {
                openPopup();
                $(".copy_check_list").show();
                $(".loader_dv").hide();
                $('#inner_copy_checklist').show();
                $('#inner_copy_checklist').html(data);
                $.material.init();
                $(".select").select2();
            }
        }
...
function cpyCheckList(obj) {
    var project_id = $(obj).attr('data-project-id');
    var old_case_id = $(obj).attr('data-old-caseid');
    var case_id = $('#select_task_to_checklist').val();
    if (case_id == null) {
        showTopErrSucc('error', _("Please select a task to copy checklist items"));
        closePopup();
        return false;
    }
    $.post(HTTP_ROOT + "check-lists/cpyCheckListToOtherTask", {
        'project_id': project_id,
        'old_case_id': old_case_id,
        'case_id': case_id
    }, function (data) {
        if (data) {
            showTopErrSucc('success', _("Checklists copied successfully."));
            $('#cchecklist').prop('disabled', true);
            closePopup();
            return true;
        }
    });
...
function resourceAssignProject(userId) {
    ...
    $.post(HTTP_ROOT + "users/ajaxGetResources", {
        "user_id": user_id,
...
function archiveCase(id, cno, pid, dtlsid) {
    ...
    if (id == 'all') {
        ...
        if (chked == 0) {
            if (CONTROLLER == "epics" && PAGE_NAME == "index") {
                showTopErrSucc('error', _("Please check atleast one epic to archive"));
            } else if (CONTROLLER == "epics" && PAGE_NAME == "features") {
                showTopErrSucc('error', _("Please check atleast one feature to archive"));
            } else {
    ...
    if (id == 'all') {
        if (al_id_disa.length != al_id_noa) {
            if (CONTROLLER == "epics" && PAGE_NAME == "index") {
                var conf = confirm(_("All the selected epic(s) and its subtasks including closed epics(s) will be archived.") + "\n" + _("Are you sure you want to proceed?"));
            } else if (CONTROLLER == "epics" && PAGE_NAME == "features") {
                var conf = confirm(_("All the selected features(s) and its sub-features including closed features(s) will be archived.") + "\n" + _("Are you sure you want to proceed?"));
            } else {
            ...
            if (CONTROLLER == "epics" && PAGE_NAME == "index") {
                var conf = confirm(_("All the closed epic(s) will be archived.") + "\n" + _("Are you sure you want to proceed?"));
            } else if (CONTROLLER == "epics" && PAGE_NAME == "features") {
                var conf = confirm(_("All the closed feature(s) will be archived.") + "\n" + _("Are you sure you want to proceed?"));
            } else {
        ...
        if (CONTROLLER == "epics" && PAGE_NAME == "index") {
            var conf = confirm(_("Are you sure you want to archive the epic #") + cno + " " + _("and all its child epics?"));
        } else if (CONTROLLER == "epics" && PAGE_NAME == "features") {
            var conf = confirm(_("Are you sure you want to archive the feature #") + cno + " " + _("and all its child features?"));
        } else {
    ...
    if (conf == false) {
        ...
        $.post(strurl, {
            ...
            if (data.status == 'success') {
                ...
                showTopErrSucc('success', entityType + " " + data.arch_cno + " " + _("archived successfully."));
                ...
                $.post(HTTP_ROOT + "users/projectMenu", {
                    "page": 1,
                    "limit": 6
                }, function (data) {
                    if (data) {
                ...
                if (hashtag[0] == 'milestonelist') {
                    ...
                    easycase.showKanbanTaskList();
                } else if (hashtag[0] == 'backlog') {
                    easycase.showbacklog();
                } else if (hashtag[0] == 'activesprint') {
                    easycase.showsprint();
...
function closeHierarchyPopup() {
    var $panel = $('.hierarchy_move');
    if (!$panel.data('hierarchyConfirmed')) {
        var mCurRow = $panel.data('taskCurRow');
        if (mCurRow) {
            $('#caseLoader').hide();
            $('#' + mCurRow).fadeIn(100);
        }
    }
    $panel.removeData(['taskId', 'taskCno', 'taskPid', 'taskDtlsid', 'taskIsSub',
        'taskIsDtlPop', 'taskCurRow', 'taskType', 'hierarchyConfirmed']);
    closePopup();
}

function _applyDeleteCaseResult(data, id, cno, curRow, isDtlPop, dtlsid, isSub) {
    if (data.status == 0) {
        $('#caseLoader').hide();
        $("#" + curRow).fadeIn(100);
        if (CONTROLLER == "epics" && PAGE_NAME == "index") {
            showTopErrSucc('error', _("Failed to delete epic #") + cno + ". " + _("Please try again."));
        } else if (CONTROLLER == "epics" && PAGE_NAME == "features") {
            showTopErrSucc('error', _("Failed to delete feature #") + cno + ". " + _("Please try again."));
        } else {
    ...
    easycase.refreshTaskList();
    showTopErrSucc('success', _(data.type || 'task') + " #" + cno + " " + _("is deleted."));
    if (isDtlPop) {
    ...
    if (isSub) {
        var parent_id = +(data.parent_id) || +(data.feature_id) || +(data.epic_id);
        if (parent_id != '') {
            loadSubtaskInDetail(parent_id);
    ...
    if ($('#onlyforthis').attr('data-page-url') == 'activesprint') {
        window.location.href = HTTP_ROOT + 'dashboard#/activesprint';
    }
}
...
function deleteCase(id, cno, pid, dtlsid, recurring) {
    ...
    if (CONTROLLER == "epics" && PAGE_NAME == "index") {
        confMesg = _("Are you sure you want to delete the epic ?");
    }
    if (CONTROLLER == "epics" && PAGE_NAME == "features") {
        confMesg = _("Are you sure you want to delete the feature ?");
    }
    if (confirm(confMesg)) {
        if (recurring == 1) {
        ...
        refreshActvt = 1;
        var hashtag = parseUrlHash(urlHash);
        var curRow = hashtag[0] == "taskgroups" ? "curRow_subtask_" + id : "curRow" + id;
        $('#caseLoader').show();
        // Step 1: check for hierarchy children (epics→features, features→stories, etc.) first
        $.post(HTTP_ROOT + "easycases/get_children_task", {
            "task_id": id,
            "pid": pid
        }, function (hierRes) {
            if (hierRes.success && hierRes.data && hierRes.data.has_children) {
                // Has hierarchy children — show popup with two options
                $('#caseLoader').hide();
                $('.hierarchy_move')
                    .data('taskId', id)
                    .data('taskCno', cno)
                    .data('taskPid', pid)
                    .data('taskDtlsid', dtlsid)
                    .data('taskIsSub', isSub)
                    .data('taskIsDtlPop', isDtlPop)
                    .data('taskCurRow', curRow)
                    .data('taskType', hierRes.data.task_type);
                openPopup();
                $('.hierarchy_move').show();
                // Build a clearer parent + children summary for the new
                // modal layout (introduced alongside fix 78df4265).
                // Parent label = "<Type> #<cno>", children summary handles
                // pluralisation so we don't say "1 stories" or "2 story".
                var parentTypeRaw = (hierRes.data.task_type || 'task');
                var childTypeRaw  = (hierRes.data.child_type || 'child');
                var childCount    = parseInt(hierRes.data.child_count, 10) || 0;
                function titleCase(s) {
                    return s ? s.charAt(0).toUpperCase() + s.slice(1) : s;
                }
                function pluralize(noun, n) {
                    // Naive pluralisation: append 's' unless noun already
                    // ends in 's' / 'es' / 'y'. Sufficient for the
                    // hierarchy nouns we have (task, story, feature, epic,
                    // subtask). "story" → "stories" handled explicitly.
                    if (n === 1) return noun;
                    if (/y$/i.test(noun)) return noun.replace(/y$/i, 'ies');
                    if (/(s|x|z|ch|sh)$/i.test(noun)) return noun + 'es';
                    return noun + 's';
                }
                $('#hierarchyParentLabel').text(titleCase(parentTypeRaw) + ' #' + cno);
                $('#hierarchyChildSummary').text(
                    childCount + ' ' + pluralize(childTypeRaw, childCount)
                        + ' ' + _('linked to this') + ' ' + parentTypeRaw + '.'
                );

                var hasTargets = hierRes.data.targets && hierRes.data.targets.length > 0;
                var $sel = $('#hierarchyTargetSelect').empty()
                    .append($('<option>').val('').text(_('Select a new parent…')));
                if (hasTargets) {
                    $.each(hierRes.data.targets, function (i, t) {
                        var fullTitle = '#' + t.case_no + ' ' + t.title;
                        var label = fullTitle.length > 60 ? fullTitle.substring(0, 60) + '…' : fullTitle;
                        $sel.append($('<option>').val(t.id).text(label).attr('title', fullTitle));
                    });
                    $('#hierOptionMove').show();
                    $('#hierarchyMoveSection').show();
                    // Default selection stays on "Delete this task"
                    // (already marked .selected in the template). The user
                    // explicitly picks "Move" if they want to re-parent.
                } else {
                    // No targets → no Move option at all. Only path is
                    // delete-this-task; children survive as standalone.
                    $('#hierOptionMove').hide();
                    $('#hierarchyMoveSection').hide();
                }
            } else {
                // No hierarchy children — check for plain subtasks then delete
                $("#" + curRow).fadeOut(500);
                $.post(HTTP_ROOT + "easycases/check_parent_before_delete", {
                    "parent_key": id,
                    "validate_parent": 1,
                    "pid": pid
                }, function (res_chk) {
                    ...
                    if (reconf == 1 || confirm(reconf)) {
                        $.post(HTTP_ROOT + "easycases/delete_case", {
                            "id": id,
                            "cno": cno,
                            "pid": pid
                        }, function (data) {
                            _applyDeleteCaseResult(data, id, cno, curRow, isDtlPop, dtlsid, isSub);
                        }, 'json');
                    } else {
                        $('#caseLoader').hide();
                        $("#" + curRow).fadeIn(100);
...
// Scenario 2: Delete this task together with all its children
$(document).on('click', '#hierarchyDeleteAllBtn', function () {
    var $modal = $('.hierarchy_move');
    var mId       = $modal.data('taskId');
    var mCno      = $modal.data('taskCno');
    var mPid      = $modal.data('taskPid');
    var mDtlsid   = $modal.data('taskDtlsid');
    var mIsSub    = $modal.data('taskIsSub');
    var mIsDtlPop = $modal.data('taskIsDtlPop');
    var mCurRow   = $modal.data('taskCurRow');
    $modal.data('hierarchyConfirmed', true);
    closeHierarchyPopup();
    $("#" + mCurRow).fadeOut(500);
    $('#caseLoader').show();
    // Check for plain subtasks before issuing the delete
    $.post(HTTP_ROOT + "easycases/check_parent_before_delete", {
        "parent_key": mId,
        "validate_parent": 1,
        "pid": mPid
        ...
        var reconf = 1;
        if (res_chk.status == 'error') {
            reconf = _('All subtasks of this parent task #') + mCno + ' ' + _('will be deleted, are you sure you want to continue?');
        }
        if (reconf == 1 || confirm(reconf)) {
            $.post(HTTP_ROOT + "easycases/delete_case", {
                "id": mId,
                ...
                _applyDeleteCaseResult(data, mId, mCno, mCurRow, mIsDtlPop, mDtlsid, mIsSub);
            }, 'json');
        } else {
            $('#caseLoader').hide();
            $("#" + mCurRow).fadeIn(100);
...
// Scenario 1: Move children to another parent then delete
$(document).on('click', '#hierarchyMoveConfirmBtn', function () {
    var $modal = $('.hierarchy_move');
    var mId       = $modal.data('taskId');
    var mCno      = $modal.data('taskCno');
    var mPid      = $modal.data('taskPid');
    ...
    var mIsDtlPop = $modal.data('taskIsDtlPop');
    var mCurRow   = $modal.data('taskCurRow');
    var taskType  = $modal.data('taskType');
    var newParentId = $('#hierarchyTargetSelect').val();
    if (!newParentId) {
        return;
    }
    $modal.data('hierarchyConfirmed', true);
    ...
    $("#" + mCurRow).fadeOut(500);
    $('#caseLoader').show();
    $.post(HTTP_ROOT + "easycases/move_children_and_delete", {
        "id": mId,
        "cno": mCno,
        "pid": mPid,
...
function changeEstHour(caseId, caseUniqId, cno, value) {
    ...
    var esthr = estHour;
    $.post(HTTP_ROOT + "easycases/ajax_change_est_hour", {
        "caseId": caseId,
        "estHour": estHour
    }, function (data) {
        $('#lst_uptd').html(data.last_updated);
...
function changeStoryPoint(caseId, caseUniqId, cno, value) {
    ...
    $.post(HTTP_ROOT + "easycases/ajax_change_story_point", {
        "caseId": caseId,
        "caseUId": caseUniqId,
...
function displayAssignToMem(csId, project, caseAssgnUid, caseUniqId, page, cno, client_status) {
    ...
    var detTeamId = (page == 'details') ? parseInt($('#det_task_team_' + csId).val() || 0) : 0;
    if (!detTeamId && countJS(PUSERS) && PUSERS[project]) {
        ...
        $('.detasgnlod').hide();
        $('.assgn').hide();
        if ($('#assgnload' + csId).length) {
            $('#assgnload' + csId).show();
        }
        if ($('#detAssgnload' + csId).length) {
            $('#detAssgnload' + csId).show();
        }
        _assignToXhr = $.post(HTTP_ROOT + "easycases/ajaxAssigntoMem", {
            "project": project
        }, function (data) {
            _assignToXhr = null;
            if (data) {
...
function appendAssignUsers(csId, project, caseUniqId, page, cno, caseAssgnUid, client_status) {
    ...
    if (page == 'details') {
        $('#detShowAsgnToMem' + csId).append('<li class="assignee_search_li"><input type="text" id="assignee_search" placeholder="Search Assignee.."/></li>');
    } else {
    ...
    for (ui in PUSERS[project]) {
        if (detTeamId) {
            var memTeams = PUSERS[project][ui].User.teams;
            if (!(memTeams && memTeams.indexOf(detTeamId) !== -1)) {
                continue;
            }
        }
...
function changeAssignTo(caseId, caseUniqId, assignId) {
    ...
    var prevAssignHtml = $("#" + showUpdAssign).html();
    $("#" + showUpdAssign).html("");
    $("#" + asgnlod).show();
    var caseMenuFilters = $('#caseMenuFilters').val();
    var projFil = $('#projFil').val();
    ...
    $.post(HTTP_ROOT + "easycases/ajax_change_AssignTo", {
        ...
        $("#" + asgnlod).hide();
        $("#" + showUpdAssign).html(prevAssignHtml);
    });
}
...
function ajaxassignAllTaskToUser(alltask) {
    ...
    if (projFil == 'all') {
        ...
        if (typeof alltask != 'undefined') {
            ...
            case_id = '';
            if (gethsh == 'features') {
                var title = _('Assign selected feature(s)');
            } else if (gethsh == 'epics') {
                var title = _('Assign selected epic(s)');
            } else {
        ...
        $.post(HTTP_ROOT + "easycases/ajax_assign_task_to_user", {
            ...
            if (data && data.indexOf('Database Connection Error') !== -1) {
                $("#err_msgassn_dv").html(data).show();
                $('#inner_asntskuser').hide();
            } else if (data) {
            ...
            $(".loader_dv").hide();
            $("#err_msgassn_dv").html('<div class="alert alert-danger">Error: Unable to load users. Please try again.</div>').show();
            $('#inner_asntskuser').hide();
        });
    }
}
...
function changeDueDate(caseId, duedt, text, caseUniqId) {
    var cno = 'temp';
    if (parseInt(localStorage.getItem("is_change_reason_set")) && (typeof arguments[4] == 'undefined')) {
        $.post(HTTP_ROOT + "task-actions/ajaXCheckDueDateExists", {
            "caseId": caseId
        }, function (resp) {
            if (resp.status == 'success') {
                localStorage.setItem("change_reason_param", caseId + '__' + duedt + '__' + text + '__' + caseUniqId + '__' + cno);
                addDuedateReason();
            } else {
                localStorage.setItem("change_reason_param", '');
                changeDueDate(caseId, duedt, text, caseUniqId, 0);
            }
        }, 'json');
        return false;
    } else {
    ...
    $("#" + datelod).show();
    $("#t_" + caseUniqId).remove();
    $.post(HTTP_ROOT + "easycases/ajaxChangeDueDate", {
        "caseId": caseId,
        "duedt": duedt,
        "text": text,
        "reason_id": reason_id
...
function commonDueDateChange(caseId, dateText, text, datelod, old_duetxt, showUpdDueDate, vobj) {
    if (parseInt(localStorage.getItem("is_change_reason_set")) && (typeof arguments[7] == 'undefined')) {
        $.post(HTTP_ROOT + "task-actions/ajaXCheckDueDateExists", {
            "caseId": caseId
        }, function (resp) {
            if (resp.status == 'success') {
                localStorage.setItem("change_reason_param", caseId + '__' + dateText + '__' + text + '__' + datelod + '__' + old_duetxt + '__' + showUpdDueDate + '__' + vobj);
                localStorage.setItem("common_due_change", 'comn');
                addDuedateReason();
            } else {
                localStorage.setItem("common_due_change", '');
                localStorage.setItem("change_reason_param", '');
                commonDueDateChange(caseId, dateText, text, datelod, old_duetxt, showUpdDueDate, vobj, 0);
            }
        }, 'json');
        return false;
    } else {
...
function addDuedateReason() {
    openPopup();
    $(".popup-reason-to-select").select2({
        tags: true,
        createTag: function (params) {
            ...
            if (term.length > 250) {
                var msg = "Reason must not exceed 250 characters!";
                showTopErrSucc('error', msg);
                return null;
            }
            return {
        ...
        if (evt.params.data.newTag == true) {
            ...
            $('#caseLoader').show();
            $('.new_due_change_reason').find('#sav_reason_lnk').addClass('loginactive');
            $.post(HTTP_ROOT + 'task_actions/ajaXCreateNewReason', {
                'name': evt.params.data.id
            }, function (res) {
                $('#caseLoader').hide();
                $('.new_due_change_reason').find('#sav_reason_lnk').removeClass('loginactive');
                if (res.status == 'error' && res.msg == 'name') {
                    showTopErrSucc('error', _('Due date change reason already added. Please add another reason.'));
                    $('.popup-reason-to-select option[value="' + name + '"]').remove();
                    $('.popup-reason-to-select').trigger('change');
                } else if (res.status == 'success') {
                    if (res.msg == 'saved') {
                        $(".popup-reason-to-select").find('option[value="' + name + '"]').remove();
                        $(".popup-reason-to-select").append("<option value='" + res.id + "' selected>" + name + "</option>");
                        $('.popup-reason-to-select option[value="' + res.id + '"]').prop('selected', true);
                        $('.popup-reason-to-select').trigger('change');
                    } else {
                        showTopErrSucc('error', _('Due date change reason can not be added'));
                        $('.popup-reason-to-select option[value="' + name + '"]').remove();
                        $('.popup-reason-to-select').trigger('change');
                    }
                }
    ...
    $(".popup-reason-to-select").val('').trigger('change');
    $.post(HTTP_ROOT + 'task-actions/ajaXGetDueReasons', {}, function (resdat) {
        if (resdat.status) {
            $('.popup-reason-to-select option').remove();
            $.each(resdat.reasons, function (key, value) {
                $('.popup-reason-to-select').append('<option value="' + key + '">' + ucfirst(formatText(value)) + '</option>');
            });
        }
        $(".popup-reason-to-select").select2('focus');
    }, 'json');
    $(".new_due_change_reason").show();
    $.material.init();
}

function saveDuedateReason() {
    var inpt_params = $.trim(localStorage.getItem("change_reason_param"));
    if (inpt_params != '') {
        inpt_params = inpt_params.split('__');
        if (inpt_params[4] == 'temp') {
            changeDueDate(inpt_params[0], inpt_params[1], inpt_params[2], inpt_params[3], $(".popup-reason-to-select").val());
        } else if (localStorage.getItem("common_due_change") != '' && localStorage.getItem("common_due_change") != null) {
            commonDueDateChange(inpt_params[0], inpt_params[1], inpt_params[2], inpt_params[3], inpt_params[4], inpt_params[5], inpt_params[6], $(".popup-reason-to-select").val());
        } else if (localStorage.getItem("change_reason_duedt") != '' && localStorage.getItem("change_reason_duedt") != null) {
            localStorage.setItem("change_reason_editask", $(".popup-reason-to-select").val());
            submitAddNewCase(inpt_params[0], parseInt(inpt_params[1]), inpt_params[2], inpt_params[3], inpt_params[4], parseInt(inpt_params[5]), inpt_params[6]);
        } else {
            detChangeDueDate(inpt_params[0], inpt_params[1], inpt_params[2], inpt_params[3], inpt_params[4], $(".popup-reason-to-select").val());
        }
        closePopup();
    }
}
function detChangeDueDate(caseId, duedt, text, caseUniqId, cno) {
    if (parseInt(localStorage.getItem("is_change_reason_set")) && (typeof arguments[5] == 'undefined')) {
        $.post(HTTP_ROOT + "task-actions/ajaXCheckDueDateExists", {
            "caseId": caseId
        }, function (resp) {
            if (resp.status == 'success') {
                ...
                localStorage.setItem("change_reason_param", '');
                detChangeDueDate(caseId, duedt, text, caseUniqId, cno, 0);
            }
        }, 'json');
        return false;
    } else {
    ...
    $.post(HTTP_ROOT + "easycases/ajaxChangeDueDate", {
        ...
        $('#detail_popup_reload').trigger('click');
        $('#caseLoaderPopupKB').hide();
        if (flag) {
            $("#" + showUpdDueDate).show();
            $("#" + datelod).hide();
        ...
        $.post(HTTP_ROOT + "easycases/ajax_change_priority", {
            ...
            if (data) {
                ...
                if (params[0] == 'details' || isPopup == 1) {
                    $(".task-detail-head").removeClass('urgent_priority high_priority medium_priority low_priority').addClass(data.protyCls);
                    $("#" + showUpdPri).removeClass('urgent_priority high_priority medium_priority low_priority').addClass(data.protyCls);
                    ...
                    appendReplyThread(data.curCaseId, caseId);
                } else if (params[0] == 'taskgroup' || params[0] == 'backlog') {
...
function setSubtaskGroupBy(value) {
    localStorage.setItem('AJAX_TASK_GROUPBY', value || 'None');
    showTaskByTaskGroupNew();
}

function subtaskGroupLabel(casegroupby, row) {
    var easy = (row && row.Easycase) || {};
    var assign = (row && row.AssignTo) || {};
    switch (casegroupby) {
        case 'Status':
            if (typeof easycase !== 'undefined' && typeof easycase.getStatus === 'function') {
                return easycase.getStatus(easy.type_id, easy.legend, 'detail');
            }
            var legendMap = { '1': 'New', '2': 'In Progress', '3': 'Closed', '4': 'In Progress', '5': 'Resolved' };
            return legendMap[String(easy.legend)] || ('Status ' + easy.legend);
        case 'Assign to':
            if (!easy.assign_to || easy.assign_to == 0) return 'Unassigned';
            var nm = (assign.name || '') + (assign.last_name ? ' ' + assign.last_name : '');
            return nm.trim() || ('User #' + easy.assign_to);
        case 'Priority':
            // Matches templates/element/case_project_v2.php (lines 774-788).
            if (String(easy.priority) === '1') return 'Medium';
            if (String(easy.priority) === '2') return 'Low';
            return 'High';
        case 'Date':
            var dt = easy.dt_created || '';
            if (!dt) return '— Tasks';
            var d = new Date(String(dt).replace(' ', 'T'));
            if (isNaN(d.getTime())) return String(dt).substr(0, 10) + ' Tasks';
            var today = new Date(); today.setHours(0,0,0,0);
            var yday = new Date(today); yday.setDate(today.getDate() - 1);
            var rowDay = new Date(d); rowDay.setHours(0,0,0,0);
            if (rowDay.getTime() === today.getTime()) return 'Today Tasks';
            if (rowDay.getTime() === yday.getTime()) return "Y'day Tasks";
            var months = ['Jan','Feb','Mar','Apr','May','Jun','Jul','Aug','Sep','Oct','Nov','Dec'];
            return months[rowDay.getMonth()] + ' ' + rowDay.getDate() + ', ' + rowDay.getFullYear() + ' Tasks';
    }
    return '';
}

function treeSortBy(type, el) {
    var fieldMap = {
        caseno: '#caseNum',
        title: '#caseTitle',
        caseAt: '#caseAtsort',
        duedate: '#caseDueDate',
        status: '#caseLegendsort'
    };
    var target = fieldMap[type];
    if (!target) return;
    var current = ($(target).val() || '').toLowerCase();
    var next = current === 'asc' ? 'desc' : 'asc';
    $.each(fieldMap, function (k, sel) {
        if (sel !== target) $(sel).val('');
    });
    $(target).val(next);
    showTaskByTaskGroupNew();
}
...
window.__cfFilterFields = null;

function renderCustomFieldFilter() {
    var $menu = $('#dropdown_menu_custom_field');
    if (window.__cfFilterFields === null) {
        $menu.html('<li style="padding:10px;color:#8f96a3;text-align:center;font-size:12px;">Loading…</li>');
        $.get(HTTP_ROOT + 'easycases/ajaxGetTaskCustomFieldsForFilter', function (data) {
            if (data && data.success) {
                window.__cfFilterFields = data.data || [];
                paintCustomFieldFilterList();
            } else {
                $menu.html('<li style="padding:10px;color:#e47a7a;text-align:center;font-size:12px;">Failed to load</li>');
            }
        }, 'json');
    } else {
        paintCustomFieldFilterList();
...
function paintCustomFieldFilterList() {
    var $menu = $('#dropdown_menu_custom_field');
    var all = window.__cfFilterFields || [];
    var optionFields = all.filter(function (f) {
        return (f.field_type === 8 || f.field_type === 9 || f.field_type === 10 || f.field_type === 11)
            && f.options && f.options.length > 0;
    });
    var inputFields = all.filter(function (f) {
        return (f.field_type === 1 || f.field_type === 2 || f.field_type === 3
                || f.field_type === 4 || f.field_type === 5 || f.field_type === 6);
    });

    if (!optionFields.length && !inputFields.length) {
        $menu.html('<li style="padding:10px;color:#8f96a3;text-align:center;font-size:12px;">No active custom fields.</li>');
        return;
    }

    var selected = parseCustomFieldFilter(localStorage['CUSTOM_FIELD']);
    var html = ''
        + '<input type="hidden" id="custom_field_all">'
        + '<input type="text" placeholder="Search" class="searchType" onkeyup="searchFilterItems(this);" />';

    optionFields.forEach(function (f, fIdx) {
        html += '<li class="li_check_radio" style="padding:6px 12px 2px 12px;list-style:none;">'
            + '<div style="font-size:11px;color:#7a8190;font-weight:600;text-transform:uppercase;letter-spacing:0.4px;">'
            + escapeHtml(f.label) + '</div></li>';
        f.options.forEach(function (o, oIdx) {
            var id = 'cf_' + f.id + '_' + oIdx;
            var checked = (selected[String(f.id)] && selected[String(f.id)].indexOf(String(o.value)) !== -1) ? ' checked="checked"' : '';
            html += '<li class="li_check_radio">'
                + '<div class="checkbox">'
                + '<label>'
                + '<input type="checkbox" class="custom_field_type_cls" id="' + id + '"'
                + ' data-field="' + f.id + '" data-value="' + escapeAttr(o.value) + '"'
                + ' onclick="toggleCustomFieldFilter();"' + checked + ' /> '
                + escapeHtml(o.name)
                + '</label>'
                + '</div></li>';
        });
    ...
    inputFields.forEach(function (f) {
        var inputType = 'text';
        var placeholder = 'Contains…';
        if (f.field_type === 2) { inputType = 'number'; placeholder = 'Exact value'; }
        else if (f.field_type === 3) { inputType = 'date'; placeholder = ''; }
        else if (f.field_type === 4) { inputType = 'text'; placeholder = 'Contains URL…'; }
        else if (f.field_type === 5) { inputType = 'text'; placeholder = 'Contains email…'; }
        var existing = (selected[String(f.id)] && selected[String(f.id)][0]) || '';
        html += '<li class="li_check_radio" style="padding:8px 12px 4px 12px;list-style:none;">'
            + '<div style="font-size:11px;color:#7a8190;font-weight:600;text-transform:uppercase;letter-spacing:0.4px;margin-bottom:4px;">'
            + escapeHtml(f.label) + '</div>'
            + '<input type="' + inputType + '" class="custom_field_input_cls"'
            + ' data-field="' + f.id + '"'
            + ' value="' + escapeAttr(existing) + '"'
            + ' placeholder="' + escapeAttr(placeholder) + '"'
            + ' style="width:100%;padding:5px 8px;border:1px solid #cfd4dc;border-radius:4px;font-size:12px;"'
            + ' />'
            + '</li>';
    });
    ...
    $menu.find('.custom_field_input_cls').on('input change blur', function () {
        if (typingTimer) clearTimeout(typingTimer);
        typingTimer = setTimeout(toggleCustomFieldFilter, 500);
    });

    if (typeof $.material !== 'undefined' && $.material.init) $.material.init();
}
...
function parseCustomFieldFilter(raw) {
    var out = {};
    if (!raw || raw === 'all') return out;
    raw.split(';').forEach(function (seg) {
        if (seg.indexOf(':') < 0) return;
        var p = seg.split(':');
        var fid = p[0];
        var vals = (p[1] || '').split('|').filter(function (v) { return v !== ''; });
        if (vals.length) out[String(fid)] = vals;
    });
    return out;
}

function buildCustomFieldFilter(selected) {
    var parts = [];
    Object.keys(selected).forEach(function (fid) {
        if (selected[fid] && selected[fid].length) {
            parts.push(fid + ':' + selected[fid].join('|'));
        }
    });
    return parts.join(';');
}

function toggleCustomFieldFilter() {
    var selected = {};
    $('#dropdown_menu_custom_field .custom_field_type_cls:checked').each(function () {
        var fid = String($(this).attr('data-field'));
        var val = $(this).attr('data-value');
        if (!selected[fid]) selected[fid] = [];
        selected[fid].push(val);
    });
    $('#dropdown_menu_custom_field .custom_field_input_cls').each(function () {
    ...
    var wire = buildCustomFieldFilter(selected);
    if (wire === '') {
        delete localStorage['CUSTOM_FIELD'];
        $('#caseCustomField').val('');
    } else {
        localStorage['CUSTOM_FIELD'] = wire;
        $('#caseCustomField').val(wire);
    }
    if (typeof filterRequest === 'function') {
        filterRequest('custom_field');
    } else if (typeof easycase !== 'undefined' && easycase.refreshTaskList) {
        easycase.refreshTaskList();
    }
}
        ...
        if (type == 'status') {
            $.post(HTTP_ROOT + "requests/ajax_case_status", {
                ...
                if (data) {
                    ...
                    $.material.init();
                    if (typeof localStorage['STATUS'] != 'undefined' && localStorage['STATUS'] != 'all') {
                    ...
                    } else if (typeof localStorage['CUSTOM_STATUS'] == 'undefined' && $("#status_all").is(":checked")) {
                        $('#dropdown_menu_status')
                            .find('input[type="checkbox"]')
                            .not('#status_all')
                            .prop('checked', true);
                    }
                    if (typeof localStorage['CUSTOM_STATUS'] != 'undefined' && localStorage['CUSTOM_STATUS'] != 'all') {
            ...
            $.post(HTTP_ROOT + "requests/ajax_case_status", {
                ...
                'milestoneIds': milestoneIds,
                'checktype': checktype,
                'page_type': 'ajax_bunit',
                "pageload": 0
            }, function (data) {
                if (data) {
                    $('#dropdown_menu_bunit').html(data);
                    $.material.init();
                    if (typeof localStorage['BUNIT'] != 'undefined') {
                        x = localStorage['BUNIT'].split("-");
                        for (var i = 0; i < x.length; i++) {
                            if (x[i] == 'unassigned') {
                                $(".bunit0").attr("checked", "checked");
                            } else {
                                $(".bunit" + x[i]).attr("checked", "checked");
                            }
                        }
                    }
                }
            ...
            $.post(HTTP_ROOT + "requests/ajax_case_status", {
                ...
                'milestoneIds': milestoneIds,
                'checktype': checktype,
                'page_type': 'ajax_epics',
                "pageload": 0
            }, function (data) {
                if (data) {
                    $('#dropdown_menu_epics').html(data);
                    $.material.init();
                    if (typeof localStorage['EPICS'] != 'undefined') {
                        x = localStorage['EPICS'].split("-");
                        for (var i = 0; i < x.length; i++) {
                            $(".epic" + x[i]).attr("checked", "checked");
                        }
                    }
                }
            });
        } else if (type == 'features') {
            var caseEpics = typeof localStorage['EPICS'] != 'undefined' ? localStorage['EPICS'] : $('#caseEpics').val();
            $.post(HTTP_ROOT + "requests/ajax_case_status", {
                "projUniq": projFil,
                "pageload": 0,
                "caseMenuFilters": caseMenuFilters,
                ...
                'checktype': checktype,
                'caseEpics': caseEpics,
                'page_type': 'ajax_features',
                "pageload": 0
            }, function (data) {
                if (data) {
                    $('#dropdown_menu_features').html(data);
                    $.material.init();
                    if (typeof localStorage['FEATURES'] != 'undefined') {
                        x = localStorage['FEATURES'].split("-");
                        for (var i = 0; i < x.length; i++) {
                            $(".feature" + x[i]).attr("checked", "checked");
                        }
                    }
                }
            });
            ...
            $.post(HTTP_ROOT + "requests/ajax_case_status", {
                ...
                'milestoneIds': milestoneIds,
                'checktype': checktype,
                'page_type': 'ajax_skill',
                "pageload": 0
            }, function (data) {
                if (data) {
                    $('#dropdown_menu_skill').html(data);
                    $.material.init();
                    if (typeof localStorage['SKILL'] != 'undefined') {
                        x = localStorage['SKILL'].split("-");
                        for (var i = 0; i < x.length; i++) {
                            $(".skill" + x[i]).attr("checked", "checked");
                        }
                    }
                }
            });
...
function common_reset_filter(ftype, id, obj) {
    ...
    } else if (ftype == 'custom_field') {
        // id is "{fieldId}|{optionValue}" — drop just that (field, value) pair
        // from the wire string and uncheck the matching checkbox if visible.
        var pipe = id.indexOf('|');
        if (pipe < 0) return;
        var dropFid = id.substring(0, pipe);
        var dropVal = id.substring(pipe + 1);
        var raw = $('#caseCustomField').val() || localStorage['CUSTOM_FIELD'] || '';
        if (!raw || raw === 'all') { $('#caseCustomField').val('all'); return; }
        var rebuilt = raw.split(';').map(function (seg) {
            if (seg.indexOf(':') < 0) return '';
            var p = seg.split(':');
            if (String(p[0]) !== String(dropFid)) return seg; // untouched
            var vals = (p[1] || '').split('|').filter(function (v) { return v !== '' && v !== dropVal; });
            return vals.length ? (p[0] + ':' + vals.join('|')) : '';
        }).filter(function (s) { return s !== ''; }).join(';');
        if (rebuilt === '') {
            $('#caseCustomField').val('all');
            localStorage.removeItem('CUSTOM_FIELD');
        } else {
            $('#caseCustomField').val(rebuilt);
            localStorage.setItem('CUSTOM_FIELD', rebuilt);
        }
        // Uncheck the corresponding option-box, or clear the free-form input
        // for that field, whichever applies.
        $('#dropdown_menu_custom_field .custom_field_type_cls').each(function () {
            if (String($(this).attr('data-field')) === String(dropFid)
                && String($(this).attr('data-value')) === String(dropVal)) {
                $(this).prop('checked', false);
            }
        });
        $('#dropdown_menu_custom_field .custom_field_input_cls').each(function () {
            if (String($(this).attr('data-field')) === String(dropFid)
                && String($(this).val()) === String(dropVal)) {
                $(this).val('');
            }
        });
    ...
    if (!$('#reset_btn').is(":visible") || $('#caseStatusprev').val() != status) {
        ...
        if (parseInt($("#subtask-container").height()) > 70) {
            ...
            if ($('#caseMenuFilters').val() == 'kanban') {
                easycase.showKanbanTaskList();
            } else if ($('#caseMenuFilters').val() == 'calendar' || paramdata[0] == 'calendar') {
                calendarView('calendar');
...
function customStatusTop(status) {
    ...
    casePage = 1;
    paramdata = parseUrlHash(urlHash);
    if (!$('#reset_btn').is(":visible") || $('#caseCustomStatusprev').val() != status || document.getElementById('reset_btn').style.display == 'inline-block') {
...
easycase.routerHideShow = function (page) {
    /*
     * Hard stop on the Vue task pages. This is the legacy hash router: it hides
     * and rebuilds the old task chrome, which on those pages means ripping out
     * the Vue app's markup and replacing it with the old list. Several legacy
     * paths still call it (close-detail, angular routes, action handlers), so
    ...
    if (page != 'details') {
        ...
        if (page == 'overview') {
            ...
            if ($('#lviewtype').val() && $('#lviewtype').val().trim() == 'compact') {
                $('.tasksortby-div').hide();
                $('#sortby_items').hide();
                $('.taskgroupby-div').hide();
                $('#groupby_items').hide();
...
easycase.showFiles = function (type) {
    ...
    var projFil = $('#projFil').val();
    var projIsChange = $('#projIsChange').val();
    displayMenuProjects('dashboard', '8', 'files');
...
function getkanbanCounts() {
    ...
    var case_due_date = $('#casedueDateFil').val();
    var case_srch = $('#case_srch').val();
    var caseIdElement = document.getElementById('caseId');
    var caseId = caseIdElement ? caseIdElement.value : '';
    var kanban_config = {
        "projFil": projFil,
        "projIsChange": projIsChange,
        "casePage": 0,
    ...
    var strURL = HTTP_ROOT + "easycases/";
    var fileUrl = HTTP_ROOT + "boards/kanbanTask";
...
function downloadTask(csUid, caseNum) {
    ...
    $(w.document.body).html('<div class="loader_dv" style="font-family:\'Helvetica\',\'Helvetica\',\'Helvetica Neue\',\'Helvetica\',\'Arial\',\'sans-serif\';font-size:17px;"><center><img src="' + HTTP_IMAGES + 'images/case_loader2.gif" alt="Loading..." title="Loading..." /><br>Please wait we are preparing your download...</center></div>');
    $.post(url, {
        'caseUid': csUid
    }, function (res) {
        if (res) {
...
function actiononTask(taskid, taskUid, taskNum, actiontype, legend) {
    ...
    var is_sub = (typeof arguments[6] != 'undefined' && parseInt(arguments[6]) != '') ? 1 : 0;
    if (in_array('closeWithCheckLists', params)) {
        $('#checklistCloseBtn').hide();
        $('#checkListCloseLdr').show();
    }
    let postParams = {
        'taskId': taskid,
        'taskUid': taskUid,
    ...
    if (in_array('closeWithCheckLists', params)) {
        postParams.closeCheckLists = 1;
    }
    $.post(HTTP_ROOT + 'easycases/taskactions', postParams, function (res) {
        if (res) {
            if (!in_array('closeWithCheckLists', params)) {
                if (typeof res.checklistOpen != "undefined" && res.checklistOpen) {
                    res.params = params;
                    $("#all-checklists").html(tmpl("checklist_close_tmpl", res));
                    $.material.init();
                    checkListPopup();
                }
            }
            ...
            var hashtag = parseUrlHash(urlHash);
            if (res.succ) {
                var current_task_type = res.task_details?.Easycase?.type_id;
                var task_label_map = {
                    13: _('Epic'),
                    14: _('Story'),
                    15: _('Feature'),
                };
                var task_label = task_label_map[current_task_type] || _('Task');
                $('#caseId').val("");
                if (actiontype == 'tasktype' || actiontype == 'duedate' || actiontype == 'priority' || actiontype == 'assignto' || actiontype == 'story_point' || actiontype == 'esthour' || actiontype == 'cmpltsk' || actiontype == 'titleChange' || actiontype == 'removeFile') {
                    if (actiontype != 'removeFile') {
                        showTopErrSucc('success', task_label + ' #' + taskNum + ' ' + _('updated.'));
                    }
                } else {
                    if (hashtag[0] == 'tasks') {
                    ...
                    if (actiontype.toLowerCase() == 'new') {
                        showTopErrSucc('success', _('Status of ') + task_label + ' #' + taskNum + ' ' + _('changed to') + ' ' + actiontype + '.');
                    } else {
                        showTopErrSucc('success', task_label + ' #' + taskNum + ' ' + actiontype + 'ed.');
                    }
                }
                $.post(HTTP_ROOT + "easycases/ajaxemail", {
                    'json_data': res.data,
            ...
            if ((getCookie('TASKGROUPBY') == 'milestone')) { }
            if (hashtag[0] == 'tasks') { } else if (hashtag[0] == 'taskgroup') { } else if (hashtag[0] == 'details') { } else if (hashtag[0] == 'timesheet_weekly') { } else if (hashtag[0] == 'activesprint') {
                easycase.showsprint();
            } else if (hashtag[0] == 'backlogByStatus') {
                easycase.showbacklogBystatus();
            } else if (hashtag[0] == 'backlog') { } else if ($('#caseMenuFilters').val() == 'milestonelist') {
                showMilestoneList();
            } else if (hashtag[0] == 'taskgroups') {
                showTaskByTaskGroupNew();
            } else {
                if (PAGE_NAME == "resourceAvailability") { } else {
                    statusid = '1';
        ...
        if (in_array('closeWithCheckLists', params)) {
            $('#checklistCloseBtn').show();
            $('#checkListCloseLdr').hide();
            closePopup();
            easycase.refreshTaskList(taskUid, 1);
            $("#all-checklists").html('');
        }
    }, 'json');
}
...
function editmessage(obj, id, projid) {
    ...
    $.post(HTTP_ROOT + "easycases/edit_reply", {
        ...
        if ($('#replytext_content' + id).has(".remove-editor-img")) {
            $("#replytext_content" + id + " .remove-editor-img").remove();
        }
        ...
        tinymce.init({
            ...
            plugins: 'image paste importcss autolink directionality fullscreen link   table charmap hr pagebreak nonbreaking anchor advlist lists wordcount autoresize help mention',
            menubar: false,
            branding: false,
            statusbar: false,
            toolbar: 'undo redo | bold italic underline strikethrough | outdent indent | numlist bullist | forecolor backcolor  | fullscreen',
            toolbar_sticky: true,
            importcss_append: true,
            image_caption: true,
            browser_spellcheck: true,
...
function save_editedvalue_reply(caseno, id, proj_id, repUniqId) {
    ...
    $.post(HTTP_ROOT + "easycases/save_editedvalue", {
        ...
        if (data.message == "fail") {
            ...
            $('.delete-editor-img').off().on('click', function (e) {
                e.stopPropagation();
                var img_file = $(this).attr('data-name');
                if (confirm(_('Would you like to delete this file?'))) {
                    $.ajax({
                        url: HTTP_ROOT + "easycases/ajaXRemoveEditorFile",
                        data: {
...
function update_sequence(caseIds, milestone_id, project_id) {
    var prev_mid_t = (typeof arguments[3] != 'undefined') ? arguments[3] : '';
    var url = HTTP_ROOT + "milestones/updateSequence";
    var taskLen = parseInt($('#backlog_tsk_cnt_' + milestone_id).text()) + 1;
    var taskLen_p = parseInt($('#backlog_tsk_cnt_' + prev_mid_t).text()) - 1;
    if (milestone_id != 0 && prev_mid_t != milestone_id && (caseIds.length != taskLen || $('.os_sprite' + milestone_id).find('.material-icons').text() == 'keyboard_arrow_right')) {
        $.each(caseIds, function (k, v) {
            $('#curRow' + v).remove();
        });
    }
    var sprint_stated = 0;
    var urlHash = getHash();
    if (urlHash == 'backlog') {
        if ($('#st_crt_btn_' + milestone_id).length || $('#st_crt_btn_' + prev_mid_t).length) {
            if ($('#tobody_' + milestone_id).hasClass('no_shortable') || $('#tobody_' + prev_mid_t).hasClass('no_shortable')) {
                sprint_stated = 1;
            }
        }
    }
    $.post(url, {
        ...
        'milestone_id': milestone_id,
        'project_id': project_id,
        'sprint_stated': sprint_stated
    }, function (res) {
        createCookie("PREOPENED_TASK_GROUP_ID", milestone_id, 365, DOMAIN_COOKIE);
        if (res) {
            if (prev_mid_t != milestone_id && prev_mid_t.indexOf("curRow") === -1) {
                showTopErrSucc('success', _('Task moved successfully.'));
            }
            if (urlHash != 'backlog') {
                easycase.refreshTaskList();
            } else {
                if (res.dataChild.length) {
                    taskLen_p = taskLen_p - parseInt(res.dataChild.length);
                    taskLen = taskLen + parseInt(res.dataChild.length);
                    $.each(res.dataChild, function (key, val) {
                        $('#curRow' + val).remove();
                    });
                }
                if (typeof res.parallel_sprint != 'undefined' && res.parallel_sprint == 1) {
                    $('.openSprint').text(0);
                    $('.wipSprint').text(0);
                    $('.closeSprint').text(0);
                    $.each(res.arry_sts_cnt, function (key, data) {
                        $("#st_crt_btn_" + key).find('.openSprint').text(data.opn);
                        $("#st_crt_btn_" + key).find('.wipSprint').text(data.wip);
                        $("#st_crt_btn_" + key).find('.closeSprint').text(data.done);
                    });
                } else {
                    if (typeof res.arry_sts_cnt.opn != 'undefined') {
                        $('.openSprint').text(res.arry_sts_cnt.opn);
                        $('.wipSprint').text(res.arry_sts_cnt.wip);
                        $('.closeSprint').text(res.arry_sts_cnt.done);
                    }
                }
                if (prev_mid_t != '' && prev_mid_t != milestone_id) {
                    $('#backlog_tsk_cnt_' + prev_mid_t).text(taskLen_p);
                    $('#backlog_tsk_cnt_' + milestone_id).text(taskLen);
                }
                if ($('.os_sprite' + milestone_id).find('.material-icons').text() == 'keyboard_arrow_down') {
                    showBacklogByMilestone(milestone_id, project_id, $('.os_sprite' + milestone_id));
                    comnBacklogItemSum(prev_mid_t, res.velocity);
                }
            }
        }
    }, 'json');
...
function collapse_by_backlog_title(mid, pid) {
    var ichk = (typeof arguments[2] != 'undefined') ? arguments[2] : 0;
    collapse_backlog($('.os_sprite' + mid), ichk);
    var hashtag = parseUrlHash(urlHash);
    if (hashtag[0] == 'backlog') {
        showBacklogByMilestone(mid, pid, $('.os_sprite' + mid));
    }
}
...
function tasklistexport() {
    ...
    var caseStatus = $('#caseStatus').val();
    var caseCustomStatus = getGlobalFilters('CUSTOM_STATUS');
    var priFil = ($('#priFil').val() == '') ? 'all' : $('#priFil').val();
    var caseTypes = ($('#caseTypes').val() == '') ? 'all' : $('#caseTypes').val();
    var caseLabel = ($('#caseLabel').val() == '') ? 'all' : $('#caseLabel').val();
    var caseMember = ($('#caseMember').val() == '') ? 'all' : $('#caseMember').val();
    var caseComment = $('#caseComment').val();
    var caseTaskGroup = $('#caseTaskgroup').val();
    var caseAssignTo = ($('#caseAssignTo').val() == '') ? 'all' : $('#caseAssignTo').val();
    var caseDate = $('#caseDate').val();
    var case_date = $('#caseDateFil').val();
    var case_due_date = $('#casedueDateFil').val();
    var taskgroup_fil = '';
    ...
    var caseCreateDate = $('#caseCreatedDate').val();
    var projIsChange = $('#projIsChange').val();
    var casePageType = $('#casePageType').val() || '';
    if (caseMenuFilters != 'milestone' && caseMenuFilters != 'files') {
        if (caseMenuFilters === "") {
            caseMenuFilters = "cases";
        }
...
function showBacklogByMilestone(mid, pid, obj) {
    if ($.trim($(obj).text()) == 'keyboard_arrow_down') {
        var url = HTTP_ROOT + 'requests/loadTaskByBacklog';
        var tbody = $(obj).closest('tbody');
        tbody.find('tr[id^="curRow"],.white_bg_tr,.noRecord').remove();
        tr = '<tr class="loadTasks"><td colspan="10" align="center"><div class="loader_bg"><div class="loadingdata"><img src="' + HTTP_ROOT + 'images/rolling.gif?v=' + RELEASE + '" alt="' + _('loading') + '..." title="' + _('loading') + '..."/></div></div></td></tr>';
        tbody.append(tr);
        ...
        var caseDate = $('#caseDate').val();
        var hashtag = parseUrlHash(urlHash);
        var caseSearch = $("#inner-search-backlog").val();
        var caseMenuFilters = $('#caseMenuFilters').val();
        var projIsChange = $('#projIsChange').val();
        var caseUrl = "";
        var detailscount = 0;
        var reply = 0;
        var mstype = "";
        var casePage = (localStorage.getItem("BACKLOG_PAGE")) ? localStorage.getItem("BACKLOG_PAGE") : '1';
        var filters_bk = filterBacklog('quk_fil_bklog', 1);
        if (!filters_bk.length) {
            filters_bk = '';
        }
        var casebckLogSort = [];
        if (localStorage.backlog_case_sort) {
        ...
        $.post(url, {
            ...
            caseMenuFilters: caseMenuFilters,
            caseUrl: caseUrl,
            bkFilters: filters_bk,
            projIsChange: projIsChange,
            casebckLogSort: casebckLogSort
        }, function (res) {
            tbody.find('.loadTasks').remove();
            tbody.append(tmpl("task_by_backlo_tmpl", res));
            $('#backlog_tsk_cnt_' + mid).text(res.caseAll.length);
            iniDateFilter();
            $('.qt_task_type_backlog').select2({
                templateSelection: formatTaskType,
                templateResult: formatTaskType,
            }).on('change', function (evt) { });
            $.material.init();
...
function createStatusNewKbn(id) {
    ...
    $.post(HTTP_ROOT + 'custom-statuses/createNewStatus', {
        id: btoa(id),
        'from_page': 'kanban'
    }, function (res) {
            ...
            success: function (res) {
                if (res.status) {
                    ...
                    if (urlHash == 'tasks' || urlHash == 'taskgroup' || urlHash == 'backlog' || urlHash == 'details' || urlHash == 'taskgroups') {
                        ...
                        if (datatype == 1) {
                            like_txt = like_txt - 1;
                            var str = '<a href="javascript:void(0);" class="caseFav" onclick="setCaseFavourite(' + id + ',' + pid + ',\'' + cuid + '\',' + type + ',0)" rel="tooltip" original-title="Set favourite" style="color:#888888;"><i class="material-icons" style="font-size:18px;color:#888888;">star_border</i></a>';
                            $("#caseProjectSpanFav" + id).html(str);
                        } else {
                            like_txt = like_txt + 1;
                            var str = '<a href="javascript:void(0);" class="caseFav" onclick="setCaseFavourite(' + id + ',' + pid + ',\'' + cuid + '\',' + type + ',1)" rel="tooltip" original-title="Remove favourite" style="color:#FFDC77;"><i class="material-icons" style="font-size:18px;color:#FFDC77;">star</i></a>';
                            $("#caseProjectSpanFav" + id).html(str);
                        }
                        if (urlHash == 'backlogByStatus') {

                        } else {

                        }
                        $('#tskTabFavCnt').html('(' + like_txt + ')');
                    } else if (urlHash == 'kanban' || urlHash == 'milestonelist' || urlHash == 'activesprint' || urlHash == 'backlogByStatus' || urlHash.indexOf("kanban") >= 0) {
                        if (datatype == 1) {
                            $("#kanbanDivFav" + id).attr('title', _('Set favourite'));
                            var str = '<a href="javascript:void(0);" class="caseFav" onclick="setCaseFavourite(' + id + ',' + pid + ',\'' + cuid + '\',' + type + ',0)" rel="tooltip" original-title="Set favourite" style="margin-top:0px;color:#888888;"><i class="material-icons" style="font-size:18px;color:#888888;">star_border</i></a>';
                            $("#kanbanDivFav" + id).html(str);
                        } else {
                            $("#kanbanDivFav" + id).attr('title', _('Remove favourite'));
                            var str = '<a href="javascript:void(0);" class="caseFav" onclick="setCaseFavourite(' + id + ',' + pid + ',\'' + cuid + '\',' + type + ',1)" rel="tooltip" original-title="Remove favourite" style="margin-top:0px;color:#FFDC77;"><i class="material-icons" style="font-size:18px;color:#FFDC77;">star</i></a>';
                            $("#kanbanDivFav" + id).html(str);
                        }
                    }
                    if (type == 2) {
                        if (datatype == 1) {
                            var str = '<a href="javascript:void(0);" class="caseFav" onclick="setCaseFavourite(' + id + ',' + pid + ',\'' + cuid + '\',' + type + ',0)" rel="tooltip" original-title="Set favourite" style="color:#888888;"><i class="material-icons" style="color:#888888;">star_border</i></a>';
                            $("#caseDetailsSpanFav" + id).html(str);
                        } else {
                            var str = '<a href="javascript:void(0);" class="caseFav" onclick="setCaseFavourite(' + id + ',' + pid + ',\'' + cuid + '\',' + type + ',1)" rel="tooltip" original-title="Remove favourite" style="color:#FFDC77;"><i class="material-icons" style="color:#FFDC77;">star</i></a>';
                            $("#caseDetailsSpanFav" + id).html(str);
                        }
                        var like_txt = $.trim($('#tskTabFavCnt').html());
                        like_txt = parseInt(like_txt.replace(/[{()}]/g, ''));
                        ...
                        if (datatype == 1) {
                            var str = '<a href="javascript:void(0);" class="caseFav" onclick="setCaseFavourite(' + id + ',' + pid + ',\'' + cuid + '\',' + type + ',0)" rel="tooltip" original-title="Set favourite" style="color:#888888;"><span id="fav_icon" class="cmn_tskd_sp starline_icon"></span>Favorite</a>';
                            $("#caseDetailsSpanFav" + id).html(str);
                        } else {
                            var str = '<a href="javascript:void(0);" class="caseFav" onclick="setCaseFavourite(' + id + ',' + pid + ',\'' + cuid + '\',' + type + ',1)" rel="tooltip" original-title="Remove favourite" style="color:#888888;"><span id="fav_icon" class="cmn_tskd_sp starfill_icon"></span>Favorite</a>';
                            $("#caseDetailsSpanFav" + id).html(str);
                        }
                    }
                }
    ...
    $.post(HTTP_ROOT + "easycases/ajax_get_task_list", {
        ...
        if (data) {
            ...
            if (data === "not_possible") {
                ...
                if ($('#inner_mk_tsk_sbtsk').find('#no_parent_task_found').length) { $('#mksbtsk_btn').hide(); }
                $.material.init();
                $(".select").select2();
            }
        }
...
function showTaskByTaskGroupNew() {
    ...
    $.post(url, {
        ...
        caseChangePriority: caseChangePriority,
        caseChangeType: caseChangeType,
        casegroupby: (localStorage.getItem('AJAX_TASK_GROUPBY') || 'None'),
        mstype: mstype,
        priFil: priFil,
        caseTypes: caseTypes,
        caseLabel: caseLabel,
...
function ajaxSubtaskListViewPagination() {
    $('#subtask-load-more').text(_('Loading tasks...'));
    ...
    if (res1.resCaseProj.length) {
        ...
        $('#subtask-load-more').text(_('Load More Task'));
    } else {
        $('#subtask-load-more').hide();
    }
}
...
$(document).ready(function () {
    $(document).on("click", "#over-due", function () {
        $("#sprint_duedate_overdue").trigger('click');
    });
...
function DeleteAllCaseTaskList(type) {
    ...
    var cno = case_no.join(',');
    if (gethsh == 'features') {
        var confMesg = _("Are you sure you want to delete the feature ?");
    } else if (gethsh == 'epics') {
        var confMesg = _("Are you sure you want to delete the epic ?");
    } else {
        var confMesg = case_no.length == 1 ? _("Are you sure you want to delete the task ?") : _("Are you sure you want to delete ") + case_no.length + _(" tasks ?");
    }
    if (confirm(confMesg)) {
...
function markBillabltType(obj) {
    ...
    if (tlg_ids.length > 0) {
        $("#caseLoader").show();
        var strurl = HTTP_ROOT + "LogTimes/updateBillableType";
        $.post(strurl, {
            "log_id": tlg_ids,
...
function openSplitTaskPopup(startdate = null, duedate = null, type = null, caseid = null, assignto = null, projid = null, estdhr = null) {
    if (startdate != null && duedate != null && type == 'details') {
        var start_date = startdate;
        var due_date = duedate;
        var case_id = caseid;
        var user_id = assignto;
        var prj_id = projid;
        var estd_hr = estdhr;
    } else {
        var start_date = $('#start_date').val();
        var due_date = $('#due_date').val();
        var case_id = $('#easycase_uid').val();
        var user_id = $('.crtskasgnusr ').val();
        var prj_id = $('.prj-select').val();
        var estd_hr = $('#estimated_hours').val();
    }
    page_name = getHash();
    page_name = parseUrlHash(page_name);
    if (start_date == '1970-01-01' || due_date == '1970-01-01') {
        showTopErrSucc('error', _("Start date and due date field can not be blanked"));
        return false;
    }
    if ((start_date == '' && due_date == '') || (user_id == 0)) {
        showTopErrSucc('error', _("Start date and due date field can not be blanked"));
        return false;
    }
    old_start_date = localStorage.getItem('split_start_date');
    old_end_date = localStorage.getItem('split_end_date');
    old_assign_to = localStorage.getItem('split_assign_to');
    var date1 = new Date(start_date);
    var date2 = new Date(due_date);
    var Difference_In_Time = date2.getTime() - date1.getTime();
    var Difference_In_Days = Difference_In_Time / (1000 * 60 * 60 * 24);
    if (Difference_In_Days > 15) {
        showTopErrSucc('error', _("You can schedule only for 15 days."));
        return false;
    }
    split_t_hr = $('#split_t_hr').val();
    openPopup();
    $('.split_estimation_task_popup').show();
    if ((old_start_date != start_date || old_end_date != due_date || old_assign_to != user_id) || $('#create_another_task').prop('checked') == true || (split_t_hr == null)) {
        $('#split_task_loader').show();
        $('#split_tsk_sve_cncl').hide();
        $.post(HTTP_ROOT + 'easycases/ajaxRangeStartDueDate', {
            ...
            if (res) {
                localStorage.setItem('split_start_date', start_date);
                localStorage.setItem('split_end_date', due_date);
                localStorage.setItem('split_assign_to', user_id);
                $('#split_est_task_tmpl').html(tmpl('split_est_task_tmpl_page', res));
                $('#split_task_loader').hide();
                $('#split_tsk_sve_cncl').show();
                if (page_name[0] == 'details' && $('#easycase_uid').val() == "") {
                    calculateTotalHr();
                }
            }
        }, "json");
...
function saveSplitTaskEstimation() {
    page_name = getHash();
    page_name = parseUrlHash(page_name);
    date = localStorage.getItem('split_estd_date');
    date = date.split(",");
    case_id = $('#task_id_for_split').val();
    totalhr = $('#split_t_hr').val();
    booked_hr = {};
    for (var i = 0; i < date.length; i++) {
        booked_hr[date[i]] = $('#splt_estd' + date[i]).val();
    }
    var ahf = JSON.stringify(booked_hr);
    if (page_name[0] == 'details' && $('#easycase_uid').val() == "") {
        $.post(HTTP_ROOT + 'easycases/ajaxSaveSplitEstdFromDetailPage', {
            'split_estd_task': ahf,
            'case_id': case_id,
            'totalhr': totalhr
        }, function (res) {
            if (res) {
                if (1 || res.updated_est_hr) {
                    var esthor = format_time_hr_min(res.updated_est_hr);
                    $('#est_hr_updated').html(esthor);
                    estTxt = res.updated_est_hr;
                    $('#est_time').text(format_time_hr_min(estTxt));
                    localStorage.setItem('is_splitted', 1);
                    $("#est_hr" + case_id).hide();
                    $('#estdiv' + case_id).find('.estb').show();
                    $('#estdiv' + case_id).find('.estb span').html(format_time_hr_min(estTxt)).show();
                    $('.tlog_top_cnt').find('h6:nth-child(2)').find('span').html(format_time_hr_min(estTxt));
                }
                showTopErrSucc('success', _("Saved the estimation hour"));
                $('.caleder-due-date').addClass('no-pointer');
            }
        }, 'json');
    } else {
        if (totalhr == "00:00" || totalhr == '') { } else {
            $('#CS_issplit_value').val(ahf);
...
function calculateTotalHr() {
    ...
    totalhr = 0;
    for (var i = 0; i < date.length; i++) {
        var sptime = $('#splt_estd' + date[i]).val();
        if (sptime != '') {
            if (sptime.indexOf(':') > '-1') {
                ...
                if (sp_hr > 18 || (sp_hr == 18 && sp_min > 0)) {
                    showTopErrSucc('error', _("Estimation hour can not be more than 18 hour."));
                    $('#splt_estd' + date[i]).val('');
                    return false;
                }
            } else {
                sp_hr = sptime;
                sp_min = '0';
                sp_cmp_hr = parseInt(sp_hr);
                if (sp_cmp_hr > 18) {
                    showTopErrSucc('error', _("Estimation hour can not be more than 18 hour."));
                    $('#splt_estd' + date[i]).val('');
                    return false;
            ...
            totalhr += total_sp_min;
        } else {
            total_sp_min = 0;
        }
    }
    var task_id = $('#task_id_for_split').val();
    var totalhour = toHoursAndMinutes(totalhr);
    if (totalhour == "00:00" || totalhour == "") {
        resetSplitTaskValues();
    }
    $('#splitted_total_hour').text(totalhour);
    $('#split_t_hr').val(totalhour);
    $('#estimated_hours').val(totalhour);
    $('#est_hr' + task_id).val(totalhour);
}
...
function resetSplitTaskValues() {
    $('#CS_issplit_value').val('');
    $('#splitted_total_hour').text('');
    if ($('#estimated_hours').prop('disabled') == true) {
        $('#estimated_hours').prop('disabled', false);
        $('#CS_issplit_value').val('');
    }
    if ($('#is_recurring').prop('disabled') == true) {
        $('#is_recurring').prop('disabled', false);
    }
    if ($('#start_date').prop('disabled') == true) {
        $('#start_date').prop('disabled', false);
    }
    if ($('#due_date').prop('disabled') == true) {
...
function toggleEpicOptions() {
    $("#mvTaskToProj,#cpTaskToProj").show();
    $('.chkOneTsk:checked').each(function () {
        if ($(this).attr('data_epic_type') == 'true') {
            $("#mvTaskToProj,#cpTaskToProj").hide();
            return false;
        }
    })
}
...
// `pagehide` rather than the deprecated `unload`, which browsers now block via
// Permissions-Policy (logging a console violation and skipping the handler).
window.addEventListener('pagehide', function () {
    createCookie("PREOPENED_TASK_GROUP_IDS", JSON.stringify([]), 365, DOMAIN_COOKIE);
});



/**
 * LabelCustomizer — Angular `| t` filter backed by gettext.js's catalog.
 *
 *   Text:        {{ 'Quick Task' | t }}
 *   Attribute:   placeholder="{{ 'Search Projects' | t }}"
 */
(function () {
    function makeTranslateFilter() {
        return function (input) {
            if (typeof input !== 'string') { return input; }
            var jq = window.jQuery;
            var M = (jq && jq.gt && jq.gt.messages && jq.gt.messages[jq.gt.lang]) || {};
            return Object.prototype.hasOwnProperty.call(M, input) ? M[input] : input;
        };
    }
    if (typeof angular !== 'undefined') {
        ['case_dashboard_App', 'dashboard_App', 'commonMethods'].forEach(function (mod) {
            try { angular.module(mod).filter('t', makeTranslateFilter); } catch (e) { /* module not in this page */ }
        });
    }
})();

/**
 * Start date, edited from the task-detail Timeline panel.
 *
 * Start date had no editor anywhere in the detail view — only the calendar's
 * drag-to-reschedule wrote gantt_start_date. ajaxChangeDueDate already accepts
 * `startdt` (the calendar view posts both dates through it), so this reuses
 * that endpoint rather than adding another write path.
 */
function osEditStartDate(caseId) {
    var label = document.getElementById('os_startdate_' + caseId);
    var input = document.getElementById('os_startdate_input_' + caseId);
    if (!label || !input) return;
    label.style.display = 'none';
    input.style.display = '';
    input.focus();
    if (typeof input.showPicker === 'function') {
        try { input.showPicker(); } catch (e) { /* not user-initiated in some browsers */ }
    }
}

function osCancelStartDate(caseId) {
    var label = document.getElementById('os_startdate_' + caseId);
    var input = document.getElementById('os_startdate_input_' + caseId);
    if (!label || !input) return;
...
function osSaveStartDate(caseId, isoDate) {
    var label = document.getElementById('os_startdate_' + caseId);
    var input = document.getElementById('os_startdate_input_' + caseId);
    if (!isoDate) { osCancelStartDate(caseId); return; }

    // The endpoint parses with strtotime and stores Y-m-d; m/d/Y is what the
    // rest of the app posts, so keep the same shape.
    var parts = isoDate.split('-');
    var posted = parts[1] + '/' + parts[2] + '/' + parts[0];

    $.post(HTTP_ROOT + 'easycases/ajaxChangeDueDate', {
        caseId: caseId,
        startdt: posted
    }, function (data) {
        if (data && data.success === 'No') {
            showTopErrSucc('error', data.message || _('Could not set the start date.'));
            osCancelStartDate(caseId);
            return;
        }
        if (label) {
            label.textContent = new Date(isoDate + 'T00:00:00')
                .toLocaleDateString(undefined, { day: 'numeric', month: 'short', year: 'numeric' });
        }
        if (input) input.value = isoDate;
        if (data && data.last_updated) $('#lst_uptd').html(data.last_updated);
        osCancelStartDate(caseId);
        showTopErrSucc('success', _('Start date updated') + '.');
    }, 'json').fail(function () {
        showTopErrSucc('error', _('Could not set the start date.'));
        osCancelStartDate(caseId);
    });
}

/**
 * Estimated hours and due date, edited from the Timeline panel.
 *
 * Both delegate to the functions the header already uses — changeEstHour() and
 * detChangeDueDate() — rather than posting directly. That keeps the due-date
 * change-reason prompt intact, and both of those already write back to the
 * Timeline's own #est_time / #duedate_id, so the two copies never diverge.
 *
 * Spent hours has no editor by design: it is the sum of the task's time logs,
 * so the "+" opens the manual time-entry form (createlog) instead.
 */
function osEditEstHours(caseId) {
    var label = document.getElementById('est_time');
    var input = document.getElementById('os_esthr_input_' + caseId);
    if (!label || !input) return;
    label.style.display = 'none';
    input.style.display = '';
    input.focus();
...
function osCancelEstHours(caseId) {
    var label = document.getElementById('est_time');
    var input = document.getElementById('os_esthr_input_' + caseId);
    if (!label || !input) return;
    input.style.display = 'none';
...
function osEstHoursKey(ev, caseId) {
    if (ev.key === 'Escape') { osCancelEstHours(caseId); return; }
    ...
    // changeEstHour() ignores its `value` argument and reads the header's own
    // #est_hr<id> input, so the typed value has to land there or the save
    // silently persists whatever the header still held.
    var headerInput = document.getElementById('est_hr' + caseId);
    if (headerInput) {
        headerInput.value = value;
        changeEstHour(caseId, input.getAttribute('data-uniq'), input.getAttribute('data-cno'), value);
        return;
    }

    // No header editor on this task (permission or task type) — post directly
    // and repaint the two places that show the estimate.
    $.post(HTTP_ROOT + 'easycases/ajax_change_est_hour', {
        caseId: caseId,
        estHour: value
    }, function (data) {
        var seconds = value.indexOf(':') > 0
            ? ((parseInt(value.split(':')[0], 10) * 60) + parseInt(value.split(':')[1], 10)) * 60
            : value * 3600;
        if (label) label.textContent = seconds > 0 ? format_time_hr_min(seconds) : _('None');
        $('#est_hr_updated').html(seconds > 0 ? format_time_hr_min(seconds) : _('None'));
        if (data && data.last_updated) $('#lst_uptd').html(data.last_updated);
    }, 'json').fail(function () {
        showTopErrSucc('error', _('Could not save the estimated hours.'));
    });
}

function osEditDueDate(caseId) {
    var label = document.getElementById('duedate_id');
    var input = document.getElementById('os_duedate_input_' + caseId);
    if (!label || !input) return;
    label.style.display = 'none';
    input.style.display = '';
    input.focus();
...
function osCancelDueDate(caseId) {
    var label = document.getElementById('duedate_id');
    var input = document.getElementById('os_duedate_input_' + caseId);
    if (!label || !input) return;
    input.style.display = 'none';
...
function osSaveDueDate(caseId, isoDate) {
    var input = document.getElementById('os_duedate_input_' + caseId);
    if (!input) return;
    if (!isoDate) { osCancelDueDate(caseId); return; }

    var parts = isoDate.split('-');
    var posted = parts[1] + '/' + parts[2] + '/' + parts[0];
    var label = new Date(isoDate + 'T00:00:00')
        .toLocaleDateString(undefined, { day: 'numeric', month: 'short', year: 'numeric' });

    osCancelDueDate(caseId);
    detChangeDueDate(caseId, posted, label, input.getAttribute('data-uniq'), input.getAttribute('data-cno'));
}
