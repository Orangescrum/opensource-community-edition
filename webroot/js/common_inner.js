...
Date Created  : 29th March 2010(0329_2010)
*********************************************************************/
/************popup.js begin******/
var time;
...
function milestoneadd()
{
	...
	if(title.trim() == "")
	{
		msg = "'Task Group Title' cannot be left blank!";
...
function centerPopup(page,param1,param2,param3)
{
	...
	$.post(strURL,{"mstid":param1,"projid":param2,"countmanage":param3},function(data) {
	 if(data) {
			...
			$('#mstonename').text(decodeURIComponent((str + '').replace(/\+/g, '%20')));
...
function centerprPopup(page,uid,count,name)
{
	...
	$.post(strURL,{"uid":uid,'count':count},function(data) {
	 if(data) {
				...
				$('#usermanagename').text(decodeURIComponent(name.replace(/\+/g,' ')));
...
function showProjectName(name,id) {
	document.getElementById('projUpdateTop').textContent=decodeURIComponent(name);
	...
	if(page == "dashboard") {
		...
		if(value == "files"){
			...
			if((durl.indexOf('?case=') != -1) && (durl.indexOf('&project=') != -1)){
				...
				ajaxCaseView('case_project');
				document.getElementById('pageheading').innerHTML='Task Group';
			}
		}
		else{
			if((durl.indexOf('?case=') != -1) && (durl.indexOf('&project=') != -1)){
...
function view_project_menu(){
	...
	if($('#ajaxViewProjects').html() == ""){
		...
		if($('#ajaxViewProjects').is(':visible')){
			...
			$('#loader_prmenu').show();
			$.post(usrUrl+"users/projectMenu",{"page":"dashboard","limit":6,"filter":caseMenuFilters}, function(data)	{
				if(data) {
...
function view_import_project_menu(){
	...
	if($('#ajaxViewProjects').html() == ""){
		...
		if($('#ajaxViewProjects').is(':visible')){
			...
			$('#ajaxViewProjects').html('');
			$('#loader_prmenu').show();
			$.post(usrUrl+"users/projectMenu",{"page":"import","limit":6,"filter":caseMenuFilters}, function(data)	{
				if(data) {
					$('#ajaxViewProjects').html(data);
					$('#checkload').val('1');
					$('#loader_prmenu').hide();
   ...
   // Code added for reset filteration during switch project---- Start
	...
	if($('#reset_btn').is(":visible") && !$('#customFIlterId').val()){
		if(confirm('Do you want to reset the filters already active ?')){
			...
			$('#caseComment').val("all");  // Filter by Member
...
// Code added for reset filteration during switch project---- End
	if(all == '0'){
		$("#projUpdateTop").text(decodeURIComponent(pname));
		$('#projpopup').hide();
		var strurl = $('#pageurl').val();
		strURL = strurl+"easycases/";
		if(pname && (page!= "import")) {
			$('#pname_dashboard').text(decodeURIComponent(pname));
			$('.pname_dashboard').text(ucfirst(decodeURIComponent(pname)));
		}
		if(page == "dashboard"){
...
// by default; the heading is the toggle). Named function — not inline — because
// the comment box renders via the tmpl() micro template, which mangles inline
// JS containing quotes.
...
function milestonerestoreall(){
		...
		if(val.length!='0')
		{
			if(confirm("Are you sure you want to restore task group?"))
			{
				...
				$.post(url,{"val":val}, function(data){
				  if(data)
				  {
					showTopErrSucc('success','Task Group(s) have been restored.');
			...
			alert("Please select at least one task group to restore.");
...
function removeallmilestone(){
		...
		if(val.length!='0')
		{
				if(confirm("Are you sure you want to remove the task group?"))
				{
					document.getElementById('caseLoader').style.display="block";
					var pageurl = document.getElementById('pageurl').value;
					...
					$.post(url,{"val":val}, function(data){
					  if(data)
					{
						showTopErrSucc('success','Task Group have been removed.');
						var pageurl = document.getElementById('pageurl').value;
						var url = pageurl+"archives/milestone_list";
						$.post(url,{"pjid":pjid}, function(data){
					  		if(data) {
			...
			alert("Please select at least one task group to remove.");
		}
     }
	function removeall(){
		open_pop(this);
...
// ######### jyoti Archive section start #########
 function chgmile(pjid,pname){ //alert('hi');
		...
		document.getElementById('caseLoader').style.display="block";
          $('#pname_mile').text(decodeURIComponent(pname));
	...
	$('#search_project_menu_act').val('');
	document.getElementById('popup_option').style.display="none";
	$('#pname_activity').text(decodeURIComponent(pname));
	$('#prjid').val(pjid);
	...
	function chgcase(pjid,pname){
		...
		$('#ajaxbeforesrch').show();
          document.getElementById('popup_option').style.display="none";
		document.getElementById('caseLoader').style.display="block";
          $('#pname_case').text(decodeURIComponent(pname));
	...
	function chgfile(pjid,pname){ //alert(pname);
          document.getElementById('popup_option').style.display="none";
		document.getElementById('caseLoader').style.display="block";
          $('#pname_file').text(decodeURIComponent(pname));
		var pageurl = document.getElementById('pageurl').value;
		var url = pageurl+"archives/file_list";
					$.post(url,{"pjid":pjid,"casePage":1}, function(data){
				  		if(data) {
...
// ######### jyoti Archive section end #########
	...
	if(done == 0) {
		...
		if(type == 'bug'){
			...
			$('#piechart_container').load(url+'reports/hoursPiechart',{'pjid':pjid,'sdate':sdate,'edate':edate,'dtsearch':1});
			$('#grid_container').load(url+'reports/hoursGridview',{'pjid':pjid,'sdate':sdate,'edate':edate},function(res){
				if($('#thrs').length > 0){
			...
			$('#piechart_container').load(url+'reports/tasksPiechart',{'pjid':pjid,'sdate':sdate,'edate':edate});
			$('#statistic_container').load(url+'reports/tasksStatistics',{'pjid':pjid,'sdate':sdate,'edate':edate});
			$('#container').load(url+'reports/tasksTrend',{'pjid':pjid,'sdate':sdate,'edate':edate},function(res){
				document.getElementById('subprof1').style.display='block';
				document.getElementById('subprof2').style.display='none';
			});
	...
	function ajax_popup(obj){
		...
		if($('#ajaxViewProjects').html() == ""){
			...
			if($('#ajaxViewProjects').is(':visible')){
				...
				$.post(usrUrl+"users/projectMenu",{"page":"reports","limit":6,"page_name":pgname}, function(data){
					if(data) {
						$('#ajaxViewProjects').html(data);
						$('#checkload').val('1');
						document.getElementById('loader_prmenu').style.display= "none";
...
{
	var conf = confirm("Are you sure you want to delete task group '"+name+"' ?");
	if(conf == true) {
		var strURL = document.getElementById('pageurl').value;
		document.getElementById('caseLoader').style.display='block';
		$.post(strURL+"milestones/ajax_delete_milestone",{"uniqid":uniqid},function(data) {
			 //if(data == "success") {
				 showTopErrSucc('success','Task Group has been deleted.');
				 ajaxCaseView();
			 //}
		});
	}
	...
	if(DESK_NOTIFY && window.webkitNotifications) {
		...
		if (havePermission == 0) {
			...
			var notification = window.webkitNotifications.createNotification(
				HTTP_IMAGES+'logo/orangescrum-200-200.png',
				title,
				desc
			);
