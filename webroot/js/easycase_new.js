$(document).ready(function() {
    ...
    $(document).on("click", '[id^="titlehtml"],.titlehtml', function() {
        ...
        if (typeof params[0] != "undefined" && params[0] == "caselist") {
            ...
            if ($("#caseMenuFilters").length && $("#caseMenuFilters").val().trim() == "kanban" && !params[1]) {
        ...
        if (params[0] == "timelog" || params == "kanban" || params == "milestonelist" || (typeof params[0] != "undefined" && params[0] == "kanban" && typeof params[1] != "undefined") || params == "activesprint" || params == "backlogByStatus" || params == "backlog" || parseInt($("#subtask-container").height()) > 70 || (params[0] == "tasks" || params[0] == "taskgroup") || params[0] == "taskgroups" || PAGE_NAME == "resourceAvailability" || params[0] == "epics" || params[0] == "features" ) {
            $("#myModalDetail").modal();
...
function loadCaseMenu(strURL, params, ispageload) {
    ...
    params.caseCustomField = typeof localStorage["CUSTOM_FIELD"] != "undefined" ? localStorage["CUSTOM_FIELD"] : params.caseCustomField;
    ...
    $("#caseCustomField").val(params.caseCustomField);
        ...
        if (data) {
            try {
                if (parseInt(data.caseNewOrg) >= 0) {
                    const letter = data.caseNewOrg.toString();
                    const truncatedValue = letter.length > 2 ? letter.substring(0, 2) + '..' : letter;
                    $("#taskCnt,#kanban_taskCnt").html("(" + data.caseNewOrg + ")").show();
                    $("#taskCnt,#kanban_taskCnt").attr("title", data.caseNewOrg + _(" Tasks"));
                    $("#tskTabAllCnt,#kanban_tskTabAllCnt").html(data.caseNewOrg);
                    $("#tskTabAllCnt,#kanban_tskTabAllCnt").html(truncatedValue);
                    $("#tskTabAllCnt,#kanban_tskTabAllCnt").tipsy({
                        gravity: 's',
                        fade: true
                    }).attr("title", data.caseNewOrg);
                }
                if (parseInt(data.caseFiles) >= 0) {
                    // $("#fileCnt,#kanban_fileCnt").html(data.caseFiles).hide();
                    // $("#fileCnt,#kanban_fileCnt").attr("title", data.caseFiles + " File");
                }
                if (parseInt(data.assignToMeOrg) >= 0) {
                    $("#tskTabMyCnt,#kanban_tskTabMyCnt").html(data.assignToMeOrg);
                    $("#tskTabMyCnt,#kanban_tskTabMyCnt").tipsy({
                        gravity: 's',
                        fade: true
                    }).attr("title", data.assignToMeOrg);
                }
                if (parseInt(data.caseFavouriteOrg) >= 0) {
                    $("#tskTabFavCnt,#kanban_tskTabFavCnt").html(data.caseFavouriteOrg);

                    $("#tskTabFavCnt,#kanban_tskTabFavCnt").tipsy({
                        gravity: 's',
                        fade: true
                    }).attr("title", data.caseFavouriteOrg);

                }
                if (parseInt(data.delegateToOrg) >= 0) $("#tskTabDegCnt,#kanban_tskTabDegCnt").html(data.delegateToOrg);
                if (parseInt(data.highPriOrg) >= 0) {
                    $("#tskTabHPriCnt,#kanban_tskTabHPriCnt").html(data.highPriOrg);

                  $("#tskTabHPriCnt,#kanban_tskTabHPriCnt").tipsy({
                        gravity: 's',
                        fade: true
                    }).attr("title", data.highPriOrg);

                }
                if (parseInt(data.overdueOrg) >= 0) {
                    $("#tskTabOverdueCnt,#kanban_tskTabOverdueCnt").html(data.overdueOrg);
                    $("#tskTabOverdueCnt,#kanban_tskTabOverdueCnt").tipsy({
                        gravity: 's',
                        fade: true
                    }).attr("title", data.overdueOrg);
                }
                if (parseInt(data.openedtasksOrg) >= 0){
                    const letter = data.openedtasksOrg.toString();
                    const truncatedValue = letter.length > 2 ? letter.substring(0, 2) + '..' : letter;
                    $("#tskTabOpenedcnt,#kanban_tskTabOpenedcnt").html(data.openedtasksOrg);
                     $("#tskTabOpenedcnt,#kanban_tskTabOpenedcnt").html(truncatedValue);
                     $("#tskTabOpenedcnt,#kanban_tskTabOpenedcnt").tipsy({
                        gravity: 's',
                        fade: true
                    }).attr("title", data.openedtasksOrg);

                }
                if (parseInt(data.closedtasksOrg) >= 0) {
                    const letter = data.closedtasksOrg.toString();
                    const truncatedValue = letter.length > 2 ? letter.substring(0, 2) + '..' : letter;
                    $("#tskTabClosedCnt,#kanban_tskTabClosedCnt").html(data.closedtasksOrg);
                    $("#tskTabClosedCnt,#kanban_tskTabClosedCnt").html(truncatedValue);
                    $("#tskTabClosedCnt,#kanban_tskTabClosedCnt").tipsy({
                        gravity: 's',
                        fade: true
                    }).attr("title", data.closedtasksOrg);
                }
                if (parseInt(data.showDetails) == 1) {
                    currentUrl = getHash();
                    if ($("#caseMenuFilters").val() == "assigntome") $("#task_count_of").html(_("Displaying") + " <b>" + data.assignToMe + "</b> " + _("/") + " <b>" + data.caseNewOrg + "</b> " + _("tasks"));
                    else if ($("#caseMenuFilters").val() == "favourite") $("#task_count_of").html(_("Displaying") + " <b>" + data.caseFavourite + "</b> " + _("/") + " <b>" + data.caseNewOrg + "</b> " + _("tasks"));
                    else if ($("#caseMenuFilters").val() == "openedtasks") $("#task_count_of").html(_("Displaying") + " <b>" + data.openedtasks + "</b> " + _("/") + " <b>" + data.caseNewOrg + "</b> " + _("tasks"));
                    else if ($("#caseMenuFilters").val() == "overdue") $("#task_count_of").html(_("Displaying") + " <b>" + data.overdueOrg + "</b> " + _("/") + " <b>" + data.caseNewOrg + "</b> " + _("tasks"));
                    else if ($("#caseMenuFilters").val() == "delegateto") $("#task_count_of").html(_("Displaying") + " <b>" + data.delegateTo + "</b> " + _("/") + " <b>" + data.caseNewOrg + "</b> " + _("tasks"));
                    else if ($("#caseMenuFilters").val() == "highpriority") $("#task_count_of").html(_("Displaying") + " <b>" + data.highPri + "</b> " + _("/") + " <b>" + data.caseNewOrg + "</b> " + _("tasks"));
                    else if ($("#caseMenuFilters").val() == "closedtasks") $("#task_count_of").html(_("Displaying") + " <b>" + data.closedtasks + "</b> " + _("/") + " <b>" + data.caseNewOrg + "</b> " + _("tasks"));
                    else {
                        if ((data.caseNew != data.caseNewOrg || (typeof localStorage.SEARCHFILTER != "undefined" && $.trim(localStorage.SEARCHFILTER) != "")) && data.showDetailsAll != "1") {
                            $("#task_count_of").html(_("Displaying") + " <b>" + data.caseNew + "</b> " + _("/") + " <b>" + data.caseNewOrg + "</b> " + _("tasks"));
                        } else {
                            if (currentUrl.includes("features")) {
                                $("#task_count_of").html(_("Displaying all features"));
                            } else if (currentUrl.includes("epics")) {
                                $("#task_count_of").html(_("Displaying all epics"));
                            } else {
...
function resetBreadcrumbFilters(strURL, caseStatus1, caseCustomStatus1, priFil1, caseTypes1, caseMember1, caseComment1, caseAssignTo1, resetall1, case_date, case_due_date, casePage, caseSearch, clearCaseSearch, caseMenuFilters, milestoneIds, caseLabel, caseTaskgroup1) {
    ...
    var caseFeatures = (typeof localStorage['FEATURES'] != 'undefined') ? localStorage['FEATURES'] : $('#caseFeatures').val();
    var caseSkill = (typeof localStorage['SKILL'] != 'undefined') ? localStorage['SKILL'] : $('#caseSkill').val();
    var caseCustomField = (typeof localStorage['CUSTOM_FIELD'] != 'undefined') ? localStorage['CUSTOM_FIELD'] : $('#caseCustomField').val();
    $.post(strURL + "ajax_common_breadcrumb", {
        ...
        var $filtered_items_subtask = $('#filtered_items,#filtered_items_subtask');
        $filtered_items_subtask.show();
        if (data) {
            if (data.val) {
                $filtered_items_subtask.show();
            ...
            if ((getHash() == 'kanban' && parseInt($("#subtask-container").height()) < 70) || gethash.indexOf("tasks") != -1) {
                if (data.case_taskgroup != 'All' && (caseTaskgroup1 != 'All' && $.trim(caseTaskgroup1) != '')) {
                    $filtered_items_subtask.append(data.case_taskgroup);
                }
            }
            if (data.case_types != 'All' && (caseTypes1 != 'All' && $.trim(caseTypes1) != '')) {
                $filtered_items_subtask.append(data.case_types);
            }
            if (data.case_label != 'All' && (caseLabel != 'All' && $.trim(caseLabel) != '')) {
                $filtered_items_subtask.append(data.case_label);
            }
            if (data.case_custom_field && data.case_custom_field !== 'All' && caseCustomField && $.trim(caseCustomField) !== '') {
                $filtered_items_subtask.append(data.case_custom_field);
            }
            if (data.case_status != 'All' && (caseStatus1 != 'All' && $.trim(caseStatus1) != '')) {
                $filtered_items_subtask.append(data.case_status);
            }
            if (data.case_custom_status != 'All' && (caseCustomStatus1 != 'All' && $.trim(caseCustomStatus1) != '')) {
                $filtered_items_subtask.append(data.case_custom_status);
            }
            if (data.case_epics != 'All' && (caseEpics != 'All' && $.trim(caseEpics) != '')) {
                $filtered_items_subtask.append(data.case_epics);
            }
            if (data.case_features != 'All' && (caseFeatures != 'All' && $.trim(caseFeatures) != '')) {
                $filtered_items_subtask.append(data.case_features);
            }
            if (data.case_skill != 'All' && (caseSkill != 'All' && $.trim(caseSkill) != '')) {
                $filtered_items_subtask.append(data.case_skill);
            }
            if (data.date != 'Any Time' && (case_date != 'Any Time' && $.trim(case_date) != '')) {
                $filtered_items_subtask.append(data.date);
            }
            if (data.pri != 'All' && (priFil1 != 'All' && $.trim(priFil1) != '')) {
                $filtered_items_subtask.append(data.pri);
            }
            if (data.tasksortby) {
            ...
            if (data.search_case) {
                $filtered_items_subtask.append(data.case_search);
            ...
            clearSearchvis();
            if (data.page_case) {
                $filtered_items_subtask.append(data.case_page);
            }
            if (data.val) {
                $('#reset_btn').show();
            ...
            if (data.mlstn !== 'All') {
                $filtered_items_subtask.append(data.mlstn);
            }
            if (data.tskgrp != 'All') {
                $filtered_items_subtask.append(data.tskgrp);
            }
            $('[rel=tooltip]').tipsy({
                gravity: 's',
                fade: true
            ...
            if ($.trim($('#filtered_items').html()) == '') {
                if (CONTROLLER === "epics" && (PAGE_NAME === "index" || PAGE_NAME === "features")) {
                    toggleEpicBoardFilteredItems();
                }else{
...
function toggleEpicBoardFilteredItems() {
    if ($('#epic_board_filtered_items').children().length > 0) {
        $('#epic_board_filtered_item').css('display', 'inline-block');
    } else {
        $('#epic_board_filtered_item').css('display', 'none');
    }
}
...
function checkboxStatus(id, typ) {
    ...
    if (id == 'status_all') {
        ...
        if (isChecked) {
            if ($('#status_new').length) {
                ...
                document.getElementById('status_resolve').checked = true;
            }
            if ($('#status_file').length) {
                document.getElementById('status_file').checked = true;
                document.getElementById('status_upd').checked = true;
            }
            $("[id^=custom_status_]").prop('checked', true);
        } else {
    ...
    if (document.getElementById('status_new') && document.getElementById('status_new').checked == true) {
        x = 1 + "-";
    }
    if (document.getElementById('status_open') && document.getElementById('status_open').checked == true) {
        x = 2 + "-" + x;
    }
    if (document.getElementById('status_close') && document.getElementById('status_close').checked == true) {
        x = 3 + "-" + x;
    }
    if (document.getElementById('status_resolve') && document.getElementById('status_resolve').checked == true) {
        x = 5 + "-" + x;
    }
    if ($('#status_file').length) {
...
function syncStatusAllCheckbox() {
    var container = document.getElementById('dropdown_menu_status');

    var checkboxes = container.querySelectorAll(
        'input[type="checkbox"]:not(#status_all)'
    );
    ...
    document.getElementById('status_all').checked = allChecked;
}
...
/* todo optimize */
function checkboxTeamUsers(id, typ) {
    var x = "";
    var totid = $('#totTeamUserId').val();
    if (typ == "check") {
        if (document.getElementById(id).checked == true) {
            document.getElementById(id).checked = true;
        } else {
    ...
    for (var j = 1; j <= totid; j++) {
        var checkboxid = "TeamUsers_" + j;
        if (document.getElementById(checkboxid).checked == true) {
            var typeid = "TeamUserIds_" + j;
            var typevalue = $('#' + typeid).val();
            x = typevalue + "-" + x;
        }
    }
    ...
    $('#caseTeamUser').val(Asns);
    casePage = 1;
    remember_filters('TEAM_USERS', Asns);
}

function checkboxEpics(id, typ) {
    var x = "";
    var totid = $('#totEpics').val();
    $('#dropdown_menu_features').html('');
    if (id == 'epics_all') {
        document.getElementById(id).checked = true;
        for (var i = 1; i <= totid; i++) {
            var checkboxid = "Epic_" + i;
        ...
        if (document.getElementById('epics_all')) {
            document.getElementById('epics_all').checked = false;
        }
        if (typ == "check") {
            if (document.getElementById(id).checked == true) {
                document.getElementById(id).checked = true;
    ...
    $('.epics_type_cls').each(function() {
        var dt_id = $(this).attr('data-id');
        if ($("#" + this.id).is(':checked')) {
            var typeid = "Epicids_" + dt_id;
            var typevalue = $("#" + typeid).val();
            x = typevalue + "-" + x;
        }
    });
    if (x === "" || x === "-") {
        if (document.getElementById('epics_all')) {
            document.getElementById('epics_all').checked = true;
        }
        var epics = "all";
    } else {
        var epics = x.substring(0, x.length - 1);
    }
    $('#caseEpics').val(epics);
    casePage = 1;
    remember_filters('EPICS', epics);
}

function checkboxFeatures(id, typ) {
    var x = "";
    var totid = $('#totFeatures').val();
    if (id == 'features_all') {
        document.getElementById(id).checked = true;
        for (var i = 1; i <= totid; i++) {
            var checkboxid = "Feature_" + i;
            if (document.getElementById(checkboxid)) {
                document.getElementById(checkboxid).checked = false;
            }
        }
        ...
        if (document.getElementById('features_all')) {
            document.getElementById('features_all').checked = false;
        }
        if (typ == "check") {
            if (document.getElementById(id).checked == true) {
                document.getElementById(id).checked = true;
    ...
    $('.features_type_cls').each(function() {
        var dt_id = $(this).attr('data-id');
        if ($("#" + this.id).is(':checked')) {
            var typeid = "Featureids_" + dt_id;
            var typevalue = $("#" + typeid).val();
            x = typevalue + "-" + x;
        }
    });
    if (x === "" || x === "-") {
        if (document.getElementById('features_all')) {
            document.getElementById('features_all').checked = true;
        }
        var features = "all";
    } else {
        var features = x.substring(0, x.length - 1);
    }
    $('#caseFeatures').val(features);
    casePage = 1;
    remember_filters('FEATURES', features);
}

function checkboxSkill(id, typ) {
    var x = "";
    var totid = $('#totSkill').val();
    if (id == 'skill_all') {
        document.getElementById(id).checked = true;
        for (var i = 1; i <= totid; i++) {
            var checkboxid = "Skill_" + i;
            if (document.getElementById(checkboxid)) {
                document.getElementById(checkboxid).checked = false;
            }
        }
        ...
        if (document.getElementById('skill_all')) {
            document.getElementById('skill_all').checked = false;
        }
        if (typ == "check") {
            if (document.getElementById(id).checked == true) {
                document.getElementById(id).checked = true;
    ...
    $('.skill_type_cls').each(function() {
        var dt_id = $(this).attr('data-id');
        if ($("#" + this.id).is(':checked')) {
            var typeid = "Skillids_" + dt_id;
            var typevalue = $("#" + typeid).val();
            x = typevalue + "-" + x;
        }
    });
    if (x === "" || x === "-") {
        if (document.getElementById('skill_all')) {
            document.getElementById('skill_all').checked = true;
        }
        var skill = "all";
    } else {
        var skill = x.substring(0, x.length - 1);
    }
    $('#caseSkill').val(skill);
    casePage = 1;
    remember_filters('SKILL', skill);
}
...
function resetAllFilters(type) {
    ...
    try {
        ...
        $('#caseDateFil').val('');
        $('#casedueDateFil').val('');
        if(document.getElementById('status_all')) document.getElementById('status_all').checked = true;
        if(document.getElementById('status_new')) document.getElementById('status_new').checked = false;
        if(document.getElementById('status_open')) document.getElementById('status_open').checked = false;
        if(document.getElementById('status_close')) document.getElementById('status_close').checked = false;
        if(document.getElementById('status_resolve')) document.getElementById('status_resolve').checked = false;
        if(document.getElementById('status_upd')) document.getElementById('status_upd').checked = false;
        var totid = $('#totMemId').val();
        for (var i = 1; i <= totid; i++) {
        ...
        if(document.getElementById('priority_all')) document.getElementById('priority_all').checked = true;
        if(document.getElementById('priority_High')) document.getElementById('priority_High').checked = false;
        if(document.getElementById('priority_Medium')) document.getElementById('priority_Medium').checked = false;
        if(document.getElementById('priority_Low')) document.getElementById('priority_Low').checked = false;
        var totid = $('#totType').val();
        for (var i = 1; i <= totid; i++) {
            var checkboxid = "types_" + i;
            var elem = document.getElementById(checkboxid);
                    ...
                    if (getCookie('SUBTASKVIEW') == 'subtaskview') {
                        ...
                        if (parseInt($("#subtask-container").height()) > 70) {
                            ...
                            if (hashtag[0] == 'kanban') {
                                ...
                                easycase.refreshTaskList();
                            } else if (hashtag[0] == 'epics') {
                                $('#customFIlterId').val('');
                                window.location.hash = 'epics';
                                easycase.refreshTaskList();
                            } else if (hashtag[0] == 'features') {
                                $('#customFIlterId').val('');
...
function archive_common_breadcrumb(arcFilStatus, arcFilDate, arcFilProject, arcFilUser, arcFilDuedate, arcFilAssign, type) {
    var url = HTTP_ROOT + "archives/commonFilterDet";
    $.post(url, {
...
function createTaskTemplate(action) {
    ...
    if (tempTitle == '') {
        ...
        var url = pageurl + "templates/ajaxAddTaskTemplate";
        if (action == 'add') {
...
function createTemplate() {
    ...
    if (trim(tempTitle) == '') {
        ...
        $("#prj_btn").hide();
        var pageurl = HTTP_ROOT;
        var url = pageurl + "templates/ajaxAddTemplateModule";
        ...
        $.post(url, {
            ...
            if (response) {
                ...
                if (response.tmpl_id) {
                    closePopup();
                    showTopErrSucc('success', _('Project plan created successfully.'));
                    document.location.href = pageurl + "advanced-project-templates#/";
                } else if (response.error == 'fail' || response.error == 'duplicate') {
...
function addToProject(val, temp_name) {
    ...
    $('#inner_tmp_add').hide();
    $('.add-tmp-btn').hide();
    var strURL = HTTP_ROOT + "templates/addToProject";
    $.post(strURL, {
        "temp_id": val
    }, function(data) {
        if (data) {
...
function addTempToTask(val, temp_name, IsFromAddProject) {
    ...
    var strURL = HTTP_ROOT + "templates/addTemplate";
    $.post(strURL, {
        "temp_id": val,
        "temp_name": temp_name
    }, function(data) {
...
function save_edit_template() {
    ...
    $("#project_temp_err_edit").html('');
    var strURL = HTTP_ROOT + 'templates/ajaxTemplateEdit';
    if (temp_title.trim() != '') {
            ...
            $.post(strURL, {
                ...
                if (data) {
                    if (data.trim() == 'fail') {
                        ...
                        $("#project_temp_err_edit").html(_("This Project template already exists."));
                        return false;
                        ...
                        $("#prjtemploader_edit").hide();
                        $("#project_temp_err_edit").show();
                        $("#project_temp_err_edit").html(_("Invalid project template name. HTML tags are not allowed."));
                        return false;
                    } else {
                        if ((pageNum && parseInt(pageNum) == 1) || (pageNum == '')) {
                            document.location.href = HTTP_ROOT + 'advanced-project-templates#/';
                        } else {
                            document.location.href = HTTP_ROOT + 'advanced-project-templates#/?page=' + pageNum;
                        }
                        showTopErrSucc('success', _("Project template updated successfully."));
                        $("#prjtemploader_edit").hide();
                    }
                }
            });
        ...
        $("#project_temp_err_edit").html(_("Project template name can't be blank."));
        return false;
    }
}
...
function add_cases_project() {
    ...
    if (done == 0) {
        ...
        $.post(strURL + "ajaxAddTemplateCases", {
            "pj_id": pj_id,
...
function getkanbanFilters() {
    ...
    $.post(HTTP_ROOT + 'Boards/kanbanFilters', {
...
function importTasksTemplate(val, temp_name, IsFromAddProject) {
    openPopup();
    if (!temp_name) {
        temp_name = $('#textTemp_' + val).text();
    }
    $(".import_prod_temp_name").html(_('Import tasks to') + ' "' + temp_name + '" ' + _('project template'));
    $("#template_id").val(val);
    $("#template_name").val(temp_name);
    $(".import_task_to_project_template").show();
    $("#IsFromAddProject").val(IsFromAddProject);
    $('#inner_tmp_add').hide();
    $(".loader_dv").hide();
}

let isAnyCheckboxChecked = (checkboxIds) => {
    ...
    // If none of the checkboxes are checked, return false
    return false;
};


let applyUnvFilters = (type) => {
    if(type == "type"){
        let typeIds = new Array();
        $('[id^=types_]').each(function( index ) {
            if($(this).attr('id') != "types_all"){
                typeIds.push($(this).attr('id'));
            }
        });
        if(isAnyCheckboxChecked(typeIds)){
            easycase.refreshTaskList();
        }
        easycase.refreshTaskList();
    }else if(type == "cstatus"){
        let cStstusIds = new Array();
        $('[id^=custom_status_]').each(function( index ) {
            cStstusIds.push($(this).attr('id'));
        });
        if(isAnyCheckboxChecked(cStstusIds)){}
        easycase.refreshTaskList();
    }else if(type == "priority"){
        let prtyIds = ['priority_Low', 'priority_Medium', 'priority_High'];
        $('[id^=custom_status_]').each(function( index ) {
            prtyIds.push($(this).attr('id'));
        });
        if(isAnyCheckboxChecked(prtyIds)){
            easycase.refreshTaskList();
        }
    }else if(type == "cmntby"){
        let cmntByIds = new Array();
        $('[id^=coms_]').each(function( index ) {
            cmntByIds.push($(this).attr('id'));
        });
        if(isAnyCheckboxChecked(cmntByIds)){
            easycase.refreshTaskList();
        }
    }else if(type == "tg"){
        let tgIds = new Array();
        $('[id^=groups_]').each(function( index ) {
            tgIds.push($(this).attr('id'));
        });
        if(isAnyCheckboxChecked(tgIds)){
            easycase.refreshTaskList();
        }
    }else if(type == "crtdby"){
        let crtIds = new Array();
        $('[id^=mems_]').each(function( index ) {
            crtIds.push($(this).attr('id'));
        });
        if(isAnyCheckboxChecked(crtIds)){
            easycase.refreshTaskList();
        }
    }else if(type == "agnto"){
        let agntoIds = new Array();
        $('[id^=Asns_]').each(function( index ) {
            agntoIds.push($(this).attr('id'));
        });
        if(isAnyCheckboxChecked(agntoIds)){
            easycase.refreshTaskList();
        }
    }else if(type == "status"){
        easycase.refreshTaskList();
        ...
        if(isAnyCheckboxChecked(labelIds)){
            easycase.refreshTaskList();
        }
