<?php if ($project_list) { ?>
    <div class="scrl-ovr">
        <table cellpadding="0" cellspacing="0" class="table table-striped table-hover" style="border-collapse: separate;">
            <thead>
                <tr class="head-row hd-new">
                    <th>
            ...
            <tbody>
                ...
                foreach ($project_list as $prj_lst) {
                    $project_name = $prj_lst['name'];
                    $project_id = $prj_lst['id'];
                    $cscount++;
                    if ($cscount % 2 == 0) {
                        $class = "row_col";
                    } else {
                    ...
                    <tr id="listing<?php echo $cscount; ?>" class="tr_all rw-cls <?php echo $class; ?>">
                        <td>
                            <div class="fl checkbox">
                                <label>
                                    <input type="checkbox" class="chkbx_cur ad-usr-prj removePrjFromuser" id="actionChk<?php echo $cscount; ?>" value="<?php echo $project_id; ?>" data-prj-name="<?php echo trim($prj_lst['name']); ?>" />
                                </label>
                            </div>
                            <input type="hidden" id="actionCls<?php echo $cscount; ?>" value="0" />
                        </td>
                        <td>
                            <div class="rem_proj_usr ellipsis-view" title="<?php echo $prj_lst['name']; ?>">
                                <?php echo $prj_lst['name']; ?>
                            </div>
                        </td>
                        <td>
                            <?php echo $prj_lst['short_name']; ?>
                        </td>
                        <?php if (!$is_invite_user) { ?>
                            <td>
                                <?php
