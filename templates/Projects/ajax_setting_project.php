...
<div class="modal-body popup-container">
    ...
    <?php echo $this->Form->create(null, array('url' => '/projects/ajax_setting_project', 'id' => 'projsettings', 'name' => 'projsettings', 'enctype' => 'multipart/form-data')); ?>
    <input type="hidden" id="ProjectSettingId" value="<?php echo $projArr['id'] ?? '' ?>" name="data[ProjectSetting][id]" />
        ...
        <div class="col-lg-12 padlft-non padrht-non">
            <div class="col-lg-12 col-sm-12">
                <div class="select2__wrapper" id="setting_dropdown">
                    <?php $velocity_reports = $projArr['velocity_reports'] ?? null; ?>
                    <select name="data[ProjectSetting][velocity_reports]" class="form-control floating-label proj_velocity" placeholder="<?php echo __('Estimation Statistic'); ?>" data-dynamic-opts=true id="ProjectSettingVelocityReports">
                        <option value='0' <?php if ($velocity_reports == 0) { ?>selected <?php } ?>><?php echo __('Story Points'); ?></option>
                        <option value='1' <?php if ($velocity_reports == 1) { ?>selected <?php } ?>><?php echo __('Original Time Estimate'); ?></option>
                        <option value='2' <?php if ($velocity_reports == 2) { ?>selected <?php } ?>><?php echo __('Task Count'); ?></option>
                    </select>
                </div>
            </div>
        </div>
    ...
    <div class="modal-footer">
        <div class="fr popup-btn">
            ...
            <span id="btn-setting-project" class="project_edit_button">
