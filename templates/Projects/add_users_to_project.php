...
<div class="modal-body popup-container" style="padding-bottom:0px;">
    <div class="row">
        <div class="col-lg-12">
            <ul>
                <li class="nav_tab_both active" <?php if(SES_TYPE < 3) { ?> onclick="asign_existing_users(this); <?php } ?>">
            ...
            <div class="col-lg-12">
                ...
                <div class="col-lg-4 col-sm-4 col-xs-4 padrht-non">
                        <div class="form-group label-floating">
												<label class="control-label find-user fins-new-label" for="srch_usr_to_assgn"><?php echo __('Find User');?></label>
				...
				<div style="min-height:50px;">
					...
					<?php
                        foreach ($allProjUsers as $ak => $av) {
                            if(!isset($allUsers[$av]))
                                continue;
                            $name = $allUsers[$av]['name'];
                            if (trim($allUsers[$av]['name']) == '') {
                                $at_name = explode('@', $allUsers[$av]['email']);
                                $name = $at_name[0];
                            }
                            $place_holder = '';
                            if ($allUsers[$av]['CompanyUsers']['is_client']) {
                                $place_holder = '<small class="green-txt">'.__('Client',true).'</small>';
                            } else if ($allUsers[$av]['CompanyUsers']['user_type'] == 1) {
                                $place_holder = '<small class="green-txt">'.__('Owner',true).'</small>';
                            } else if ($allUsers[$av]['CompanyUsers']['user_type'] == 2) {
                            ...
                            <div class="checkbox custom-checkbox add-user-pro-chk">
                                <label for="add_user_proj_<?php echo $allUsers[$av]['id']; ?>" title11="<?php #echo $allUsers[$av]['email']; ?>">
                                    <input id="add_user_proj_<?php echo $allUsers[$av]['id']; ?>" type="checkbox" value="<?php echo $allUsers[$av]['id']; ?>" class="add_user_chk crntly_asgnd_usrs" <?php echo $checked; ?>/>
                                    <span class="oya-blk">
                                       <span class="assign_usr_chk" title="<?php echo $allUsers[$av]['email']; ?><?php if($allUsers[$av]['CompanyUsers']['is_active']==0){ echo " \n (".__('Disabled').")";}?>" rel="tooltip" <?php if($allUsers[$av]['CompanyUsers']['is_active']==0){ echo "style='color:#a59b9b;text-decoration:line-through;'"; }?> >
                ...
                <div class="custom_scroll" style="height:200px;">
					<?php if ($allUsers) { ?>
                        ...
                        foreach ($allUsers as $k => $v) {
                            ...
                            $place_holder = '';
                            if ($v['CompanyUsers']['is_client']) {
                                $place_holder = '<small class="green-txt">'.__('Client',true).'</small>';
                            } else if ($v['CompanyUsers']['user_type'] == 1) {
                                $place_holder = '<small class="green-txt">'.__("Owner",true).'</small>';
                            } else if ($v['CompanyUsers']['user_type'] == 2) {
                                $place_holder = '<small class="green-txt">'.__('Admin',true).'</small>';
                            }else{
                                $place_holder = '<small class="green-txt">&nbsp;</small>';
                            }

                            $checked = (in_array($v['id'], $allProjUsers)) ? 'checked="checked"' : "";
                            ?>
                            <div class="checkbox custom-checkbox add-user-pro-chk">
                                <label for="add_user_proj_<?php echo $v['id']; ?>" title11="<?php #echo $v['email']; ?>">
                                    <input id="add_user_proj_<?php echo $v['id']; ?>" type="checkbox" value="<?php echo $v['id']; ?>" class="add_user_chk usr_to_b_asgnd" <?php echo $checked; ?>/>
                                    <span class="oya-blk">
                                       <span class="assign_usr_chk" title="<?php echo $v['email']; ?><?php #echo trim($name);?>" rel="tooltip">
                                           <?php echo $this->Text->truncate($name,17,array('ellipsis' => '...','exact' => true)); ?>
                                        </span>
                                       <?php echo $place_holder; ?>
                                    </span>
...
function assignUserToPrj() {
    if ($('.act_btttn').find('.btnassignUserToPrj').hasClass('loginactive')) {
        ...
        var user_ids = $('.add_user_chk:checked').map(function() {
            return $(this).val();
        }).get().join(',');
...
function invite_asign_users(obj){
    ...
    $('.addMember_popup').html(_('Invite'));
    $('.project_to_be_assn').hide();


    $("#select_team").val('').select2();
    ...
    $("#txt_email").focus();
    $('.auto_tab_fld').html('<input type="hidden" name="data[User][pid]" class="form-control" value="'+$('#Pjid').val()+'"/><input style="width: 352px;" type"text" readonly="readonly" class="form-control" value="'+$('#Pjname').val()+'"/><a href="javascript:void(0);" onclick="changeOnInvite();" style="font-size:11px;font-weight:bold;color:#5F9AC2;">'+_('Change project')+'</a>');
