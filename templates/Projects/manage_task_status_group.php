<?php $err_sts = '';
if($session->check('ERROR') && trim($session->read("ERROR")) != ''){ $err_sts = $session->write("ERROR",''); ?>
<?php } ?>
<div class="slide_rht_con">
    <div class="task_listing timelog_lview  hidetablelog timelog-detail-tbl status_work_flow_page mng_sts_page">
        <div class="tlog_top_cnt">
            <div class="col-lg-5 padlft-non">
                <div class="task-list-bar wf_page_nav_tabs"> <!---->
                    <ul class="proj_stas_bar">
                        <li class="<?php if(!isset($params[0]) || $params[0] !='project'){ ?> active-list <?php } ?> " ><a href="<?php echo HTTP_ROOT.'workflow-setting'; ?>" style="max-width: 160px"><?php echo __('Master Status Workflow');?></a></li>
                        <li  <?php if(isset($params[0]) && $params[0] == 'project' ){ ?> class="active-list" <?php } ?>><a href="<?php echo HTTP_ROOT.'workflow-setting/project'; ?>" style="max-width: 160px"><?php echo __('Project Status Workflow');?></a></li>
                    </ul>
                </div>
            ...
            <div class="col-lg-2 padnon btn_tlog_top mtop10">
                <?php // Creating a status workflow is an Enterprise feature. The
                      // predefined workflows below stay available and usable. ?>
            </div>
            <div class="cb"></div>
        </div>
        <div class="m-cmn-flow cmn-overflow-list" style="overflow: auto;width: 100%;;">
            <table class="table table-striped table-hover m-list-tbl" style="table-layout: fixed; border-collapse: separate;">
                <tbody>
                <tr class="hd-new"> 
                ...
                <?php if(count($result) > 0 || count($dflt_stsarr)> 0){
                    if(count($dflt_stsarr) > 0){
                        foreach($dflt_stsarr as $kd=>$vd){ ?>
                            <tr>
                                <td>
                                    <a style="color:#2d6dc4;" href="<?php echo HTTP_ROOT.'status-setting/'.base64_encode($vd['id'])?>"><?php echo $vd['name'];?></a>
                                </td>
                                <td><?php echo count($vd['custom_statuses']);?></td>
                    ...
                    if(count($result) > 0){
                        foreach($result as $k=>$v){ ?>
                            <tr>
                                <td><a style="color:#2d6dc4;" href="<?php echo HTTP_ROOT.'status-setting/'.base64_encode($v['id'])?>"><?php echo $v['name'];?></a></td>
                                <td><?php echo count($v['custom_statuses']);?></td>
                                <td><?php echo $v['description'];?></td>
                                <td><?php echo date('d M, Y',strtotime($v['modified']));?></td>
                                <td class="action_section">
                                    <a href="javascript:void(0)" onclick="editStatusGroup(<?php echo $v['id'];?>)" title="<?php echo __('Edit');?>"><i class="material-icons">edit</i></a>
                                    <?php if(count($v['projects']) == 0){ ?>
                                        <a href="javascript:void(0)" onclick="deleteStatusGroup(<?php echo $v['id'];?>)" title="<?php echo __('Delete');?>"><i class="material-icons">delete_outline</i></a>
                                    <?php } ?>
                                </td>
                            </tr>
                        <?php } } }else{ ?>
