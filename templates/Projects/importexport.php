<style type="text/css">
    ...
    .import_project_div #ajaxViewProjects{width: 100%;overflow: hidden;}
    .import_project_div #projpopup{width: 75%;}
</style>
<div class="user_profile_con setting_wrapper task_listing import-export-page cmn_tbl_widspace width_hover_tbl">
    <!--Tabs section starts -->
    <?php echo $this->element('import_page_tabs', ['mode' => 'importexport']); ?>
    ...
    <div class="row exp_innerdiv" id="imploade_file"  style="<?php if (isset($fileds)) { echo "display:none;"; } ?>"  >
            ...
            <div class="import-info-dif import_proj">
                <div class="chk_content">
                    ...
                    <div class="download_samplefile">
                        <a href="<?php echo HTTP_ROOT; ?>projects/downloadSampleCsvFile"><?php echo __('Download the sample file');?></a> <?php echo __('to see what you can import');?>
                    </div>
                    <ul class="chk_desc">
                    ...
                    <ul class="chk_desc">
                        <li><?php echo __('Choose a project to Import Task');?> <br />
                        <span><?php echo __('NOTE');?> :-</span> 
                        <br /><?php echo __('We can upload more one project if we select All project from project dropdown');?></li>
        ...
        <?php if (isset($fileds)) { ?>
            ...
            <div class="imp_data_outer mbtm30 weekly_btm_sumry imp_task">
                ...
                <div class="data-import-err">
                    ...
                    <p class="text-success" style="<?php echo !empty($tasks) ? 'color:red;' : 'color:green;'; ?>">
                                ...
                                <?php foreach ($fileds as $hk => $hv) { ?>
                                    <?php if (isset($v[strtolower($hv)])) { ?>
                                        <td <?php if ($error_arr[$k][strtolower($hv)]) {
                                            ...
                                            <?php
                                            if (in_array(strtolower($hv), ['taskgroup', 'title', 'description'])) {
                ...
                <td colspan="2">
                    <table>
                        ...
                        <?php if (isset($non_create_projects) && !empty($non_create_projects)) { ?>
                            <tr>
                                <td colspan="2" >
                                    <span class="upld-sum-lebel"><?php echo __("Project(s) can't Imported");?> :&nbsp;</span>
                                    <b><?php echo!empty($non_create_projects) ? $non_create_projects : 'NA'; ?></b>
                                </td>
                            </tr>
                        <?php } ?>
...
<script type="text/javascript">
    ...
    function check_csvfile() {
        ...
        if ($('#import_csv').val()) {
            ...
            if (ext == 'csv' || ext == 'CSV') {
                ...
                data.append('project_id', $('#proj_id').val());
    ...
    function deleteCsvFile(file) {
        var strURL = '<?php echo $this->Url->build(['controller' => 'ProjectImports', 'action' => 'deleteCsvFile'], ['fullBase' => true]); ?>';
    ...
    function call_validation() {
        ...
        if (pro_id == 'all') {
            var url = '<?php echo $this->Url->build(['controller' => 'ProjectImports', 'action' => 'checkfileCsvValidation'], ['fullBase' => true]); ?>';
            $('#loader_img_csv').show();
            $.ajax({
                url: url,
                data: form,
    ...
    function check_multiple_project() {
        ...
        var form = new FormData($('#data_import_form')[0]);
        var url = '<?php echo $this->Url->build(['controller' => 'ProjectImports', 'action' => 'checkMultipleProject'], ['fullBase' => true]); ?>';
        $.ajax({
            url: url,
            data: form,
            cache: false,
    ...
    $(function () {
        ...
        var action = "<?php echo PAGE_NAME; ?>";
        if (cont == 'projects' && action == 'confirmImport') {
            var valid_val = "<?php echo!empty($total_valid_rows) ? $total_valid_rows : 0; ?>";
            if (valid_val != 0) {
                showTopErrSucc('success', "<?php echo __('Successfully imported');?>.");
            } else {
                var msg = "<?php echo __('Invalid CSV file');?>, <a href='" + csvurl+"' style='text-decoration:underline;color:#0000FF'><?php echo __('Download');?></a> <?php echo __('and check with our sample file');?>.";
