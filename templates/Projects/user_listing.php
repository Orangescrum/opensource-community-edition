    ...
    <?php if (!empty($memsExtArr['Member']) || !empty($memsExtArr['Invited']) || !empty($memsExtArr['Disabled'])) { ?>
        ...
        <table cellpadding="0" cellspacing="0" class="table table-striped table-hover">
            <thead>
                <tr class="head-row">
                    <th>
                        <div class="fl checkbox"><label><input type="checkbox" value="1" class="chkbx_cur" onclick="selectremuserAll(1, 0)" id="remcheckAll" /></label></div>
                    </th>
            ...
            <tbody>
                ...
                <?php if (!empty($memsExtArr['Member'])) { ?>
                    <?php foreach ($memsExtArr['Member'] as $v1) { ?>
                        ...
                        $user_email = $v1['email'];
                        $user_name = $v1['name'];
                        $user_shortName = $v1['short_name'];
                        $user_istype = $v1['istype'];
                        $cscount++;
                        ?>
                        <tr id="listing<?php echo $cscount; ?>" class="rw-cls 12346">
                            <td class="td-1st <?php if ($v1['CompanyUsers']['user_type'] == 1 && SES_TYPE != 1) { ?>disabletoptab<?php } ?>">
                                <div class="fl checkbox <?php if ($v1['CompanyUsers']['user_type'] == 1 && SES_TYPE != 1) { ?>custom-checkbox<?php } ?>">
                                    <label>
                                        <?php if ($v1['CompanyUsers']['user_type'] == 1 && SES_TYPE != 1) { ?>
                            ...
                            <td class="td-2nd">
                                ...
                                $usr_typ_name = '';
                                $colors = '';
                                if ($v1['CompanyUsers']['user_type'] == 1) {
                                    $colors = 'color:Green';
                                    $usr_typ_name = __('Owner');
                                } else if ($v1['CompanyUsers']['user_type'] == 2) {
                                    $colors = 'color:Red';
                                    $usr_typ_name = __('Admin');
                                }

                                if ($v1['CompanyUsers']['is_active'] == 0) {
                                    $colors = 'color:Blue';
                                    $usr_typ_name = __('Invited');
                                }
                                ?>
                            ...
                            <td class="td-5th" style="width:20%;">
                                <div class="togglebutton">
                                    <label class="checkedonofflbl mrg0">
                                        <?php
                                        if ($v1['ProjectUsers']['default_email'] == 0) {
                                            $checked = "";
                                            $checkedlbl = "OFF";
                                        } else {
                                        ...
                                        ?>
                                        <input class="checkedonoff" type="checkbox" onchange="changeUserNotificatioSetting(this, '<?php echo $v1['ProjectUsers']['id'] ?>')" <?php echo $checked; ?>>
                ...
                <?php if (!empty($memsExtArr['Disabled']) || !empty($memsExtArr['Invited'])) { ?>
                    ...
                    foreach ($memsExtArr['Disabled'] as $v2) {
                        ...
                        $user_email = $v2['email'];
                        $user_name = $v2['name'];
                        $user_shortName = $v2['short_name'];
                        $user_istype = $v2['istype'];
                        $cscount++;
                    ?>
                        <tr id="disabledlist<?php echo $cscount; ?>" class="disable-cls">
                            ...
                            <td class="td-5th">
                                <?php
                                $usr_typ_name = '';
                                if ($v2['CompanyUsers']['is_active'] == 0) {
                                    $colors = 'color:#DD4D4B';
                    ...
                    ?>
                        <?php
                        if ($v['UserInvitations']['is_active'] == 1) {
                            $colors = 'color:Blue';
                            $usr_typ_name = __('Invited');
                        }
                        ?>
