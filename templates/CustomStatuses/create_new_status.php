    ...
    <div class="modal-body status_custom_cont">
        <div id="inner_label">
            ...
            <div class="form-group label-floating">
                ...
                <input type="text"  class="form-control" name="data[CustomStatus][name]" value="<?php echo $res['name'] ?? '';?>" id="custom_statuses_name" placeholder="" maxlength="35" />
                <input type="hidden" class="form-control" value="<?php echo $res['id'] ?? '';?>" name="data[CustomStatus][id]" id="custom_statuses_id" placeholder="" />
            </div>
            <div class="form-group label-floating">
                ...
                <select name="data[CustomStatus][status_master_id]" class="select2" id="custom_statuses_map">
                    <?php foreach ($statusMaster as $k => $v) { ?>
                        <option value="<?php echo $k; ?>" <?php if (isset($res['status_master_id']) && $res['status_master_id'] == $k) {
                               echo 'selected="selected"';
                           } else if (!isset($res['status_master_id']) && $k == 2) {
                               echo 'selected="selected"';
                           } ?> <?php if (isset($res['status_master_id']) && $res['status_master_id'] == 3 && $k != 3) {
            ...
            <div class="form-group label-floating">
                ...
                <select name="data[CustomStatus][progress]" class="select2" id="custom_statuses_progress">
                    <?php for($i=0; $i<= 100; $i=$i+10){ ?>
                        <option value="<?php echo $i; ?>" <?php if(isset($res['progress']) && $res['progress'] ==$i ) { echo 'selected="selected"'; } ?> <?php if(isset($res['status_master_id']) && $res['status_master_id'] == 3 && $i != 100){ echo 'disabled="disabled"'; }?>><?php echo $i; ?></option>
                    <?php } ?>
                </select>
            </div>
            <div class="form-group label-floating">
                <label class="control-label" for="label_nm"><?php echo __('Color');?></label>
                <input type="hidden" name="data[CustomStatus][color]" value="<?php if(isset($res['color'])){ echo $res['color']; }else{ ?>0000ff<?php } ?>" id="custom_task_color" />
                <div id="colorSelector" style="top:10px;"><div style="background-color: #<?php if(isset($res['color'])){ echo $res['color']; }else{ ?>0000ff<?php } ?>"></div></div>
            </div>
        </div>
    </div>
    <div class="modal-footer">
        <div class="fr popup-btn">
            <span id="create_wloader1" style="display:none;">
                <img src="<?= $this->Url->build('/img/images/case_loader2.gif') ?>" alt="Loader"/>
            </span>
            <span id="wfbtn1">
                <?php if(!isset($res['id'])) { ?>
                    <span class="fl">
                ...
                <span class="fl hover-pop-btn">
                    ...
                    <input type="button" id="newworkflow_btn" value="<?php if(isset($res['id']) && !empty($res['id'])){ echo __('Update'); }else{ echo __('Add'); } ?>" name="add" class="btn btn_cmn_efect cmn_bg btn-info cmn_size"  <?php if($from_page != ''){ ?>
                        onclick="saveKanbanNewSts('<?php echo base64_encode($wid);?>');"
                    <?php } else { ?>
                        onclick="addKanbanNewSts()"
                    <?php } ?> />
                </span>
            </span>
...
<script>
    $(document).ready(function(){
        $("#custom_statuses_progress, #custom_statuses_map").select2();
        $('#colorSelector').ColorPicker({
            color: '#<?php if(isset($res['color'])){ echo $res['color']; }else{ ?>2c41c9<?php } ?>',
    ...
    function saveKanbanNewSts(id){
        ...
        $.ajax({
            url: url + 'projects/ajaxSaveNewstatusKanban',
            type: 'POST',
            ...
            success: function (res) {
                if(res.status == 'success'){
                    ...
                    showTopErrSucc('error', _('Please Enter Status Name'));
                }
            },
            error: function() {
