			...
			<div class="project_not_contain">
				<div class="proj_note_dtl">
					<div class="project_note_profile">
						<div class="prof_sett">
							<?php if (!empty($note['Users']['photo'])) { ?>
								<img src="<?php echo HTTP_ROOT; ?>users/image_thumb/?type=photos&file=<?php echo $note['Users']['photo']; ?>&sizex=28&sizey=28&quality=100"
									class=" round_profile_img" height="28" width="28" />
							<?php } else {
								$random_bgclr = $this->Format->getProfileBgColr($note['Users']['id']);
								$usr_name_fst = mb_substr(trim($note['Users']['name'] . ' ' . $note['Users']['last_name']), 0, 1, "utf-8");
								?>
					...
					<div class="project_note_created">
						...
						$sh_txt = __('Created by') . ' <strong>' . $note['Users']['name'] . ' ' . $note['Users']['last_name'] . '</strong> ' . __('on');
						if ($note['is_updated']) {
							$sh_txt = __('Updated by') . ' <strong>' . $note['Users']['name'] . ' ' . $note['Users']['last_name'] . '</strong> ' . __('on');
							$locDT1 = $this->Tmzone->GetDateTime(SES_TIMEZONE, TZ_GMT, TZ_DST, TZ_CODE, $note['created'], "datetime");
							$created_on = $this->Datetime->facebook_style_date_time($locDT1, $curDateTz);
						} else {
							$locDT1 = $this->Tmzone->GetDateTime(SES_TIMEZONE, TZ_GMT, TZ_DST, TZ_CODE, $note['modified'], "datetime");
							$created_on = $this->Datetime->facebook_style_date_time($locDT1, $curDateTz);
						}
					...
					<?php if ($note['Users']['id'] == $ses_id) { ?>
						<span class="edit_delete_note">
							<span id="note_edt_id" data-uid="<?php echo $note['uniq_id']; ?>"
								onclick="inlineNoteEdit(this);" class="inline-edit-not anchor" data-prj-id="" data-prj-name=""
								title="<?php echo __('Edit Note'); ?>" rel="tooltip">
								<i class="material-icons">&#xE254;</i>
							</span>

							<span id="note_del_id" data-uid="<?php echo $note['uniq_id']; ?>"
								onclick="inlineNoteDelete(this);" class="inline-edit-not anchor" data-prj-id="" data-prj-name=""
								title="<?php echo __('Delete Note'); ?>" rel="tooltip">
								<i class="material-icons">&#xE872;</i>
							</span>
						</span>
					<?php } ?>
					<div class="project_note_title" id="not<?php echo $note['uniq_id']; ?>">
						<span id="not_title<?php echo $note['uniq_id']; ?>">
							<?php
							$caseMsgRep = $this->Format->formatCms($note['note']);
