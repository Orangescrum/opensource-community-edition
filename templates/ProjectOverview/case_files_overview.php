        ...
        <table>
            <tbody>
                ...
                ?>
                    ...
                    <?php } ?>
                    <tr id="curRow<?php echo $value['CaseFiles']['id']; ?>">
                                ...
                                <span class="dropdown">
                                    ...
                                    <ul class="dropdown-menu">
                                        <?php if ($this->Format->isAllowed('Archive File', $roleAccess)) { ?>
                                            <li><a onClick="archiveFile(this)" data-id="<?php echo $value['CaseFiles']['id']; ?>" data-name="<?php echo $value['file_name']; ?>" href="javascript:void(0);"><i class="material-icons">&#xE149;</i> <?php echo __('Archive'); ?></a></li>
                                        <?php } ?>
                                        <?php if ($this->Format->isAllowed('Delete File', $roleAccess)) { ?>
                                            <li><a onclick="removeFileFrmFiles(<?php echo $value['CaseFiles']['id']; ?>)" data-name="<?php echo $value['file_name']; ?>" id="file_remove_<?php echo $value['CaseFiles']['id']; ?>" href="javascript:void(0);"><i class="material-icons">&#xE15C;</i> <?php echo __('Remove'); ?></a></li>
                                        <?php } ?>
                                    </ul>
                                </span>
                            <?php } ?>
                        </td>
                        <td><a href="<?php echo HTTP_ROOT . 'dashboard#details/' . $value['uniq_id']; ?>">#<?php echo $value['case_no']; ?> </a></td>
                        <td>
                        ...
                        <td>
                            <div class="user_name">
                                <span class="pfl_img overview-img">
                                    <span rel="tooltip" title="<?php echo ucfirst($value['usrName']); ?>">
                                        <?php if ($value['usrPhoto']) { ?>
                                            <img src="<?php echo HTTP_ROOT; ?>users/image_thumb/?type=photos&file=<?php echo $value['usrPhoto']; ?>&sizex=30&sizey=30&quality=100" title="<?php echo $value['usrName']; ?>" rel="tooltip" alt="<?php echo __('Loading'); ?>" />
                                        <?php } else {
                                            $random_bgclr = $this->Format->getProfileBgColr($value['CaseFiles']['user_id']);
                        ...
                        <td>
                            <?php if ($this->Format->isAllowed('Download File', $roleAccess)) { ?>
                                <?php if ($value['download_url'] != '') { ?>
                                    <a href="<?php echo $value['download_url']; ?>" data-toggler="tooltip" title="<?php echo __('Download'); ?>" data-url="<?php echo $value['download_url']; ?>" target="_blank">
                                    <?php } else { ?>
                                        <a href="javascript:void(0);" data-toggler="tooltip" title="<?php echo __('Download'); ?>" data-url="<?php echo $value['link_url']; ?>" onclick="downloadImage(this);">
...
<?php } else { ?>
    <style>
        .os_projct_overview .wbox_data .download_icon {
            cursor: text;
        }
    </style>
