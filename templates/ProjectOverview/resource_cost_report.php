        ...
        <tbody>
            ...
            if (!empty($resource_cost_details)) {
                foreach ($resource_cost_details as $k => $val) {
                    $actual_cost = (isset($val['actual_rate']) && !empty($val['actual_rate'])) ? $val['actual_rate'] . "&nbsp;<small>" . $val['InvoiceCustomers']['currency'] . "</small>" : 0;
                    $billing_cost = (isset($val['billable_rate']) && !empty($val['billable_rate'])) ? $val['billable_rate'] . "&nbsp;<small>" . $val['InvoiceCustomers']['currency'] . "</small>" : 0;
                    $total_actual_rate = (($val['hours'] / 3600) * $val['actual_rate']) != 0 ? round(($val['hours'] / 3600) * $val['actual_rate'], 2) . "&nbsp;<small>" . $val['InvoiceCustomers']['currency'] . "</small>" : 0;
                    $total_billing_rate = (int)(($val['hours'] / 3600) * $val['billable_rate']) != 0 ? round(($val['hours'] / 3600) * $val['billable_rate'], 2) . "&nbsp;<small>" . $val['InvoiceCustomers']['currency'] . "</small>" : 0;
                    $clnt_cmnpy_name = !empty($val['project_company_name']) ? $val['project_company_name'] : __('None');
            ?>
                    <tr <?php if ($k >= $limit) { ?> style="display:none;" <?php } ?> class="resource_tr">
                        <td class="width-14-per text-left"><span class="dynaic_elipse_data"><?php echo ucfirst($val['Projects']['name']); ?></span></td>
                        <td class="width-13-per text-left"><span class="dynaic_elipse_data"><?php echo ucfirst($clnt_cmnpy_name); ?></span></td>
                        <td class="width-13-per text-left"><span class="dynaic_elipse_data"><?php echo ucfirst($val['user_name']);
                                                                                            ?></span></td>
                        <td class="width-10-per text-left" style="color:#8e78f9"><?php echo $this->Format->format_time_hr_min($val['hours']) ?></td>
                ...
                <tr>
                    <td colspan="8">
                        <div class='mytask_txt colr_red text-center font16'><?php echo __("Oops! No Resource has logged time! What's happening"); ?>?</div>
                    </td>
                </tr>
            <?php } ?>
