...
if(isset($labelArr) && !empty($labelArr))
{
    ...
    foreach($labelArr as $label)
    {
        ...
        <li class="li_check_radio" <?php if($m > 5){$h++;?> id="hidLabel_<?php echo $h; ?>"  <?php }?>>
            <div class="checkbox">
                <label>
                    <input type="checkbox" class="label_type_cls" data-id="<?php echo $m; ?>" id="Label_<?php echo $m ?>" onClick="checkboxLabel('Label_<?php echo $m; ?>','check');filterRequest('label');" /> <?php echo $this->Format->formatText($LabelName); ?>
                </label>
