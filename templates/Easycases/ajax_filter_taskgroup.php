...
if(isset($groupArr))
{
    ...
    {
        // [TODO fix the key issue in query]
        $groups=explode("-",$CookieGroup??'');
        $m++;
        $groupId = $group['Milestone']['id'] ?? $group['id'];
        $groupName = $group['Milestone']['title'] ?? $group['title'];
        ?>
        <li class="li_check_radio" <?php if($m > 5){$h++;?> id="hidGroup_<?php echo $h; ?>"  <?php }?> >
            <div class="checkbox">
                <label>
                    <input type="checkbox" id="groups_<?php echo $m; ?>" class="group<?php echo $groupId; ?>" onClick="checkboxGroups('groups_<?php echo $m; ?>','check');filterRequest('groups');"  style="cursor:pointer;" <?php if (in_array($groupId, $groups) ) { echo "checked"; } ?>/> <?php echo $groupName; ?>
                </label>
