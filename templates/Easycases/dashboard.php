            ...
            $('body').ready(function() {
                setTimeout(function() {
                    if ($('.onboard-overlay').size() > 0) {
                        ...
                        $('.task_listing table').find('tr:nth-of-type(5) td:nth-child(2) .check-drop-icon').css({
                            'visibility': 'visible'
                        });
                        ...
                        $('.task_listing table').find('tr:nth-of-type(5) td:nth-child(2) .check-drop-icon .dropdown-toggle').next('ul').css({
                            'z-index': '998'
                        }).show();
                    }
                }, '5000');
...
<div id="easycaseDashboard" ng-app="case_dashboard_App">
    <div>
        ...
        <div id="kanban_list" class="kanban_section kanban_resp kanban_board_container kanban-new" style="display:block"></div>
        ...
        <div id="caseTimeLogDv" style="display:block">
            <div id="caseTimeLogViewSpan" class="pr" style="display:block"></div>
            ...
            <?php if (SES_TYPE == 3) { ?>
                <div class="tsk-typ-txt tsk-typ-txt-tg" style="display:none;">
                    <p style="margin:0px;"><img style="margin-right:5px;" alt="" src="<?php echo HTTP_ROOT; ?>img/idea-ico.png"><?php
                    echo sprintf(
                        '%s %s',
                        __('You are seeing your time logs only.'),
                        __('To view others time log please contact your account Owner/Admin')
                    );
                      ?>.</p>
                </div>
            <?php } ?>
        ...
        <div class="loader_bg" id="caseLoader">
            <div class="loadingdata">
                <img src="<?= $this->Url->build('/images/rolling.gif') ?>" alt="loading..." title="<?php echo __('loading'); ?>..." />
            </div>
        </div>
                ...
                <div class="col-lg-9 activity_ipad">
                    <div class="width100 dash-activity">
                        ...
                        <div id="more_loader" class="morebar">
                            <img src="<?= $this->Url->build('/img/images/case_loader2.gif') ?>" alt="loading..." title="<?php echo __('loading'); ?>..." />
                        </div>
                    </div>
                </div>
...
<script type="text/template" id="case_project_tmpl">
    <?php echo $this->element('case_project_v2'); ?>
</script>
...
<script type="text/template" id="checklist_close_tmpl">
    <?php echo $this->element('close_checklist'); ?>
</script>
<script type="text/template" id="case_meeting_tmpl">
</script>
...
<script type="text/template" id="list_thread_tmpl">
    <?php echo $this->element('case_list_tr_v2'); ?>
</script>
...
<script type="text/template" id="case_link_task_cmn_tmpl">
    <?php echo $this->element('case_link_task_new'); ?>
</script>
...
<input type="hidden" name="caseTaskgroup" id="caseTaskgroup" value="" size="4" readonly="true" />
<input type="hidden" name="caseComment" id="caseComment" value="" size="4" readonly="true" />
<input type="hidden" name="caseLabel" id="caseLabel" value="<?php echo $caseLabel ?? ''; ?>" readonly="true" />
<input type="hidden" name="caseCustomField" id="caseCustomField" value="<?php echo h($caseCustomField ?? ''); ?>" readonly="true" />
<input type="hidden" name="caseAssignTo" id="caseAssignTo" value="<?php echo $caseAssignTo ?? ''; ?>" size="4" readonly="true" />
<input type="hidden" name="caseBunit" id="caseBunit" value="<?php echo $caseBunit ?? ''; ?>" size="4" readonly="true" />
<input type="hidden" name="caseTeamUser" id="caseTeamUser" value="<?php echo $caseTeamUser ?? ''; ?>" readonly="true" />
<input type="hidden" name="caseEpics" id="caseEpics" value="<?php echo $caseEpics ?? ''; ?>" readonly="true" />
<input type="hidden" name="caseFeatures" id="caseFeatures" value="<?php echo $caseFeatures ?? ''; ?>" readonly="true" />
<input type="hidden" name="caseSkill" id="caseSkill" value="<?php echo $caseSkill ?? ''; ?>" readonly="true" />
...
<input type="hidden" name="caseId" id="caseId" value="" size="14" readonly="true" />
...
<input type="hidden" name="caseMenuFilters" id="caseMenuFilters" value="<?php echo $caseMenuFilters ?? 'cases'; ?>" size="4" readonly="true" />
...
<script type="text/javascript">
    ...
    function inArray(needle, haystack) {
        if (typeof haystack != 'undefined' && haystack != null) {
            ...
            if (length != 0) {
                ...
                // DF_224: Empty array means no columns selected - return false, not true
                return false;
            }
        } else {
            return true;
    ...
    function getAllowedColumns() {
        var checkedVals = $('.show_hide_selectedcols:checkbox:checked').map(function() { 
            // Class "show_hide_selectedcols" is used for Task list show/hide checkboxes
            return this.value;
        }).get();
        
        // Prevent saving with no columns selected
        if (checkedVals.length === 0) {
            showTopErrSucc('error', '<?php echo __("Please select at least one column"); ?>');
            return false;
        }
        
        // OSS edition: per-page column preferences are not persisted (the
        // task_fields table was removed). The show/hide toggle applies
        // client-side; just reload the list with the current selection.
        // (Previously POSTed to the removed task-fields/saveSelectedColumns
        // route, which 404'd.)
        reloadTasks();
    }
    ...
    function checkboxColumn(ev) {
        ...
        $(".show_hide_selectedcols").prop("checked", status); // Class "show_hide_selectedcols" is used for Task list show/hide checkboxes
