...
echo $this->Form->create(null , array('name' => 'taskcsvForm', 'id' => 'taskcsvForm', 'url' => $url, 'onsubmit' => "return validateCsvForm();"));
?>  
<div class="modal-body popup-container">
        ...
        <select id="changeProject"  name="data[Easycase][project]" class="select form-control floating-label" placeholder="Project" data-dynamic-opts=true  <?php if (intval($is_milestone)) { ?>onchange="change_milestone(this);change_member_assignto(this);milestone_export();"<?php } else { ?>onchange="change_member_assignto(this);"<?php } ?>>
            ...
            <?php if (isset($projArr)) { ?>
                <?php foreach ($projArr as $prj) { ?>
                    <option value="<?php echo $prj['id']; ?>" <?php if ($prj['uniq_id'] == $uniq_id) { ?>selected="selected"<?php } ?>><?php echo ucfirst($prj['name']); ?></option>
                <?php } ?>
            <?php } ?>
        </select>
    </div> 
    <?php if (intval($is_milestone)) { ?>
        <div class="form-group" id="milestone_dv">
            <select name="data[Easycase][milestone]" class="select form-control floating-label" placeholder="<?php echo __('Task Group'); ?>" data-dynamic-opts=true  onchange="milestone_export(this);" id="exportcsv_milestone">
    ...
    <div class="form-group">
        ...
        <select name="data[Easycase][types]" class="select form-control floating-label" placeholder="Types" data-dynamic-opts=true>
            ...
            <?php foreach ($typeArr as $key => $value) { ?>
                <option value="<?php echo $value['id']; ?>" <?php if (trim($types) == $value['id']) { ?>selected="selected"<?php } ?>><?php echo $value['name']; ?></option>
            <?php } ?>
        </select>
    </div>
	 <div class="form-group">
        ...
        <select name="data[Easycase][members]" class="select form-control floating-label" placeholder="Members" data-dynamic-opts=true>
             ...
             <?php if (isset($memArr)) { ?>
                 <?php foreach ($memArr as $mem) { ?>
                    ...
                    <option value="<?php echo $mem['id']; ?>" <?php if (in_array($mem['id'], $members)) { ?>selected="selected"<?php } ?>><?php echo ucfirst($mem['name']); ?></option>
                <?php } ?>
            <?php } ?>
        </select>
    </div>
            ...
            <?php if (isset($asnArr)) { ?>
                <?php foreach ($asnArr as $Asn) { ?>
                  <?php  $Asnbers = explode("-", $_COOKIE['ASSIGNTO']);?>
                    <option value="<?php echo $Asn['id']; ?>" <?php if (in_array($Asn['id'], $Asnbers)) { ?>selected="selected"<?php } ?>><?php echo ucfirst($Asn['name']); ?></option>
                <?php } ?>
            <?php } ?>
        </select>
    </div>
    <div class="form-group" id="tr_milestone_list">
        <select name="data[Easycase][milestone]" class="select form-control floating-label" placeholder="<?php echo __('Task Group'); ?>" data-dynamic-opts=true id="milestone_list">
            <option value="all"><?php echo __('All');?></option>
