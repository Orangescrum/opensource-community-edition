...
<?php  } else if ($nodata_name == 'filelist') {
    ...
    $msgDesc = '';
}else if (in_array($nodata_name, ['tasklist', 'epics', 'features'])) {
...
<div class="not-fonud ml_not_found">
	...
	<div class="no-data-box extra">
		<div>
			...
			<?php if(SES_TYPE < 3){ ?>
			<p class="sub-head"><?php echo __('You have not been assigned to a project. Please add a project and assign yourself to it.');?></p>
			<?php } else { ?>
		...
		<span class="m-left">
			<a class="btn btn_cmn_efect cmn_bg btn-info cmn_size" href="javascript:void(0)" onclick="newProject();"><?php echo __('Create Project');?><div class="ripple-container"></div></a>
		</span>
	<?php } }else if( !in_array($nodata_name, ['tasklist', 'caselist', 'epics', 'features'])){ ?>
	<h2><?php echo $msgHead ?? ''; ?></h2>
	<?php }else{ ?>
	<div class="no-data-box extra">
		<div>
			<p class="head"><?php echo __('Getting Started');?></p>
			<?php if ($nodata_name == 'epics') { ?>
				<p class="sub-head"><?php echo __('Create and manage epics for your project');?></p>
			<?php } else if ($nodata_name == 'features') { ?>
				<p class="sub-head"><?php echo __('Create and manage features for your project');?></p>
			<?php } else { ?>
		...
		<br /><br />
		 <?php
			...
			if ($this->Format->isAllowed($createAction, $roleAccess) || SES_TYPE == 1) { ?>
			<span class="m-left">
					<a class="btn btn_cmn_efect cmn_bg btn-info cmn_size" href="javascript:void(0)" onclick="creatask();"><?php echo __('Create New Task');?><div class="ripple-container"></div></a>
