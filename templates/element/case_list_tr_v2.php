...
%>
<tr class="row_tr tr_all trans_row 2 <% if(typeof mid != 'undefined'){ %>tgrp_tr_all<% } %><% if(easycase.getPriority(casePriority) == 'high'){ %> background_new<% } else { %><% } %>" id="curRow<%= caseAutoId %>" <% if(typeof mid != 'undefined'){ %>data-pid="<%= projId %>" data-mid="<%= mid %>" <% } %> data-is-parent="<%= caseDet.Easycase.is_parent %>">
    ...
    <td class="ctg_check_td pr" <% if(groupby =='' || groupby !='priority'){%>class="check_list_task tsk_fst_td pr_<%= easycase.getPriority(casePriority) %>" <% } %>>
        <div class="checkbox fl<% if (page_hash === 'tasks' && (caseTypeId == 13 || caseTypeId == 15)) { %> chk-not-allowed<% } %>">
            <label>
                <% if (page_hash === 'tasks' && (caseTypeId == 13 || caseTypeId == 15)) { %>
                <input type="checkbox" style="cursor:not-allowed" id="actionChk<%= count %>" value="<%= caseAutoId + '|' + caseNo + '|' + caseUniqId %>" class="fl mglt chkOneTsk" data_epic_type="<%= caseDet.Easycase.type_id == caseDet.Easycase.original_epic_id %>" disabled="disabled">
        ...
        <div class="check-drop-icon fl">
            <div class="dropdown cmn_tooltip_hover">
              ...
              if(typeof caseDet.customStatusByProject[caseDet.Easycase.project_id] !='undefined'){
                    ...
                    <li onclick="setCustomStatus(<%= '\'' + caseAutoId + '\'' %>, <%= '\'' + caseNo + '\'' %>, <%= '\'' + caseUniqId + '\'' %>,<%= '\'' + caseDet.lastCustomStatus.LastCS.id + '\'' %>,<%= '\'' + caseDet.lastCustomStatus.LastCS.status_master_id + '\'' %>,<%= '\'' + htmlspecialchars(caseDet.lastCustomStatus.LastCS.name)  + '\'' %>);" id="new<%= caseAutoId %>">
                        <a href="javascript:void(0);">
                            <span style="background-color:#<%= caseDet.lastCustomStatus.LastCS.color %>;height: 11px;width: 11px;display: inline-block;"></span>
                            <%= htmlspecialchars(caseDet.lastCustomStatus.LastCS.name) %></a>
                    </li>
                    <% } 
							}
                      }  
                    ...
                    <% if(caseDet.Easycase.type_id == caseDet.Easycase.original_epic_id){ %>
                    ...
                    <% } %>
                    <% } %>
                    <% if((isEpicTask(caseDet.Easycase.type_id , caseDet.Easycase.original_epic_id, caseDet.Easycase.actual_dt_created) && !(page_hash === 'tasks' && caseDet.Easycase.type_id == 15))){ %>
                    ...
                    <% if((isEpicTask(caseDet.Easycase.type_id , caseDet.Easycase.original_epic_id, caseDet.Easycase.actual_dt_created) && !(page_hash === 'tasks' && caseDet.Easycase.type_id == 15)) && caseDet.Easycase.type_id != 13){ %>
                    <li onclick="addSubtaskPopup(<%= '\'' + projectUniqid + '\'' %>,<%= '\'' + caseDet.Easycase.id + '\'' %>,<%= '\'' + caseDet.Easycase.project_id + '\'' %>,<%= '\'' + caseDet.Easycase.uniq_id + '\'' %>,<%= '\'' + htmlspecialchars(caseDet.Easycase.title) + '\'' %>);">
    ...
    <td>
        <span class="ttype_global tt_<%= getttformats(caseDet.Easycase.csTdTyp[1])%>" rel="tooltip" original-title="<%= htmlspecialchars(caseDet.Easycase.csTdTyp[1]) %>" style="margin-top:2px;"></span>
    ...
    <td <% if(groupby =='' || groupby !='priority'){%>class="check_list_task tsk_fst_td pr_<%= easycase.getPriority(casePriority) %>" <% } %>>
        <div class="checkbox<% if (page_hash === 'tasks' && (caseTypeId == 13 || caseTypeId == 15)) { %> chk-not-allowed<% } %>">
            <label>
                <% if (page_hash === 'tasks' && (caseTypeId == 13 || caseTypeId == 15)) { %>
                <input type="checkbox" style="cursor:not-allowed" id="actionChk<%= count %>" value="<%= caseAutoId + '|' + caseNo + '|' + caseUniqId %>" class="fl mglt chkOneTsk" data_epic_type="<%= caseDet.Easycase.type_id == caseDet.Easycase.original_epic_id %>" disabled="disabled">
                <% } else if(caseLegend != 3 && caseTypeId != 10) { %>
 ...
 if(caseDet.Easycase.isactive == 1){
	 if(isAllowed('Change Status of Task',projectUniqid)){
        if(caseDet.Easycase.CustomStatus.status_master_id != 3){ %>
                    ...
                    <% if(caseLegend !=3 && caseTypeId != 10){ %>
                    <% if((isEpicTask(caseDet.Easycase.type_id , caseDet.Easycase.original_epic_id, caseDet.Easycase.actual_dt_created) && !(page_hash === 'tasks' && caseDet.Easycase.type_id == 15))){ %>
                    <li id="start_qt<%= caseAutoId %>" onclick="startTimer(<%= '\'' + caseAutoId + '\'' %>,<%= '\'' + escape(htmlspecialchars(caseTitle,3)) + '\'' %>, <%= '\'' + caseUniqId + '\'' %>, <%= '\'' + projectUniqid + '\'' %>, <%= '\'' + escape(projectName) + '\'' %>); ">
                        <a href="javascript:void(0);"><i class="material-icons">&#xE425;</i><?php echo __('Start Timer'); ?></a>
                    </li>
                    <% } %>
                    <% } %>
    ...
    <td class="relative <% if(inArray('basicdetail',field_name_arr)){ %>list-cont-td<% } %>" <% if(count == 1){ %>id="tour_task_title_listing" <% } %>>
            ...
            <a href="javascript:void(0);" class="ttl_listing" data-task-id="<%= caseUniqId %>">
                ...
                <div class="task_dependancy_item">
                    ...
                    <div class="task_dependancy parenttt fr">
                        ...
                        <% if(caseDet.Easycase.parent_task_id && caseDet.Easycase.parent_task_id != "" && caseDet.Easycase.parent_task_id != 0){ %>
        ...
        <div class="list-td-hover-cont">
            <span class="created-txt"><% if(getTotRepCnt && getTotRepCnt!=0) { %><?php echo __('Updated'); ?><% } else { %><?php echo __('Created'); ?><% } %> <?php echo __('by'); ?> <%= caseDet.Easycase.usrShortName %> <% if(caseDet.Easycase.updtedCapDt && caseDet.Easycase.updtedCapDt.indexOf('Today')==-1 && caseDet.Easycase.updtedCapDt.indexOf('Y\'day')==-1) { %><?php echo __('on'); ?><% } %> <%= caseDet.Easycase.updtedCapDt %></span>
            <span class="list-devlop-txt dropdown">
                <a class="dropdown-toggle" <% if(isAllowed('Change Other Details of Task',projectUniqid)){ %> data-toggle="dropdown" href="javascript:void(0);" <% } %> data-target="#">
                    ...
                    <span id="showUpdStatus<%= caseAutoId %>" class="<% if(showQuickActiononList && caseDet.Easycase.isactive == 1){ %>clsptr<% } %>" title="<%= caseDet.Easycase.csTdTyp[1] %>">
                        <span class="tsktype_colr" id="tsktype<%= caseAutoId %>"><%= caseDet.Easycase.csTdTyp[1]%><span class="due_dt_icn"></span></span>
                    </span>
                </a>
            ...
            <% if(caseDet.Easycase.epic) { %>
            <div class="label epic-label" rel="tooltip" title="<%= caseDet.Easycase.epic %>"><%= caseDet.Easycase.epic %></div>
            <% } %>
            ...
            <span class="small-list-devlop-icon">
                <% var caseFlag=""; 					
											...
											if(caseDet.Easycase.isactive == 1){ 
											if(caseFlag == 2){ %>
                <% if(typeof customStatusByProject !="undefined" && typeof customStatusByProject[caseDet.Easycase.project_id] !='undefined' && customStatusByProject[caseDet.Easycase.project_id] != null){ }else{ %>
                <% if(isAllowed('Change Status of Task',projectUniqid)){ %>
                ...
                <% } }
											...
											if(caseFlag == "delete"){ %>
                <% if(isAllowed('Delete Task',projectUniqid) || isAllowed("Delete All Task",projectUniqid)){ %>
                <a href="javascript:void(0)" rel="tooltip" title="<?php echo __('Delete'); ?>" onclick="deleteCase(<%= '\'' + caseAutoId + '\'' %>, <%= '\'' + caseNo + '\'' %>, <%= '\'' + projId + '\'' %>, <%= '\'t_' + caseUniqId + '\'' %>, <%= '\'' + caseDet.Easycase.is_recurring + '\'' %>);">
                    <i class="material-icons">&#xE872;</i>
                </a>
                <% } %>
                <% } } %>
    ...
    <td class="text-center <% if(caseDet.Easycase.csTdTyp[1] != 'Update'){ %>task_priority csm-pad-prior-td<% }else{ %>csm-pad12-prior-td<% } %>">
        ...
        <div style="" id="pridiv<%= caseAutoId %>" data-priority="<%= casePriority %>" class="pri_actions <% if(isAllowed('Change Other Details of Task',projectUniqid)){ %> <% if(showQuickActiononList){ %> dropdown<% } %> <% } %>">
            <div class="dropdown cmn_h_det_arrow">
                <div class="quick_action resize" <% if(showQuickActiononList){ %> class="quick_action" <% if(isAllowed('Change Other Details of Task',projectUniqid)){ %> data-toggle="dropdown" <% } %> style="cursor:pointer" <% } %>><span class="priority_<%= easycase.getPriority(casePriority) %> prio_lmh prio-drop-icon" rel="tooltip" title="<?php echo __('Priority'); ?>:<%= easycase.getPriority(casePriority) %>"><% if(easycase.getPriority(casePriority) == 'high'){ %>High<% }else if(easycase.getPriority(casePriority) == 'medium'){ %>Medium<% }else if(easycase.getPriority(casePriority) == 'urgent'){ %>Urgent<% }else{ %>Low<% }  %></span><i class="tsk-dtail-drop material-icons case_list_tr dropdown_icon_priority">&#xE5C5;</i></div>
                <% var csLgndRep = caseDet.Easycase.legend; %>
    ...
    <td class="esthrs_dt_tlist text-center">
        <p class="estblist ttc <% if(!isAllowed('Est Hours',projectUniqid)){ %> no-pointer <% } %>" id="est_blist<%=caseAutoId%>" data-split="<%= caseDet.Easycase.is_splitted %>" style="cursor:pointer;" case-id-val="<%=caseAutoId%>">
    ...
    <td class="text-center" title="<% if(getTotRepCnt && getTotRepCnt!=0) { %><?php echo __('updated'); ?><% } else { %><?php echo __('created'); ?><% } %> <?php echo __('by'); ?> <%= caseDet.Easycase.usrShortName %> <% if(caseDet.Easycase.updtedCapDt && caseDet.Easycase.updtedCapDt.indexOf('Today')==-1 && caseDet.Easycase.updtedCapDt.indexOf('Y\'day')==-1) { %><?php echo __('on'); ?><% } %> <%= caseDet.Easycase.updtedCapDt %> <%= caseDet.Easycase.fbstyle %>."><%= caseDet.Easycase.fbstyle %></td>
    <% } %>
    ...
    <td class="due_dt_tlist" data-split="<%= caseDet.Easycase.is_splitted %>">
    ...
    <% for(var c_id in caseDet.custom_field_ids){ var cstm_val = "no"; if(caseDet.Easycase.custom_fields[caseDet.custom_field_ids[c_id]]){
        
		if(caseDet.Easycase.custom_fields[caseDet.custom_field_ids[c_id]]['CustomField']['placeholder'] == "variation" || caseDet.Easycase.custom_fields[caseDet.custom_field_ids[c_id]]['CustomField']['placeholder'] == "timeBalance" || caseDet.Easycase.custom_fields[caseDet.custom_field_ids[c_id]]['CustomField']['placeholder'] == "taskDuration"){ var cstm_val = caseDet.Easycase.custom_fields[caseDet.custom_field_ids[c_id]]['CustomFieldValue']['value']; } } %>
    <td class="customField_tlist text-center" <% if(cstm_val != 'no' && cstm_val !== 0){ if(caseDet.Easycase.custom_fields[caseDet.custom_field_ids[c_id]] && cstm_val.toString().charAt(0) == '-'){ %>style="color:#EB7154;" <% }else{ %>style="color:#54EB7B;" <% } } %>>
        <% if(caseDet.Easycase.custom_fields[caseDet.custom_field_ids[c_id]]){ %>
        <% if(caseDet.Easycase.legend != 3 && (caseDet.Easycase.custom_fields[caseDet.custom_field_ids[c_id]]['CustomField']['placeholder'] == "variation" || caseDet.Easycase.custom_fields[caseDet.custom_field_ids[c_id]]['CustomField']['placeholder'] == "taskCmplDate")){ %>
        --
        <% }else{ %>
        <%= caseDet.Easycase.custom_fields[caseDet.custom_field_ids[c_id]]['CustomFieldValue']['value']  %>
        <% } %>
        <% }else{ %>
        --
        <% } %>
