            ...
            <div class="mtop15" id="inner_tasktype" style="display: none;">
                ...
                <form name="task_type" id="customTaskTypeForm" method="post" action="<?php echo HTTP_ROOT . "projects/addNewTaskType"; ?>" autocomplete="off">
                    <div class="select_field_wrapper mbtm15 mark_mandatory">
                        <div class="project-select-with-arrow">
                            <select name="data[Type][project_id][]" multiple="multiple" class="task_type_project form-control floating-label" id="project_task_type"  placeholder="<?php echo __('Projects');?>">
                                ...
                                <?php } ?>
                                <?php if(is_array($GLOBALS['getallprojForAdmin']) && $GLOBALS['getallprojForAdmin']) foreach($GLOBALS['getallprojForAdmin'] as $k=>$v){ ?>
                                    <option value="<?php echo $v['Project']['id'];?>"><?php echo $v['Project']['name'];?></option>
                                <?php } ?>
                            </select>
                            <span class="prjct-dropdown-arrow" onclick="$(this).toggleClass('rotate');$('#project_task_type').select2('open');"><i class="material-icons">arrow_drop_down</i></span>
                        </div>
                    </div>
                    <div class="field_wrapper">
                    <label class="control-label mark_mandatory task-type-label" for="select_role"><?php echo __('Specify task type name');?></label>
                    ...
                    <div class="field_wrapper">
                    <label class="control-label mark_mandatory task-type-label" for="select_role"><?php echo __('Give a short name for task type');?></label>
