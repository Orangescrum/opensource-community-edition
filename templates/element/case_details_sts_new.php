<?php  $dtl_opt = (isset($popup) && $popup == 1) ? 'popup' : ''; ?>
<div class="d-flex width-100-per align-item-center">
																...
																<% if(is_active){%>
																	<div class="dropdown cmn_h_det_arrow">
																		<div class="opt1" id="opt20">
																			<% var more_opt = 'more_opt20'; %>
																				<div class="status_tdet">
																					<a  class="ellipsis-view open_status quick_action custom_sts_drop_v<%= csAtId %>" href="javascript:void(0);" onclick="open_more_opt('<%= more_opt %>',<%= '\''+csAtId+'\'' %>)">
																						<% $.each(cust_sts_list, function(k_y, v_l) { 
																							if(v_l.id == custom_status_id){ %>
																							<span style="background-color:#<%= v_l.color %>;height: 8px;width: 8px;display: inline-block;margin-right:2px;"></span>
																							<span rel="tooltip" id="detail_csm_sts_drp" title="<%= v_l.name %>"><%= shortLength(v_l.name,10) %></span>
																								<% progress = v_l.progress; %>
																									<% bg_sts_colr = v_l.color; %>
																										<% } }); %>
																											<i class="tsk-dtail-drop material-icons">&#xE5C5;</i>
																					</a>
																				</div>
																		...
																		<% if(is_inactive_case == 0 && is_active == 1) {%>
																			<ul class="dropdown-menu dropdown_tg_sts">
																				<% $.each(cust_sts_list, function(k_y, v_l) { %>
																					<% if(custom_status_id != v_l.id){ %>
																						<% if(v_l.status_master_id == 3){ %>
																							<?php if ($this->Format->isAllowed('Status change except Close', $roleAccess)) { ?>
																							<li class="">
																								<a href="javascript:void(0);" onclick="setCustomStatus(<%= '\''+csAtId+'\'' %>, <%= '\''+csNoRep+'\'' %>, <%= '\''+csUniqId+'\'' %>, <%= '\''+v_l.id+'\'' %>, <%= '\''+v_l.status_master_id+'\'' %>, <%= '\''+v_l.name+'\'' %>);">
																									<span style="background-color:#<%= v_l.color %>;height: 11px;width: 11px;display: inline-block;"></span>
																									<%= v_l.name %>
																								</a>
																							</li>
																							<?php } ?>
	...
	<div class="width-50-per">
	<div id="" class="detail_fld_label">
					<?php echo __('% Completion');?>
				</div>
		<div class="d-flex align-item-center task-progress" id="pgrsdiv<%= csAtId %>">
