...
<tr class="row_tr tr_all trans_row parent_tr " id="curRow_subtask_<%= caseAutoId %>" data-mid="<%= d_mid %>">
                  ...
                  <input type="hidden" id="actionCls<%= caseAutoId %>" value="1" disabled="disabled" size="2">     
                     ...
                     <div class="dropdown"> 
                      ...
                      <% } %>
                           ...
                           <% } %>
                      <?php } ?>
                      <li data-prjid="<%= projId %>" data-caseid="<%= caseAutoId %>" data-caseno="<%= caseDet.Easycase.caseNo %>"  id="cppy_checklist<%= caseAutoId %>" onclick="copyChecklist(<%= '\'' + count + '\'' %>,this);">
					  ...
					  <% if(isAllowed("Create Task",projectUniqid)){ %>
                                    ...
                                    <li onclick="addSubtaskPopup(<%= '\'' + projectUniqid + '\'' %>,<%= '\'' + caseAutoId + '\'' %>,<%= '\'' + caseDet.Easycase.project_id + '\'' %>,<%= '\'' + caseDet.Easycase.uniq_id + '\'' %>,<%= '\'' + caseDet.Easycase.title + '\'' %>);">
					  ...
					  <% }  %>
                            ...
                            <li onclick="startTimer(<%= '\'' + caseAutoId + '\'' %>,<%= '\'' + escape(htmlspecialchars(caseTitle,3)) + '\'' %>, <%= '\'' + caseUniqId + '\'' %>,<%= '\'' + projUniq + '\'' %>,<%= '\'' + escape(htmlspecialchars(projectName,3)) + '\'' %>); trackEventWithIntercom(<%= '\'timer\'' %>,<%= '\'\'' %>);">
            ...
            <% } %>
               ...
               <td class="esthrs_dt_tlist text-center">
                  <p class="<?php if($this->Format->isAllowed('Est Hours',$roleAccess)){ ?> estblists <?php } ?> estblist_subtask" <?php if($this->Format->isAllowed('Est Hours',$roleAccess)){ ?> style="cursor:pointer;" <?php } ?> id="est_blist_sub<%= caseAutoId %>" data-split="<%= caseDet.Easycase.is_splitted %>" case-id-val="<%= caseAutoId %>">    
               ...
               <td class="due_dt_tlist text-center" data-split="<%= caseDet.Easycase.is_splitted %>">
