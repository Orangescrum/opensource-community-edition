...
<table>	
	<tbody>
		<% if(parseInt(mileSummaryList.length) > 0){ %>
			<% 
			var count=0;
			for(var sKey in mileSummaryList){
				var data_summary = mileSummaryList[sKey];
				count++;
				%>
				<tr>
					<td><%= formatText(data_summary.project_name) %></td>
					<td><%= data_summary.milestone_name %></td>
					...
					<td>
						<div class="radial_count_chat">
							...
							<div class="milestone_circle circle_percent" data-percent="<%= data_summary.percentage %>">
