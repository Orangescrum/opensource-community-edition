...
<div class="row pr" <?php if(defined('COMP_LAYOUT') && COMP_LAYOUT){ ?>style="margin-top: -28px;"<?php } ?>>
    <div class="pr">
        ...
        <?php }else{ ?>
            ...
            <% }else{ %>
            <a href="<?php echo HTTP_ROOT;?>dashboard#milestonelist" class="tgkankan hide" style="position: absolute; right: 258px;top: 0px;"><i class="material-icons">&#xE862;</i> <?php echo __('Task Group View');?></a>
            <% }
            }%>
    ...
    <span style="display: flex;cursor:pointer;flex-direction: row-reverse;padding: 15px 70px 0px 0px;"><div style="font-size:18px;color:#2d6dc4;"onclick="showOverDueTask();"><?php echo __("Overdue Tasks")  ?> : <%= over_due_task_count %></div></span>
...
<div class="kanban-auto-scroll">
    ...
    <div class="kanban-main row <% if(mlstId){ %>mtop10<% } %> kanban_mcnt task_grp kanban-view-lists">
        <%
        }
        var clscnt=1;
        var tasktype_colr = "";
        ...
        if(!morecontent){
        %>
        <div class="col-lg-3 kanban-child <% if(mlstId){ %>tsms-autoheight<%}else{%>ts-autoheight<%}%> kanban-<%= clscnt++ %>" id="<%= taskallkey %>">
            <div class="kanban_cnt_det">
                ...
                <h5 class="<%= tasktype_colr_cls %>" style="cursor:default;">
                    <%= tasktype %> (<div style="display:inline;" id="cnter_<%= taskallkey %>"><% if(typeof caseAll[taskallkey] != 'undefined'){ %><%= caseAll[taskallkey].length%><% }else{ %>0<% } %></div>)
                    <span class="cb"></span>
                </h5>
            </div>
            ...
            <div class="add_new_item kb_task_det last_title">
                ...
                <div class="add_tgle_fld">
                    <div class="form-group label-floating">
                        ...
                        <% if(mlstId && mlstId!="0"){ %>
                        <input data-mid="<%= mlstId %>" data-custom-type="<%= proj_with_custom_sts %>" class="inline_qktask<%= mlstId %> in_qt_kanban form-control add_task_fld add_task_fld_kbn custom_task_entry_<%= proj_with_custom_sts %>" type="text" onkeypress="calltoquicktask(<%= mlstId %>,<%= '\'tg\'' %>,event);" id="inline_qktask_sts_tg"/>
                        <% }else{ %>
            ...
            <div class="kban_all_task kanban_content" id="kanban-short-<%=  tasktype %>" >
            	...
            	<span class="case-title-kbn-<%= caseAutoId %>">
            		...
            		#<%= caseNo %>: <%= caseTitle %></span>
                            <span class="rt_icn">
                    <span class="dropdown">
                    <% if(getdata.Easycase.type_id != 13 && getdata.Easycase.type_id != 15) { %>
                ...
                $.each(customStatusByProject[getdata.Easycase.project_id], function (key, data) {
                    ...
                    <% } %>
                            ...
                            <% }
                                ...
                                <% if(isAllowed('Create Task',projectUniqid)){
                                  	if((getdata.Easycase.is_sub_sub_task==null) || (getdata.Easycase.is_sub_sub_task=='')){ %>
                                        ...
                                        <% if(isEpicTask(getdata.Easycase.type_id , getdata.Easycase.original_epic_id, getdata.Easycase.actual_dt)){ %>
                                    <li onclick="addSubtaskPopup(<%= '\'' + projectUniqid + '\'' %>,<%= '\'' + getdata.Easycase.id + '\'' %>,<%= '\'' + getdata.Easycase.project_id + '\'' %>,<%= '\'' + getdata.Easycase.uniq_id + '\'' %>,<%= '\'' + getdata.Easycase.title + '\'' %>);">
								...
								<% if(caseParenId){ %>
										  <?php if($this->Format->isAllowed('Change Other Details of Task',$roleAccess)){ ?>
                                              <% if(isEpicTask(getdata.Easycase.type_id , getdata.Easycase.original_epic_id, getdata.Easycase.actual_dt)){ %>
                                              <li onclick="convertToParentTask(<%= '\''+ caseAutoId+'\'' %>);" id="convertToTask<%= caseAutoId %>" style=" <% if(showQuickActiononList){ %>display:block <% } else { %>display:none<% } %>">
                            ...
                            <% } %>
                                ...
                                <li id="bk_act_reply<%= count %>" data-task="<%= caseUniqId %>" onclick="replyFromKanban(this);">
                            ...
                            <% } %>
                                ...
                                <% if((isAllowed('Edit Task',projectUniqid) && showQuickActiononListEdit) || isAllowed('Edit All Task',projectUniqid)){ %>
                                    ...
                                    <li onclick="editepic(<%= '\''+ caseUniqId+'\',\''+projectUniqid+'\',\''+htmlspecialchars(projectName)+'\'' %>);" id="edit<%= caseAutoId %>" style=" <% if(caseFlag == 3){ %>display:block <% } else { %>display:none<% } %>">
                                        <a href="javascript:void(0);"><i class="material-icons">&#xE254;</i><?php echo __('Edit');?></a>
                                </li>
                            <% }else{ %>
				...
				<div id="titlehtml<%= count %>" data-task="<%= caseUniqId %>" class="fl case-title">
					...
					<% if(getdata.Easycase.format != 2) { %><span title="files/attachments" class="att_fl" style="    display: inline-block;position: relative;top: 5px;left: 4px;"></span><% } %>
                    <% if(getdata.Easycase.is_recurring == 1 || getdata.Easycase.is_recurring == 2){ %><span class="task_title_icons_reoccurrence" rel="tooltip" title="<?php echo __('Recurring Task'); ?>"></span><% } %>
					</span>
                </div>
        ...
        <?php /*<div class="kb_task_det last_title">
            <div class="form-group label-floating">
              ...
              <input data-mid= "<%= mlstId %>" class="inline_qktask<%= mlstId %> in_qt_kanban form-control"  type="text" onkeypress="calltoquicktask(<%= mlstId %>,<%= '\'tg\'' %>,event);"/>
            </div>
        </div>*/?>
        <% }else{ %>
        <% if(projUniq !='all'){ %>
...
<div class="crt_task_btn_btm">
    ...
    <div class="os_plus">
        ...
        <a href="javascript:void(0)" <% if(mlstId){ %>onclick="creatask(<%= mlstId %>);"<% }else{ %>onclick="creatask();"<% } %>>
