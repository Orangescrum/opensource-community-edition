<div class="cmn_sec_card selected  mtop20" id="cmt_sec">
	...
	<div class="toggle_details">
		<div id="tour_detl_logs<%= csUniqId %>" class="">
			<input type="hidden" name="data[Easycase][sel_myproj]" id="CS_project_id<%= csAtId %>"
				value="<%= projUniqId %>" readonly="true">
			<input type="hidden" name="data[Easycase][myproj_name]" id="CS_project_name<%= csAtId %>"
				value="<%= htmlspecialchars(projName) %>" readonly="true">
			<input type="hidden" name="data[Easycase][case_no]" id="CS_case_no<%= csAtId %>" value="<%= csNoRep %>"
				readonly="true" />
			<input type="hidden" name="data[Easycase][type_id]" id="CS_type_id<%= csAtId %>" value="<%= csTypRep %>"
				readonly="true" />
			<input type="hidden" name="data[Easycase][title]" id="CS_title<%= csAtId %>" value="" readonly="true" />
			<input type="hidden" name="data[Easycase][priority]" id="CS_priority<%= csAtId %>" value="<%= csPriRep %>"
				readonly="true" />
			<input type="hidden" name="data[Easycase][org_case_id]" id="CS_case_id<%= csAtId %>" value="<%= csAtId %>"
				readonly="true" />
			<input type="hidden" name="data[Easycase][istype]" id="CS_istype<%= csAtId %>" value="2" readonly="true" />
			<div class="cb"></div>

			<% if(is_active){ %>
				<div class="" id="reply_box<%= csAtId %>" <?php if ($this->Format->isAllowed('Reply on Task', $roleAccess)) { ?> style="display:block;" <?php } else { ?>style="display:none;" <?php } ?>>
					<div class="user_task_comments pr align-item-center d-flex">
						...
						<div class="task_comment_input">
							<div>
								<span <% if(is_inactive_case==0 && is_active==1) {%> id="showhtml<%= csAtId %>" <% } %>
											data-task="<%= csAtId %>">
												<input type="text" name="data[Easycase][message]"
													id="<%= 'txa_comment'+csAtId %>"
													placeholder="Write your comment here ..." class="editor_input">
								</span>
							</div>
							<div class="comment_form_editor" id="cmt_sec_dis" style="display:none;">
								<div class="">
									<span id="html<%= csAtId %>" style="display:block;">
										<span class="d-block" id="hidhtml<%= csAtId %>">
											<textarea name="data[Easycase][message]" id="<%= 'txa_comments'+csAtId %>"
												rows="2" class="form-control txt_are"></textarea>
											<span id="htmlloader<%= csAtId %>"
												style="color:#999999; display: none; float:left;">
												<?php echo __('Loading'); ?>...</span>
										</span>

									</span>
									<span id="plane<%= csAtId %>" style="display:none;">
										<textarea name="data[Easycase][message]" id="txa_plane<%= csAtId %>" rows="3"
								...
								<?php if ($this->Format->isAllowed('Upload File to Task', $roleAccess)) { ?>
									<div class="">
										<input type="hidden" name="totfiles" id="totfiles<%= csAtId %>" value="0"
											readonly="true" />
										<form class="upload<%= csAtId %> attch_form" id="file_upload<%= csAtId %>"
											action="<?php echo HTTP_ROOT; ?>easycases/fileupload/?<?php echo time(); ?>"
...
">
											...
											<div class="drag_and_drop" id="holder_detl" style="min-height:125px;">
													...
													<div class="fr">
															...
															<div class="dropbox-gdrive" style="display:inline-block;vertical-align:middle;">
																<a href="javascript:void(0)"
																	onclick="openCloudStoragePicker(<%= csAtId %>);"
																	title="<?php echo __('Link from Cloud Storage'); ?>">
																	<span class="os_sprite g-drive"></span>
																</a>
															...
															<?php if (\Cake\Core\Plugin::isLoaded('Dms') && CONTROLLER == 'easycases' && PAGE_NAME == 'dashboard') { ?>
																<?= $this->element('Dms.dms_picker_button', ['scope' => 'comment', 'taskIdJs' => 'csAtId']) ?>
															<?php } ?>
													</div>
													<div class="cb"></div>
												</header>
												<div class="file_upload_detail drop-file">
													...
													<div class="customfile-button">
														<input class="customfile-input" name="data[Easycase][case_files]"
															id="tsk_attach<%= csAtId %>" type="file" multiple="" />
													</div>
													<small>
										...
										<div id="table1">
											<table class="up_files<%= csAtId %>" id="up_files<%= csAtId %>"
												style="font-weight:normal;"></table>
										</div>
										<div id="drive_tr_<%= csAtId %>">
											<form id="cloud_storage_form_<%= csAtId %>"
												name="cloud_storage_form_<%= csAtId %>" action="javascript:void(0)"
												method="POST">
												<div style="float: left;margin-top: 7px;"
													id="cloud_storage_files_<%= csAtId %>"></div>
											</form>
											<div style="clear: both;margin-bottom: 3px;"></div>
										</div>
										<?php if (\Cake\Core\Plugin::isLoaded('Dms') && CONTROLLER == 'easycases' && PAGE_NAME == 'dashboard') { ?>
											<?= $this->element('Dms.dms_picker_list', ['scope' => 'comment']) ?>
										<?php } ?>
									</div>
								<?php } ?>
								...
								<div class="">
									<div class="detail-notify-email">
										<div class="item_title_heading notify-email-toggle"
											onclick="toggleNotifyEmail(this)"
											title="<?php echo __('Choose who gets an email about this comment'); ?>">
											<?php echo __('Notify via Email'); ?>
											<div class="icon_collapse notify-email-caret"></div>
										</div>
										<div class="notify-email-body">
										<div class="checkbox custom-checkbox all-check-box">
											<label>
												<input type="checkbox" name="chkAllRep" id="<%= csAtId %>chkAllRep"
													value="all" class="" onclick="checkedAllResReply('<%= csAtId %>')"
										...
										<% var i=0; var UserClients_dtl='' ; var client_span_cont=[]; var
											client_span_contId=[]; %>
											<% if(countJS(allMems)){ %>
												<div id="mem<%= csAtId %>">
													<div id="viewmemdtls<%= csAtId %>" class="d-flex">
														<% for(var memkey in allMems){ %>
															<% var getAllMems=allMems[memkey];
																getAllMems.CompanyUser=getAllMems.CompanyUsers;
																getAllMems.User=getAllMems; %>
																<% if(getAllMems.CompanyUser.is_client=="1"
																	){client_span_cont.push(getAllMems.User.name);
																	client_span_contId.push(getAllMems.User.id); } %>
																	<% if(getAllMems.User.is_client !=1 ||
																		(client_status !=1)){ %>
																		<% var j=i%3; %>
																			<div
																				class="checkbox custom-checkbox  add-user-pro-chk">
																				<label>
																					<input type="hidden"
																						name="data[Easycase][proj_users][]"
																						id="proj_users"
																						value="<%= getAllMems.User.id %>"
																						readonly="true" />
																					<input type="checkbox"
																						name="data[Easycase][user_emails][]"
																						id="<%= csAtId %>chk_<%= getAllMems.User.id %>"
																						value="<%= getAllMems.User.id %>"
																						style="cursor:pointer;"
																						class="chk_fl <% if(getAllMems.CompanyUser.is_client== "
																						1"){%>chk_client<%}%>"
																						onClick="removeAllReply('<%=
																							csAtId %>')" <%
																								if($.inArray(getAllMems.User.id,usrArr)!=-1){
																								%> checked<% } %> />
																				...
																				<% } %>
																					<% }%>
																						<% var client_emails_show='' ;
																							...
																							if(UserClients_dtl.length>=
																							19){
																							client_emails_show =
																							'('+UserClients_dtl.substr(0,16)+'...)';
																							}else{
																							client_emails_show =
																							'('+UserClients_dtl+')';
																							}
																							...
																							<input type="hidden"
																								name="hidtotresreply"
																								id="hidtotresreply<%= csAtId %>"
																								value="<%= i %>"
																								readonly="true" />
													</div>
												</div>
												<% } %>
									</div><!-- /.notify-email-body -->
									</div>
											...
											<div id="clientdiv_dtl<%= csAtId %>" class="checkbox">
												<label class="mb-15">
													<input type="checkbox" name="chk_all"
														id="make_client_dtl<%= csAtId %>" value="0"
														onclick="chk_client_reply(<%= csAtId %>);" />
													<?php echo __('Do not show this comment to the client'); ?>&nbsp;
												</label>
												<% if(client_span_cont){ for(var csk in client_span_cont){ %>
									...
									<div class="ml-auto">
										<div class="d-inline-block" id="postcomments<%= csAtId %>">
											<span class="d-inline-block">
												<a class="btn btn-default btn_hover_link cmn_size toglle_on_click"
													data-cs_id="<%= csAtId %>" href="javascript:void(0)" id="rset">
													<?php echo __('Cancel'); ?>
												</a>
											</span>
											<?php if ($this->Format->isAllowed('Reply on Task', $roleAccess)) { ?>
												<span class="d-inline-block">
													<button
														class="btn btn_cmn_efect cmn_bg btn-info cmn_size comnt_submit_btn toglle_on_click"
														type="submit" name="data[Easycase][postdata]"
				...
				<div>
					<div class="d-flex text-right mt-30">
						<div class="ml-auto">
							<div class="d-flex">
								<div class="view_rem">
									<a id="morereply<%= csAtId %>" style="<% if(cntdta > 10) { %>display:none<% } %>;"
										class="cmn_outline_cta" href="javascript:void(0);"
									...
									<span id="hidereply<%= csAtId %>" <% if(cntdta <=10) { %> style="display:none"<% }
											%>>
											<a class="cmn_outline_cta" href="javascript:void(0);"
												onclick="showHideMoreReply(<%= '\''+csAtId+'\',\'less\'' %>)">
								...
								<div class="view_rem">
									<span id="repsort_desc_<%= csAtId %>" <%=ascStyle %>>
										<a href="javascript:void(0);" class="cmn_outline_cta"
											onclick="sortreply(<%= '\''+csAtId+'\'' %>,<%= '\''+csUniqId+'\'' %>)"
											rel="tooltip" title="<?php echo __('View oldest thread on top'); ?>">
											<?php echo __('Older'); ?>
										</a>
									</span>
									<span id="repsort_asc_<%= csAtId %>" <%=descStyle %> >
										<a href="javascript:void(0);" class="cmn_outline_cta"
											onclick="sortreply(<%= '\''+csAtId+'\'' %>,<%= '\''+csUniqId+'\'' %>)"
											rel="tooltip" title="<?php echo __('View newest thread on top'); ?>">
											<?php echo __('Newer'); ?>
										</a>
									</span>
									<span class="rep_st_icn"></span>
