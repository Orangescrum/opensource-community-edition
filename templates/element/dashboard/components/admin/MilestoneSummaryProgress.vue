<template>
  <WidgetCard :title="$t('Milestone Overall Progress')" :loading="loading">
    ...
    <div class="db-stats">
      <div class="db-stats__item db-stats__item--total"><strong>{{ graph.total || 0 }}</strong>{{ $t('Total Milestones') }}</div>
...
const { data, loading } = useMilestoneSummaryData()
