...
const filters = ref({ prog_id: '', proj_id: '', manager_id: '', status: '' })
...
export function useProjectSummaryData() {
  ...
  async function fetchData() {
    ...
    error.value = null
    try {
      const res = await api.post('my-dashboards/project-summary', filters.value)
  ...
  return {
    ...
    filters,
    fetchData,
    programList: config.filters?.program_list || {},
    projectList: config.filters?.project_list || {},
    mgrList: config.filters?.proj_mgr_list || {},
